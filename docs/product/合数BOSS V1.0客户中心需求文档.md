...
标签管理采用“统一展示、分源只读”模式，页面只包含标签库和标签组两个页签。标签来源固定为系统和企业微信，不允许扩展其他来源。
...
标签库列表展示标签名称及稳定ID、目前覆盖客户数、所属标签组、来源、状态和只读操作，不展示对象、分类、生成方式和有效期。支持按名称、ID、来源系统搜索，并按来源和状态筛选；点击覆盖数量可在当前数据权限范围内查看明细和导出。系统标签由业务事实自动产生，不允许人工创建；企业微信标签通过回调、定时任务或“同步企微”补拉，按企业微信标签ID幂等更新。

标签组用于客户画像分区和页面筛选，可配置组名、包含标签和启停状态。标签组不配置标签对象、维护组织及排序；候选项来自当前有效标签库，不得保留已不存在的标签引用。新增、编辑及标签关联变化须持久保存并记录操作人、变更前后值和时间；标签加入或移出标签组不改变标签来源及历史关系。
...
- [ ] 标签组不展示已从标签库移除的幽灵标签；标签和标签组历史关系、来源ID及变更日志完整保留；
- [ ] 标签和等级变更保留历史；
