# 合数BOSS V1.0——客户中心详细需求

| 文档属性 | 内容 |
|---|---|
| 所属项目 | 合数BOSS重构项目 |
| 所属版本 | V1.0 |
| 一级菜单 | 客户中心 |
| 文档版本 | V1.18 |
| 文档状态 | 产品需求评审稿 |
| 更新日期 | 2026-09-09 |

> 客户生命周期、客户360°档案、服务人员流转及详细数据模型以 [合数BOSS客户360°档案与生命周期需求](./合数BOSS客户360档案与生命周期需求文档.md) 为专项基线，本文负责客户中心各菜单页面和操作要求。

> **V1.14 当前生效补充：**客户生命周期仅保留“线索、意向、成交”三个阶段：引流线索进入即建立客户经营档案并标记为线索客户；发生可追溯挖需动作后转为意向客户；存在支付成功且有效的成交订单后转为成交客户。手机号与 UnionID 跨档案冲突不阻断客户经营档案建立和后续业务记录，只冻结冲突身份的自动覆盖/合并并同步创建撞单案件。客户列表“建档时间”统一为“客户创建时间”；基本档案展示昵称、手机号预览、客户地址、性别和客户编号，不展示企微客户ID与UnionID。

> **V1.15 当前生效补充：**客户列表中的“客户创建时间”统一更名为“客户添加时间”，继续使用客户主档的 `created_at` 字段，不迁移、不重算历史数据。该调整仅作用于客户列表展示及关联客户选择场景；客户总览的统计筛选仍沿用“客户创建时间”口径。

> **V1.17 当前生效补充：**标签管理只保留标签库和标签组，来源固定为系统、企业微信，两类标签在BOSS侧均为只读；系统标签由业务事实产生，企业微信标签通过同步进入，不提供新增、编辑、启停或删除标签入口。标签组支持新增、编辑、启停，可包含当前标签库任意有效标签，不配置标签对象、维护组织或排序；保存后的标签组及标签关联必须稳定保留。后文与本补充冲突的旧表述均不再生效。

> **V1.18 当前生效补充：**标签状态枚举仅保留“启用、停用”，“已同步”不再作为状态；同步成功只作为同步操作结果提示。标签ID由系统生成且全局唯一，生成后不可修改、回收或复用。标签管理页面及需求范围不提供标签日志或标签操作日志的展示入口。后文与本补充冲突的旧表述均不再生效。

## 客户中心整体流程图

```mermaid
flowchart TD
    A["引流线索进入系统"] --> A1["自动保存线索记录"]
    A1 --> A2["客户生命周期：线索客户"]
    A2 --> B["标准化身份信息<br/>手机号、UnionID、external_userid"]
    B --> C{"查询现有客户身份"}

    C -->|"手机号与 UnionID<br/>均未命中"| D["创建正式客户档案<br/>生成唯一客户编号"]

    C -->|"仅命中一个客户<br/>或共同命中同一客户"| E["关联已有客户档案<br/>补充身份及线索关系"]

    C -->|"手机号与 UnionID<br/>分别命中不同客户"| F["创建待核验客户档案<br/>状态：身份冲突"]

    subgraph COLLISION["撞单管理：不阻断建档，只阻断冲突身份自动绑定与合并"]
        F --> F1["保存本次业务数据<br/>线索、问卷、订单、来源"]
        F --> F2["自动创建撞单案件<br/>状态：待处理"]

        F2 --> F3["关联三方档案"]
        F3 --> F31["手机号命中客户"]
        F3 --> F32["UnionID 命中客户"]
        F3 --> F33["本次待核验客户"]

        F31 --> F4["领取案件<br/>状态：处理中"]
        F32 --> F4
        F33 --> F4

        F4 --> F5["核验身份、来源、订单、问卷<br/>企微关系、负责人和时间轴"]
        F5 --> F6{"人工裁决结果"}

        F6 -->|"属于手机号客户"| F7["手机号客户作为主档"]
        F6 -->|"属于 UnionID 客户"| F8["UnionID 客户作为主档"]
        F6 -->|"属于新的独立自然人"| F9["待核验档案转为正式客户"]
        F6 -->|"两个历史档案实际为同一人"| F10["选择可信主档并执行客户合并"]

        F7 --> F11["迁移本次线索、问卷、订单<br/>标签、服务关系与时间轴"]
        F8 --> F11
        F10 --> F12["迁移从档全部业务关系"]
        F12 --> F13["从档标记已合并<br/>保留历史客户编号与审计"]

        F11 --> F14["关闭待核验客户档案"]
        F13 --> F14
        F9 --> F15["保留独立客户档案<br/>解除身份冲突状态"]

        F14 --> F16["撞单案件：已解决"]
        F15 --> F16
    end

    D --> G["建立客户与来源线索关系"]
    E --> G
    F16 --> G
    G --> H["形成可经营的唯一客户主档"]

    subgraph WECOM["企业微信客户同步"]
        W1["员工添加企微客户"] --> W2["接收客户联系事件回调<br/>external_userid + userid"]
        W2 --> W3["校验、解密并快速返回成功"]
        W3 --> W4["异步获取客户详情"]
        W4 --> W5{"本地是否已有对应客户"}

        W5 -->|"已存在"| W6["更新客户基本资料"]
        W5 -->|"不存在"| W7["创建或关联客户档案"]

        W6 --> W8["保存员工跟进关系"]
        W7 --> W8
        W8 --> W9["同一客户允许关联多个员工<br/>external_userid + userid 唯一"]
        W9 --> W10["同步添加时间、add_way、state<br/>备注、描述及企微标签"]

        W11["首次接入或全量对账"] --> W12["全量同步<br/>员工列表 → 客户列表 → 客户详情"]
        W13["日常补偿"] --> W14["增量同步<br/>同步上次完成后的变化"]

        W12 --> W4
        W14 --> W4
        W10 --> W15["记录检查、新增、更新和失败数量"]
        W15 --> W16["失败延迟重试<br/>定时全量对账补偿"]
    end

    W7 --> H
    W16 --> H

    H --> I["基本档案"]
    I --> I1["昵称、手机号、地址、性别"]
    I --> I2["客户编号、企微客户ID、UnionID"]
    I --> I3["手机号默认脱敏<br/>授权后预览"]

    H --> J["线索记录"]
    J --> J1["线索编号、来源、所属期次"]
    J --> J2["当前状态、创建时间"]

    H --> K["问卷测评"]
    K --> K1["答卷编号、问卷名称、填写来源"]
    K --> K2["得分、评定等级、提交时间"]

    H --> L["学习交付"]
    L --> L1["课程、班级、交付状态"]
    L --> L2["到课、完课及服务结果"]

    H --> M["服务人员"]
    M --> M1["当前主负责人"]
    M --> M2["归属组织"]
    M --> M3["历史服务关系"]

    I --> N["客户时间轴"]
    J --> N
    K --> N
    L --> N
    M --> N

    N --> N1["只追加客户事件"]
    N1 --> N2["客户建档"]
    N1 --> N3["企微关系同步"]
    N1 --> N4["线索关联与身份变更"]
    N1 --> N5["挖需动作"]
    N1 --> N6["问卷提交与等级评定"]
    N1 --> N7["标签变化"]
    N1 --> N8["订单成交"]
    N1 --> N9["负责人及组织变化"]
    N1 --> N10["生命周期变化"]

    subgraph LIFECYCLE["客户生命周期：本期仅保留三个阶段"]
        H --> P1["线索客户"]
        P1 -->|"发生挖需动作"| P2["意向客户"]
        P1 -->|"直接产生成交订单"| P3["成交客户"]
        P2 -->|"产生成交订单"| P3

        P1 --> P4["进入条件<br/>引流线索进入系统"]
        P2 --> P5["进入条件<br/>demand_mined = true"]
        P3 --> P6["进入条件<br/>存在有效成交订单"]

        P4 --> N
        P5 --> N
        P6 --> N
    end

    subgraph OWNER["客户归属管理"]
        H --> O["当前归属"]
        O --> O1["当前负责人"]
        O --> O2["当前归属组织"]
        O --> O3["负责人变更保留历史快照"]
        O3 --> N
    end

    subgraph TAGS["标签管理"]
        N7 --> T1{"标签来源"}

        T1 -->|"业务事实事件产生"| T2["系统标签"]
        T1 -->|"企业微信同步"| T3["企业微信标签"]

        T2 --> T4["标签库"]
        T3 --> T4

        T4 --> T41["标签名称与标签ID"]
        T4 --> T42["对象、分类、来源和状态"]
        T4 --> T43["系统与企微标签均只读"]

        T41 --> T5["标签组"]
        T42 --> T5
        T43 --> T5

        T5 --> T51["按客户画像或业务主题分组"]
        T5 --> T52["配置包含标签及展示顺序"]

        T51 --> T6["客户列表标签展示"]
        T52 --> T6

        T6 --> T61["默认展示前2个标签"]
        T6 --> T62["超过2个显示 +N"]
        T6 --> T63["点击查看全部标签"]
        T6 --> T64["支持按标签筛选客户"]
    end

    H --> Q["客户中心统一数据权限"]
    Q --> Q1["当前可见范围"]
    Q --> Q2["归属组织"]
    Q --> Q3["当前客户负责人"]
    Q --> Q4["列表、档案、统计与下钻<br/>继承相同权限条件"]
```

## 1. 文档目标

客户中心以家长客户为唯一经营主体，统一管理客户身份、360°档案、当前归属、历史服务人、标签和SABC等级。客户中心不设置“跟进记录”“等级管理”“公海客户”或“商机管理”二级菜单：历史服务与互动信息在客户档案内聚合展示；客户等级在客户列表操作中编辑。撞单管理纳入 V1.0，用于处理手机号与 UnionID 分别命中不同客户的强身份冲突；公海和商机能力已取消，不纳入任何当前版本规划。

V1.0必须实现“一个家长一个客户主档、所有线索和业务关联同一客户编号、所有归属和阶段变化可追溯、冲突可裁决、误操作可恢复”。

## 2. 核心口径

- 家长是客户、系统用户、课程学员和服务权益归属人；
- 孩子不是客户、用户或独立学员，仅作为家长名下附属对象；
- 手机号或UnionID任一有效即可创建或关联唯一客户；每个有效客户主档最多保存一个主手机号；
- 两种身份均缺失时只保留线索，不得转为正式客户；
- 订单、权益、交付、归属和生命周期归属于家长客户；
- 问卷、测评、诊断和课程匹配可选关联孩子作为业务背景；
- 任一时点每个客户只有一个有效主负责人，但可同时有一转、二转、班主任、规划师等不同服务角色。
- “首次来源”回答客户从哪个平台、渠道、店铺、商品或IP而来；“添加方式”只回答客户通过什么动作进入客户池，两者独立保存、独立筛选，禁止复用同一字段。

### 2.1 客户添加方式统一枚举

字典编码为 `customer_add_method`，V1.0 仅允许以下三个稳定值：

| 字典项值 | 页面文案 | 判定口径 |
|---|---|---|
| `LINK` | 通过链接添加 | 客户通过带参链接、落地页或可识别分享链接完成添加 |
| `BUSINESS_CARD` | 名片 | 客户通过员工名片或客户名片完成添加 |
| `QR_CODE` | 扫描二维码 | 客户扫描员工活码或其他可识别二维码完成添加 |

添加方式由客户首次成功进入客户池的事件写入。系统回调能够识别时自动填写；人工建档时必选。该字段是稳定事实，后续再次扫码、点击链接或转介绍不自动覆盖；确需纠错必须具备编辑权限、填写原因并记录变更前后值。历史数据无法还原时显示“—”，不得用首次来源反推或批量伪造。

## 3. 范围与菜单

```mermaid
flowchart TD
    CUSTOMER["客户中心"] --> OVERVIEW["客户总览"]
    CUSTOMER --> LIST["客户列表"]
    CUSTOMER --> COLLISION["撞单管理"]
    CUSTOMER --> TAG["标签管理"]
```

| 二级菜单 | 主要职责 | 优先级 |
|---|---|---|
| 客户总览 | 客户规模、生命周期、首次来源、添加方式、等级、归属和转化概览 | P1 |
| 客户列表 | 唯一客户、身份、添加方式、首次来源、360°档案、负责人、客户画像及等级编辑 | P0 |
| 撞单管理 | 手机号与 UnionID 跨档案命中时自动建案、证据核验、身份划归和审计 | P0 |
| 标签管理 | 人工、规则、来源和等级标签 | P1 |

## 4. 客户身份与建档

### 4.1 创建与关联决策

```mermaid
flowchart TD
    A["引流线索进入系统"] --> B["自动保存线索并建立客户经营档案<br/>生命周期：线索"]
    B --> C{"是否取得手机号或UnionID？"}
    C -- "否" --> X["保留客户经营档案<br/>身份待补充"]
    C -- "是" --> D["同时查询手机号和UnionID绑定"]
    D --> E{"命中结果"}
    E -- "均未命中" --> F["生成唯一客户编号并绑定有效身份"]
    E -- "命中同一客户/仅一项命中" --> G["关联已有客户并补充非冲突身份"]
    E -- "分别命中不同客户" --> H["客户经营档案继续建立<br/>冻结冲突身份覆盖与自动合并<br/>同步创建撞单案件"]
    F --> L1{"是否发生挖需动作？"}
    G --> L1
    H --> I["人工核验两个候选客户"]
    I --> J{"是否同一自然人？"}
    J -- "是" --> K["选择保留主档并合并"]
    J -- "否" --> L["保留两个独立客户"]
    K --> L1
    L --> L1
    L1 -- "是" --> M["生命周期：意向"]
    L1 -- "否" --> N["保持生命周期：线索"]
    M --> O{"是否存在支付成功且有效的成交订单？"}
    N --> O
    O -- "是" --> P["生命周期：成交"]
    O -- "否" --> Q["保持当前阶段"]
```

### 4.2 身份规则

1. 手机号先做国家区号、空格、分隔符等标准化，UnionID按来源CorpID和应用上下文校验；
2. 原值加密保存，标准化哈希用于查重，不以明文建立普通索引；
3. 客户手机号采用单值主手机号模型：一个有效客户主档最多保存一个主手机号，不提供备用手机号、多个手机号或手机号数组；
4. 主手机号在所有未合并、未删除的有效客户中全局唯一，同一手机号不得同时绑定两个有效客户；
5. V1.0 中国大陆手机号按标准化后的11位号码校验；未填写手机号时必须存在有效UnionID，否则不得建档；
6. 新建或转客时手机号已命中客户，系统关联原客户，不重复创建；仅当手机号和UnionID分别命中不同客户时进入撞单管理；
7. 主手机号补录或更换必须具备客户身份维护权限，提交原因并重新执行格式、唯一性和撞单校验。旧手机号只进入身份变更历史与审计，不继续作为当前有效手机号参与查重；
8. 客户合并时目标主档只能保留一个经裁决确认的主手机号，从档手机号解除有效绑定并保留历史快照，不得将两个手机号并列写入目标客户；
9. 同一类型有效身份只能属于一个有效客户；
10. 客户编码由服务端生成、永久不变且不得复用；
11. 企微ExternalUserID、第三方平台ID和旧系统编号作为辅助身份或来源标识，不能单独绕过手机号/UnionID转客户条件；
12. 合并后原客户编码保留为历史别名，拆分依据原始身份、线索和审计关系恢复；
13. 身份新增、解绑、设为主身份、合并和拆分均为高风险操作并保存原因。

## 5. 页面详细需求

### 5.1 客户总览

展示客户总量、生命周期分布、SABC等级、首次来源、添加方式、归属组织、负责人、流失候选、成交和退款回退。顶部使用统一数据权限组件，并提供“全部授权数据/仅看我的数据、归属组织、当前客户负责人”组合筛选。业务筛选包含客户创建时间、首次来源、客户等级和添加方式；客户创建时间默认最近一个月。全部指标、分布和下钻明细必须继承权限、客户创建时间及业务筛选快照。

### 5.2 客户列表与360°档案

列表字段至少包含客户编号、客户名称、生命周期、SABC等级、手机号、客户标签、企微状态、添加方式、首次来源、负责人、归属组织、客户添加时间和操作。右上角只展示齿轮设置图标，悬浮提示“字段设置”，不展示按钮文字；点击后打开可记忆的字段设置。客户编号、客户名称和操作为固定列且不可取消，其他字段允许自由选择。客户添加时间取客户主档 `created_at`，仅调整页面名称，不改变原有存储与历史数据。

客户列表提供生命周期、客户等级、添加方式和客户标签筛选。生命周期固定为线索客户、意向客户、成交客户；流失、无效、待分配、停用不作为客户生命周期。手机号默认脱敏，旁边提供眼睛图标授权预览并记录审计。

客户标签列最多直接展示2个标签，超过2个显示“+N”；点击打开全部标签弹窗，按标签来源分组展示，并保留添加BOSS人工标签操作。外部同步或系统规则标签不得被人工直接篡改。

客户列表提供添加方式筛选，支持 `LINK`、`BUSINESS_CARD`、`QR_CODE` 精确匹配。客户详情“基本档案”和360°来源信息同时展示添加方式与首次来源，避免把“扫描二维码”误当作渠道、把“抖店/有赞”误当作添加动作。客户新建时手机号或UnionID至少填写一个，并必须选择添加方式；由线索转客户时继承线索或企微事件已识别的添加方式。

客户列表、新建弹窗、详情页、筛选项和导出表头统一使用“主手机号”单数口径。新建弹窗只提供一个主手机号输入框，输入时去除空格和分隔符并校验11位大陆手机号；页面明确提示“一个客户只能保存一个主手机号”。手机号已被客户占用时不得创建第二个客户，返回已关联原客户的结果；手机号与UnionID跨档案冲突时提示案件编号并跳转撞单管理。列表暂不提供无审计的直接改号入口；后续开放补录或换号时必须执行身份维护权限、原因、二次校验和完整日志要求。

**来源于引流线索的手机号变更与档案合并**

- 引流线索执行手机号变更时，客户中心主手机号唯一约束必须实时参与预校验；新手机号未占用时，同时更新线索及其已关联客户主手机号，旧手机号只进入身份变更历史。
- 新手机号已归属于其他有效客户时不得直接覆盖。若两个手机号对应档案属于同一一转负责人，可在具备合并权限并人工确认后执行合并；若负责人不同则阻断，提示当前与目标客户归属，后续通过归属协调或撞单流程处理。
- 同负责人以 `owner_id` 为首选、员工编号为兼容依据，不使用负责人姓名判断；双方都未分配不得视为同负责人。
- 合并时以新手机号命中的有效客户为保留主档，来源客户标记 `MERGED` 并保存指向；线索、问卷、订单、交付/服务、标签和旅程等关系迁移或改指，不物理删除来源档案。
- 合并后目标客户仍只有一个主手机号；直接变更或合并均需提交阶段二次校验、幂等控制、完整审计和受权反向恢复。

客户总览与客户列表统一提供“权限范围、全部授权数据/我的数据、客户归属组织、客户负责人”筛选。客户归属组织采用公司—部门—小组级联树，选择部门时包含直属员工和下属小组；客户负责人支持姓名和员工编号搜索，并随组织选择联动缩小候选范围。员工在职、停用及离职状态统一由员工管理处理，不作为客户筛选项。本人数据岗、小组管理岗、部门管理岗、公司数据管理岗和获批特别授权人员只能在有效数据岗位范围内筛选，页面不得返回超出授权范围的组织、员工、数量或汇总指标；业务角色只决定是否有客户页面及相关操作权限。

客户列表按当前客户负责人及当前责任组织查询，适合日常跟进和移交；客户总览中的新增、转化、成交等历史指标使用业务发生时负责人和组织快照，员工调岗或客户改派不得重写历史业绩。无负责人的客户仅在用户具有待分配、部门或公司级数据权限时展示，并可单独筛选。

当前客户档案页签为基本档案、线索记录、学习交付、服务人员、问卷测评和客户时间轴。基本档案展示昵称、手机号及授权预览、客户地址、性别和客户编号，不展示企微客户ID与UnionID；线索记录、问卷测评和客户时间轴分别展示对应模拟与业务记录，时间轴只追加不覆盖。

客户主表只保存当前核心状态和汇总，不把线索、订单、问卷答案、课程和服务人员历史堆叠为客户字段。

### 5.3 撞单管理（V1.0）

#### 5.3.1 进入条件与事务边界

同时满足以下条件时自动进入撞单管理：本次业务数据携带有效手机号与有效 UnionID；手机号命中客户 A；UnionID 命中客户 B；且 A、B 为两个不同的有效客户主档。触发来源包括线索转客户、人工建档、企微同步和数据导入。

同一手机号和 UnionID 命中同一客户、只命中一个客户或只有一个有效强身份时不进入撞单。撞单产生时不得阻断客户经营档案、线索归档及后续业务记录；必须在同一事务内冻结冲突身份覆盖与自动合并、创建撞单案件、关联本次来源并写入审计。未裁决前不得形成新的冲突有效绑定。

开放案件幂等键建议为 `hash(标准化手机号 + UnionID + 手机号命中客户ID + UnionID命中客户ID)`；同一冲突的重复回调、拉取或导入只能补充证据，不得重复建案。

#### 5.3.2 案件列表

列表展示案件编号、触发时间、本次进入身份、手机号命中客户、UnionID命中客户、触发来源、来源业务单号、处理状态和处理人。支持按案件/客户/身份关键词、触发来源、状态和触发日期筛选。页面不展示风险等级，不提供风险筛选，也不把风险状态作为案件查询或处理条件。

状态固定为：`PENDING`（待处理）→ `PROCESSING`（处理中）→ `RESOLVED`（已解决）。页面顶部只展示待处理、处理中和已解决数量。领取案件后锁定主处理人；管理员可转派，但必须留痕。

#### 5.3.3 证据核验

详情将两个候选客户并排展示，至少包含客户编号、姓名、手机号、UnionID、SABC等级、生命周期、当前归属组织/负责人、首次来源、创建时间、订单数量、问卷数量和历史服务摘要。差异字段突出显示，并展示来源线索/订单/企微事件及案件时间轴。手机号和 UnionID 明文仅对具备敏感身份查看权限的裁决人展示，每次查看写入访问日志。

#### 5.3.4 处理结论

| 结论 | 适用条件 | 系统动作 |
|---|---|---|
| 合并至手机号命中客户 | 核验为同一自然人，手机号主档更可信 | 手机号命中客户作为主档，迁移 UnionID及从档业务关系，从档标记 `MERGED` |
| 合并至UnionID命中客户 | 核验为同一自然人，企微/UnionID主档更可信 | UnionID命中客户作为主档，迁移手机号及从档业务关系，从档标记 `MERGED` |
| 确认不同自然人 | 身份采集或历史绑定错误，实际为两个人 | 两个客户均保留，记录核验依据并修正错误来源关系，不合并主档 |

裁决必须填写不少于 8 个字的理由，并确认已核验身份、订单、问卷和服务记录。客户合并不物理删除从档；客户编号保留为历史别名，线索、订单、问卷、标签、服务人、等级和审计记录迁移或建立指向主档的关系。合并前快照、字段冲突策略、操作人、时间和 `trace_id` 必须保存，用反向业务操作恢复，禁止直接删除或修改审计历史。

#### 5.3.5 权限与时效

撞单查看受两个候选客户数据范围的并集约束；撞单裁决为独立按钮权限。本人作为直接利益方时不得单独完成跨组织裁决；跨部门案件由公司数据管理岗或双方上级处理。待处理超过 24 小时进入超时提醒，超过 48 小时升级给客户主数据治理负责人。V1.0 暂不做自动裁决，任何权重或 AI 建议只能作为证据提示。

### 5.5 标签管理

标签管理采用“统一展示、分源只读”模式，页面只包含标签库和标签组两个页签。标签来源固定为系统和企业微信，不扩展其他来源；两类标签均不在BOSS侧新增、编辑、启停或删除。

#### 5.5.1 页面入口与初始状态

- 页面入口：客户中心—标签管理；具有标签查看权限的用户可进入。
- 初始页签：标签库；默认加载当前权限范围内全部标签，按更新时间倒序排列。
- 默认分页：每页10条；空数据时展示空状态，不展示新增标签引导。

#### 5.5.2 标签库筛选与展示

| 筛选字段 | 组件 | 默认值 | 查询规则 |
|---|---|---|---|
| 关键词 | 输入框 | 空 | 模糊匹配标签名称、标签ID或来源 |
| 来源 | 单选下拉 | 全部 | 枚举仅为系统、企业微信 |
| 状态 | 单选下拉 | 全部 | 枚举仅为启用、停用 |

查询条件按AND组合；重置后恢复全部来源、全部状态和第一页。列表展示标签名称、标签ID、目前覆盖客户数、所属标签组、来源、状态和操作，不展示对象、分类、生成方式、有效期或标签日志。点击标签名称进入只读详情；点击覆盖客户数查看当前数据权限范围内的客户明细。操作列仅提供“查看”和只读提示。

| 来源 | 产生方式 | BOSS侧权限 | 生命周期 |
|---|---|---|---|
| 系统 | 营期归属、客户阶段、订单支付/退款等业务事实事件 | 只读查看 | 随业务事实生成或停用 |
| 企业微信 | 企微客户联系标签回调、定时补偿或手动同步 | 只读查看 | 状态跟随企微有效性映射为启用或停用 |

标签状态只表示当前是否可继续用于新的标签组选择和业务筛选：“启用”可用，“停用”仅保留历史展示。“已同步”属于同步结果，不是标签状态；同步成功后标签按来源有效性写入启用或停用。

#### 5.5.3 标签ID生成及唯一性规则

- 标签ID由系统生成，业务用户不可填写或修改；全局唯一，生成后永久不变，停用或来源删除后均不得回收复用。
- 系统标签格式为 `TAG-SYS-{业务类型}-{6位并发安全流水号}`，业务类型使用大写英文稳定编码，例如 `ORDER`、`PERIOD`、`REFUND`；同一业务事实重复触发时通过业务幂等键返回原标签ID，不重复生成。
- 企业微信标签格式为 `TAG-WX-{CorpID摘要}-{external_tag_id}`；系统同时独立保存完整 `CorpID` 和 `external_tag_id`，并以二者组合作为来源唯一键。同一企微标签重复回调、补偿或手动同步时更新原记录，不新建标签ID。
- 写入前同时校验标签ID全局唯一及来源唯一键唯一；并发冲突时仅一个请求成功创建，其余请求读取已创建记录。生成失败不允许降级为人工编码。

#### 5.5.4 同步企微

点击“同步企微”后创建补拉任务；按钮反馈同步成功、部分成功或失败及对应数量。同步幂等，不重复创建标签；单条失败不回滚其他成功标签，可重试失败项。“已同步”仅允许出现在结果提示中，不写入标签状态字段。

#### 5.5.5 标签组新增与编辑

标签组用于客户画像分区和页面筛选，可配置标签组名称、包含标签和状态。名称必填、最长30字且不区分大小写唯一；至少选择一个启用标签；状态枚举为启用、停用。标签组不配置标签对象、维护组织及排序，候选项仅来自当前启用标签库；已停用或已不存在标签不再作为新增候选，编辑已有标签组时保留只读历史引用并提示不可重新选择。保存成功后刷新列表并持久保留配置；保存失败时保留用户输入并展示原因。

#### 5.5.6 权限、异常与边界

- 页面权限控制标签库和标签组的可见性；标签组新增、编辑为独立按钮权限，前后端均须校验。
- 覆盖明细及导出沿用客户数据权限；无权客户不展示、不计入页面可见明细。
- 企业微信接口不可用时保留现有标签数据并提示同步失败，不改变原状态；来源标签被删除时将本地标签置为停用，不物理删除历史关系。
- 标签管理页面不提供标签日志、标签操作日志或同步历史的查询与展示入口。

### 5.6 客户等级（客户列表操作）

客户中心不设置独立“等级管理”菜单。客户列表展示当前等级及来源，并在单条客户操作中提供“编辑等级”；等级历史在客户档案内查看。

线索等级和客户等级共用字典 `lead_customer_grade`，可配置值为 S、A、B、C；“未定级”是系统保留状态，不作为可配置业务等级。字典只维护稳定枚举，问卷评分条件、分值区间、优先级和版本继续在线索配置—等级规则中维护。

新客户首次建档时复制来源线索的当前有效等级，并记录来源线索、评级来源、规则版本或人工记录及继承时间；命中存量客户时不得覆盖存量等级。客户建档后，线索等级与客户等级分别维护：客户侧人工改级必须具备权限、选择 S/A/B/C、填写原因并保存变更前后值、操作人和时间；不反向修改来源线索，后续线索改级也不得自动覆盖客户等级。详见《[合数BOSS V1.0——线索中心需求文档](./合数BOSS V1.0线索中心需求文档.md)》第 5.4.6 节。

## 6. 客户生命周期

V1.0生命周期仅为线索客户、意向客户和成交客户。引流线索进入即自动保存并建立客户经营档案，生命周期为线索；发生可追溯挖需动作后转为意向；存在支付成功且有效的成交订单后转为成交。其他状态不新增生命周期阶段。

客户生命周期、线索状态、SABC等级、订单状态和交付状态必须分开维护。

## 7. 核心数据模型

| 数据域 | 核心实体 |
|---|---|
| 客户主数据 | `CUSTOMER`（含 `add_method`）、`CUSTOMER_IDENTITY`、`CUSTOMER_ACCOUNT`、`CHILD_PROFILE` |
| 线索关系 | `LEAD_CUSTOMER_RELATION` |
| 生命周期与归属 | `CUSTOMER_LIFECYCLE_HISTORY`、`CUSTOMER_OWNER_RELATION` |
| 跟进标签等级 | `FOLLOW_UP_RECORD`、`TAG_DEFINITION`、`TAG_GROUP`、`TAG_GROUP_RELATION`、`EXTERNAL_TAG_MAPPING`、`CUSTOMER_TAG_RELATION`、`CUSTOMER_GRADE_HISTORY` |
| 撞单 | `CUSTOMER_IDENTITY_COLLISION_CASE`、`CUSTOMER_IDENTITY_COLLISION_EVIDENCE`、`CUSTOMER_IDENTITY_COLLISION_AUDIT` |
| 服务关系 | `CUSTOMER_SERVICE_RELATION` |
| 360°聚合 | `CUSTOMER_EVENT`及各业务模块关联索引 |

任一时点每个客户只允许一个有效主负责人；服务角色可以并存，但每段关系必须具有开始、结束和业务来源。所有阶段、等级、归属、合并和拆分使用追加式历史。

## 8. 接口与事件

主要接口：客户创建/关联（接收并校验 `addMethod`）、身份查询、客户列表/详情（支持按添加方式筛选）、撞单列表/详情、领取撞单、提交裁决、负责人改派、跟进登记、只读标签库查询、企业微信标签同步、标签组维护、标签变更和等级试算/发布。V1.0 不建设BOSS标签新增/编辑/启停、标签规则、外部标签映射或标签治理接口。

主要事件：`customer.created`、`customer.identity_linked`、`customer.owner_changed`、`customer.lifecycle_changed`、`customer.grade_changed`、`customer.tag_attached`、`customer.tag_detached`、`external_tag.synced`、`tag_rule.matched`、`tag_governance.issue_created`、`customer.duplicate_detected`。

## 9. 权限与脱敏

- 菜单权限控制客户中心当前二级页面；
- 按钮权限控制身份明文、身份解绑、合并、拆分、撞单裁决、人工改级和跨组织改派；
- 标签权限拆分为标签查看、BOSS人工打标、标签定义维护、规则发布、外部映射和治理处理；系统及外部来源标签不可通过按钮权限获得编辑能力；
- 数据范围支持本人、协作、组织及下级、自定义和全部；
- 手机号、UnionID、问卷答案、诊断报告和订单金额分别配置字段权限；
- 明文查看、导出、高风险变更和批量操作记录审计及trace_id。

## 10. 验收标准

- [ ] 手机号或UnionID任一有效可创建或关联唯一客户；两者分别命中不同客户时进入撞单；
- [ ] 每个有效客户最多保存一个主手机号；新建、详情、筛选、导出和接口均不出现备用手机号、多手机号数组或第二个有效手机号；
- [ ] 主手机号在有效客户中全局唯一；重复手机号只能关联已有客户，不能生成重复客户；手机号与UnionID跨档案命中才进入撞单；
- [ ] 主手机号补录、更换和客户合并均重新校验唯一性并记录原因、变更前后值、操作人和时间；历史手机号不再作为当前有效身份参与查重；
- [ ] 家长是客户、用户和学员，孩子不会被错误创建为独立客户或学员；
- [ ] 客户360°档案可以查看线索、订单、交付、服务人、售后、问卷、测评和报告历史；
- [ ] 任一时点只有一个有效主负责人，历史负责人和多角色服务关系完整；
- [ ] 两个强身份分别命中不同客户时，自动建档和身份覆盖被原子阻断，并且只生成一个开放撞单案件；
- [ ] 撞单支持领取、双档案证据对比、合并至任一主档或确认不同人，结论和理由完整留痕；
- [ ] 合并不物理删除从档，历史客户编号和业务关系可追溯，误操作可通过反向业务操作恢复；
- [ ] 客户中心不展示“跟进记录”和“等级管理”二级菜单；
- [ ] 新客户建档继承线索当前等级，客户可在列表操作中独立编辑 S/A/B/C 并保留历史；
- [ ] 线索与客户共用等级字典，任一侧人工编辑不会互相覆盖；
- [ ] 客户列表、详情、新建/编辑、筛选及导出均展示统一的添加方式；枚举仅为“通过链接添加、名片、扫描二维码”；
- [ ] 添加方式与首次来源分别存储、分别筛选和分别统计，二者不会相互覆盖；历史无法还原的数据展示“—”；
- [ ] 客户总览展示添加方式分布，点击任一数量可携带筛选条件下钻客户列表；
- [ ] 标签管理只展示标签库和标签组，来源筛选只包含系统、企业微信；
- [ ] 系统与企业微信标签均只能查看，不出现新增、编辑、启停和删除入口；同步企微按外部标签ID幂等执行；
- [ ] 标签组支持新增、编辑和启停，候选项来自全部当前有效标签，不受隐藏对象字段过滤，刷新后配置及关联不丢失；
- [ ] 标签组不展示已从标签库移除的幽灵标签；标签来源ID和现存关联稳定保留；
- [ ] 标签状态筛选及列表枚举仅为启用、停用，不出现“已同步”；
- [ ] 标签ID按来源规则生成，全局唯一且不可修改、回收或复用；企微重复同步不重复创建；
- [ ] 标签管理页面不展示标签日志、标签操作日志或同步历史入口；
- [ ] 客户等级变更保留历史；

## 11. 发布与回滚

- 身份规则、等级规则和生命周期规则采用版本化发布；
- 合并、拆分、归属裁决和人工改级均使用反向业务操作回滚；
- 新规则先试算或灰度，异常时恢复上一已发布版本；
- 回滚不得删除客户编码、身份别名、线索、跟进、订单、服务关系和审计历史；
- 360°聚合索引可以重建，但原始业务数据不得依赖聚合索引作为唯一存储。

## 12. 待确认事项

- 意向客户必选识别条件；
- 线索客户和意向客户流失时限；
- 撞单各证据权重和复核角色；
- 一转、二转、班主任和规划师同时存在时主负责人定义；

## 13. 版本记录

| 版本 | 日期 | 内容 |
|---|---|---|
| V1.18 | 2026-09-09 | 按通用PRD需求说明结构重写标签管理逻辑；补充标签ID生成、唯一性与幂等规则；标签状态移除“已同步”；移除标签日志及同步历史展示要求 |
| V1.17 | 2026-09-03 | 标签来源最终收敛为系统、企业微信且均只读，移除新增标签；修正标签组幽灵引用、隐藏对象过滤和刷新后丢失问题 |
| V1.15 | 2026-08-31 | 客户列表“客户创建时间”更名为“客户添加时间”，继续使用主档 `created_at`，不迁移历史数据；客户总览筛选口径保持不变 |
| V1.14 | 2026-08-28 | 客户生命周期收口为线索/意向/成交并明确转换条件；撞单不阻断客户经营档案，仅冻结冲突身份覆盖/自动合并并同步建案；客户列表统一客户创建时间；基本档案补充客户地址并移除企微客户ID/UnionID展示；补充最终流程图 |
| V1.12 | 2026-08-28 | 客户列表字段设置仅展示齿轮图标；撞单管理移除风险统计、风险筛选和风险等级；标签库主列表移除生成方式、覆盖对象和有效期三列，详情、规则和历史数据保持不变 |
| V1.11 | 2026-08-27 | 客户总览统一数据权限并按客户创建时间筛选、默认最近一个月；客户列表新增字段设置、生命周期、客户标签、手机号授权预览及变更合并规则 |
| V1.10 | 2026-08-26 | 取消公海和商机能力，删除对应菜单、页面、数据对象、接口、事件、权限、规则和验收项；超时与离职客户统一通过待办提醒及人工改派处理 |
| V1.0 | 2026-08-17 | 从主需求文档独立形成客户中心专项需求，并引用客户360°档案基线 |
| V1.1 | 2026-08-19 | 客户总览与客户列表增加权限范围、公司/部门/小组级联和客户负责人筛选；明确当前归属与历史组织快照口径 |
| V1.2 | 2026-08-24 | 移除员工状态筛选，员工停用、离职及客户移交统一由员工管理和离职继承处理 |
| V1.2 | 2026-08-19 | 明确等级规则统一在线索配置维护；新增客户首次建档继承线索等级、存量客户不覆盖、建档后客户主档权威及统一人工调整流水 |
| V1.3 | 2026-08-24 | 移除离职继承菜单、页面、接口、数据对象和 V1.0 验收项；员工停用仍由系统管理阻断登录和新增接待 |
| V1.4 | 2026-08-25 | 移除跟进记录和等级管理菜单；客户等级并入客户列表操作；统一线索/客户 SABC 字典并明确仅建档时继承、后续独立调整 |
| V1.5 | 2026-08-25 | 标签管理升级为统一标签中心：新增标签库、标签组、自动规则、外部标签映射和标签治理；明确六类来源、分源编辑权限、历史保留及验收口径 |
| V1.6 | 2026-08-25 | 客户总览、客户列表和360°档案新增“添加方式”；统一 `customer_add_method` 字典及 LINK/BUSINESS_CARD/QR_CODE 三项枚举，明确与首次来源分离、自动识别、人工建档和历史兼容口径 |
| V1.7 | 2026-08-25 | 撞单管理纳入 V1.0；明确手机号与 UnionID 跨档案命中的自动建案、幂等阻断、双档案证据核验、三类人工裁决、合并历史保留、权限、时效和验收口径 |
| V1.8 | 2026-08-25 | 客户主手机号调整为严格单值模型：一个有效客户仅允许一个主手机号，号码全局唯一；补充重复关联、换号、合并、审计及页面验收口径 |
| V1.9 | 2026-08-26 | 补充引流线索变更手机号对客户主档的联动：未占用直接变更、已占用禁止覆盖、同一一转负责人允许人工确认合并、跨负责人阻断，并明确主档保留、关系迁移、审计与恢复规则 |
