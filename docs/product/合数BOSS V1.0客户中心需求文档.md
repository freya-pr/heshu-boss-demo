...
> **V1.14 当前生效补充：**客户生命周期仅保留“线索、意向、成交”三个阶段：引流线索进入即建立客户经营档案并标记为线索客户；发生可追溯挖需动作后转为意向客户；存在支付成功且有效的成交订单后转为成交客户。手机号与 UnionID 跨档案冲突不阻断客户经营档案建立和后续业务记录，只冻结冲突身份的自动覆盖/合并并同步创建撞单案件。客户列表“建档时间”统一为“客户创建时间”；基本档案展示昵称、手机号预览、客户地址、性别和客户编号，不展示企微客户ID与UnionID。

> **V1.15 当前生效补充：**客户列表中的“客户创建时间”统一更名为“客户添加时间”，继续使用客户主档的 `created_at` 字段，不迁移、不重算历史数据。该调整仅作用于客户列表展示及关联客户选择场景；客户总览的统计筛选仍沿用“客户创建时间”口径。

> **V1.17 当前生效补充：**标签管理只保留标签库和标签组，来源固定为系统、企业微信，两类标签在BOSS侧均为只读；系统标签由业务事实产生，企业微信标签通过同步进入，不提供新增、编辑、启停或删除标签入口。标签组支持新增、编辑、启停，可包含当前标签库任意有效标签，不配置标签对象、维护组织或排序；保存后的标签组及标签关联必须稳定保留。后文与本补充冲突的旧表述均不再生效。

> **V1.18 当前生效补充：**标签状态枚举仅保留“启用、停用”，“已同步”不再作为状态；同步成功只作为同步操作结果提示。标签ID由系统生成且全局唯一，生成后不可修改、回收或复用。标签管理页面及需求范围不提供标签日志或标签操作日志的展示入口。后文与本补充冲突的旧表述均不再生效。

> **V1.19 当前生效补充：**客户中心“标签管理”更名为“企微标签”，仅管理当前企业主体的企微企业标签和当前用户可见的企微个人标签，彻底移除系统标签来源。页面采用左侧标签分组、右侧标签列表，支持同步、新增分组、批量新增标签、编辑和删除；企微返回的 `external_tag_id` 为标签ID事实来源。后文与本补充冲突的旧表述均不再生效。

> **V1.20 当前生效补充：**企微标签页面移除旧顶部组合筛选，以左侧分组名称搜索和右侧全局标签搜索替代；全局搜索覆盖当前类型全部分组。新增分组必须同时创建至少一个标签，分组名称与标签名称最长15字；分组行悬浮显示编辑、删除，非空分组禁止删除。详细规则以企微标签专项需求 V1.2 为准。

> **V1.21 当前生效补充：**企微标签分组行改为点击“…”展示编辑、删除；“新增分组”右侧“…”进入空分组批量删除模式。标签“使用客户”数量可跳转客户列表并自动带入标签筛选。客户列表移除标题下方“唯一身份、客户来源、添加方式”说明卡片，业务字段和校验规则不变。详细标签规则以企微标签专项需求 V1.3 为准。

> **V1.22 当前生效补充：**企微标签仅保留企微企业标签，移除企微个人标签入口、数据和切换逻辑。新增与编辑分组的适用部门均支持组织树多选，“全部部门”与具体部门互斥且至少选择一项；点击分组列表中的适用部门可直接进入编辑。详细规则以企微标签专项需求 V1.4 为准。

## 客户中心整体流程图

```mermaid
flowchart TD
    A["引流线索进入系统"] --> A1["自动保存线索记录"]
    A1 --> A2["客户生命周期：线索客户"]
    A2 --> B["标准化身份信息<br/>手机号、UnionID、external_userid"]
    B --> C{"查询现有客户身份"}

    C -->|"手机号与 UnionID<br/>均未命中"| D["创建正式客户档案<br/>生成唯一客户编号"]

    C -->|"仅命中一个客户<br/>或共同命中同一客户"| E["关联已有客户档案<br/>补充身份及线索关系"]

    C -->|"手机号与 UnionID<br/>分别命中不同客户"| F["创建待核验客户档案<br/>状态：身份冲突"]

    subgraph COLLISION["撞单管理：不阻断建档，只阻断冲突身份自动绑定与合并"]
        F --> F1["保存本次业务数据<br/>线索、问卷、订单、来源"]
        F --> F2["自动创建撞单案件<br/>状态：待处理"]
        ...
        F31 --> F4["领取案件<br/>状态：处理中"]
        F32 --> F4
        F33 --> F4

        F4 --> F5["核验身份、来源、订单、问卷<br/>企微关系、负责人和时间轴"]
        F5 --> F6{"人工裁决结果"}

        F6 -->|"属于手机号客户"| F7["手机号客户作为主档"]
        F6 -->|"属于 UnionID 客户"| F8["UnionID 客户作为主档"]
        F6 -->|"属于新的独立自然人"| F9["待核验档案转为正式客户"]
        F6 -->|"两个历史档案实际为同一人"| F10["选择可信主档并执行客户合并"]

        F7 --> F11["迁移本次线索、问卷、订单<br/>标签、服务关系与时间轴"]
        F8 --> F11
        F10 --> F12["迁移从档全部业务关系"]
        F12 --> F13["从档标记已合并<br/>保留历史客户编号与审计"]

        F11 --> F14["关闭待核验客户档案"]
        F13 --> F14
        F9 --> F15["保留独立客户档案<br/>解除身份冲突状态"]
    ...
    subgraph WECOM["企业微信客户同步"]
        W1["员工添加企微客户"] --> W2["接收客户联系事件回调<br/>external_userid + userid"]
        W2 --> W3["校验、解密并快速返回成功"]
        W3 --> W4["异步获取客户详情"]
        W4 --> W5{"本地是否已有对应客户"}
        ...
        W6 --> W8["保存员工跟进关系"]
        W7 --> W8
        W8 --> W9["同一客户允许关联多个员工<br/>external_userid + userid 唯一"]
        W9 --> W10["同步添加时间、add_way、state<br/>备注、描述及企微标签"]

        W11["首次接入或全量对账"] --> W12["全量同步<br/>员工列表 → 客户列表 → 客户详情"]
        W13["日常补偿"] --> W14["增量同步<br/>同步上次完成后的变化"]

        W12 --> W4
        W14 --> W4
        W10 --> W15["记录检查、新增、更新和失败数量"]
        W15 --> W16["失败延迟重试<br/>定时全量对账补偿"]
    end
    ...
    H --> I["基本档案"]
    I --> I1["昵称、手机号、地址、性别"]
    I --> I2["客户编号、企微客户ID、UnionID"]
    I --> I3["手机号默认脱敏<br/>授权后预览"]
    ...
    subgraph LIFECYCLE["客户生命周期：本期仅保留三个阶段"]
        H --> P1["线索客户"]
        P1 -->|"发生挖需动作"| P2["意向客户"]
        P1 -->|"直接产生成交订单"| P3["成交客户"]
        P2 -->|"产生成交订单"| P3

        P1 --> P4["进入条件<br/>引流线索进入系统"]
        P2 --> P5["进入条件<br/>demand_mined = true"]
        P3 --> P6["进入条件<br/>存在有效成交订单"]
...
flowchart TD
    ...
    B --> C{"是否取得手机号或UnionID？"}
    C -- "否" --> X["保留客户经营档案<br/>身份待补充"]
    C -- "是" --> D["同时查询手机号和UnionID绑定"]
    D --> E{"命中结果"}
    E -- "均未命中" --> F["生成唯一客户编号并绑定有效身份"]
    E -- "命中同一客户/仅一项命中" --> G["关联已有客户并补充非冲突身份"]
    E -- "分别命中不同客户" --> H["客户经营档案继续建立<br/>冻结冲突身份覆盖与自动合并<br/>同步创建撞单案件"]
    F --> L1{"是否发生挖需动作？"}
    G --> L1
    H --> I["人工核验两个候选客户"]
    I --> J{"是否同一自然人？"}
...
展示客户总量、生命周期分布、SABC等级、首次来源、添加方式、归属组织、负责人、流失候选、成交和退款回退。顶部使用统一数据权限组件，并提供“全部授权数据/仅看我的数据、归属组织、当前客户负责人”组合筛选。业务筛选包含客户创建时间、首次来源、客户等级和添加方式；客户创建时间默认最近一个月。全部指标、分布和下钻明细必须继承权限、客户创建时间及业务筛选快照。

### 5.2 客户列表与360°档案

客户列表标题下方不展示“唯一身份、客户来源、添加方式”说明卡片，标题后直接进入筛选与列表区域；该展示调整不改变手机号、UnionID、首次来源和添加方式的业务定义及校验规则。

列表字段至少包含客户编号、客户名称、生命周期、SABC等级、手机号、客户标签、企微状态、添加方式、首次来源、负责人、归属组织、客户添加时间和操作。右上角只展示齿轮设置图标，悬浮提示“字段设置”，不展示按钮文字；点击后打开可记忆的字段设置。客户编号、客户名称和操作为固定列且不可取消，其他字段允许自由选择。客户添加时间取客户主档 `created_at`，仅调整页面名称，不改变原有存储与历史数据。

客户列表提供生命周期、客户等级、添加方式和客户标签筛选。生命周期固定为线索客户、意向客户、成交客户；流失、无效、待分配、停用不作为客户生命周期。手机号默认脱敏，旁边提供眼睛图标授权预览并记录审计。

客户标签列最多直接展示2个标签，超过2个显示“+N”；点击打开全部标签弹窗，按标签来源分组展示，并保留添加BOSS人工标签操作。外部同步或系统规则标签不得被人工直接篡改。

客户列表提供添加方式筛选，支持 `LINK`、`BUSINESS_CARD`、`QR_CODE` 精确匹配。客户详情“基本档案”和360°来源信息同时展示添加方式与首次来源，避免把“扫描二维码”误当作渠道、把“抖店/有赞”误当作添加动作。客户新建时手机号或UnionID至少填写一个，并必须选择添加方式；由线索转客户时继承线索或企微事件已识别的添加方式。

客户列表、新建弹窗、详情页、筛选项和导出表头统一使用“主手机号”单数口径。新建弹窗只提供一个主手机号输入框，输入时去除空格和分隔符并校验11位大陆手机号；页面明确提示“一个客户只能保存一个主手机号”。手机号已被客户占用时不得创建第二个客户，返回已关联原客户的结果；手机号与UnionID跨档案冲突时提示案件编号并跳转撞单管理。列表暂不提供无审计的直接改号入口；后续开放补录或换号时必须执行身份维护权限、原因、二次校验和完整日志要求。
...
当前客户档案页签为基本档案、线索记录、学习交付、服务人员、问卷测评和客户时间轴。基本档案展示昵称、手机号及授权预览、客户地址、性别和客户编号，不展示企微客户ID与UnionID；线索记录、问卷测评和客户时间轴分别展示对应模拟与业务记录，时间轴只追加不覆盖。
...
同一手机号和 UnionID 命中同一客户、只命中一个客户或只有一个有效强身份时不进入撞单。撞单产生时不得阻断客户经营档案、线索归档及后续业务记录；必须在同一事务内冻结冲突身份覆盖与自动合并、创建撞单案件、关联本次来源并写入审计。未裁决前不得形成新的冲突有效绑定。
...
企微标签仅维护企业微信企业标签，不再展示系统标签或企微个人标签。详细页面、校验、同步、权限和异常规则以《合数BOSS V1.0客户中心——企微标签需求文档》V1.4为准。

#### 5.5.1 页面入口与初始状态

- 页面入口：客户中心—企微标签，仅展示企微企业标签；默认选中第一个标签组。
- 页面采用左侧标签分组、右侧标签列表结构。

#### 5.5.2 查询与展示

- 左侧“新增分组”旁的搜索图标用于按标签分组名称实时筛选，关闭搜索后恢复全部分组。
- 右侧“搜索”进入全局搜索视图，在全部企微企业标签分组内，按标签名称、企微标签ID或标签分组名称实时模糊匹配；点击“取消”返回分组视图。
- 分组展示名称、适用部门和标签数量；当前分组列表展示标签名称、企微标签ID、使用客户、创建人、更新时间和操作。
- 全局结果额外展示所属标签分组和适用部门，操作必须定位结果所属分组；操作列固定右侧。“使用客户”数量可点击跳转客户列表，并自动带入标签名称和企微标签ID筛选。页面不展示标签来源列、旧顶部组合筛选或标签组类别筛选。

#### 5.5.3 标签ID生成及唯一性规则

- 标签ID以企业微信返回的 `external_tag_id` 为准，BOSS不得自行生成或修改。
- 本地以 `CorpID + external_tag_id` 作为来源唯一键；重复回调、补偿或手动同步更新原记录。

#### 5.5.4 同步企微

点击“同步企微”后创建补拉任务；按钮反馈同步成功、部分成功或失败及对应数量。同步幂等，不重复创建标签；单条失败不回滚其他成功标签，可重试失败项。“已同步”仅允许出现在结果提示中，不写入标签状态字段。

#### 5.5.5 标签组与标签维护

企业标签支持新增分组和在选中分组内批量新增标签。分组名称最长15字且同主体唯一；新增分组必须同时创建至少一个标签，支持逐个添加或按行批量新建。新增和编辑分组的适用部门均使用组织树多选，默认“全部部门”；“全部部门”与具体部门互斥，至少选择一项。分组列表多部门以顿号连接并可省略展示，点击适用部门直接打开编辑弹窗且不切换分组。分组行固定展示“…”入口，点击后显示编辑、删除；编辑回显名称和全部已选部门。非空分组禁止删除并提示先清空内容，空分组二次确认后可删除。“新增分组”右侧“…”进入批量删除模式，仅空分组可勾选，确认后批量删除。标签单次最多20个，名称以回车、换行、逗号或顿号分隔；单个名称最长15字，同批次及同组不可重复。编辑、删除同步至企业微信；部分失败不回滚成功项。

#### 5.5.6 权限、异常与边界

- 页面查看、分组新增、标签新增、编辑、删除和同步为独立权限点；标签分组按多个适用部门控制使用与编辑范围。
- 覆盖明细及导出沿用客户数据权限；无权客户不展示、不计入页面可见明细。
- 企业微信接口不可用时保留现有标签数据并提示同步失败，不改变原状态；来源标签被删除时将本地标签置为停用，不物理删除历史关系。
- 标签管理页面不提供标签日志、标签操作日志或同步历史的查询与展示入口。
...
V1.0生命周期仅为线索客户、意向客户和成交客户。引流线索进入即自动保存并建立客户经营档案，生命周期为线索；发生可追溯挖需动作后转为意向；存在支付成功且有效的成交订单后转为成交。其他状态不新增生命周期阶段。
...
| 跟进标签等级 | `FOLLOW_UP_RECORD`、`TAG_DEFINITION`、`TAG_GROUP`、`TAG_GROUP_RELATION`、`EXTERNAL_TAG_MAPPING`、`CUSTOMER_TAG_RELATION`、`CUSTOMER_GRADE_HISTORY` |
...
主要接口：客户创建/关联（接收并校验 `addMethod`）、身份查询、客户列表/详情（支持按添加方式筛选）、撞单列表/详情、领取撞单、提交裁决、负责人改派、跟进登记、只读标签库查询、企业微信标签同步、标签组维护、标签变更和等级试算/发布。V1.0 不建设BOSS标签新增/编辑/启停、标签规则、外部标签映射或标签治理接口。
...
- [ ] 菜单及页面名称为“企微标签”，不展示系统标签或标签来源筛选；
- [ ] 仅展示企微企业标签，不出现企微个人标签入口或数据；企业标签采用左分组、右列表结构；
- [ ] 新增和编辑分组均支持适用部门多选，全部部门互斥、必选校验及点击部门直接编辑正确；
- [ ] 支持新增分组、批量新增标签、编辑、删除和同步企微；
- [ ] 企微标签ID使用企业微信返回值，重复同步不重复创建；
- [ ] 页面不展示标签日志、标签操作日志或同步历史入口；
- [ ] 客户等级变更保留历史；
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V1.22 | 2026-09-14 | 企微标签移除企微个人标签模块；分组适用部门改为组织树多选，支持全部部门互斥、必选校验、多部门回显及点击列表部门直接编辑 |
| V1.21 | 2026-09-12 | 企微标签新增点击式分组操作、空分组批量删除和使用客户跳转；客户列表移除顶部业务说明卡片 |
| V1.20 | 2026-09-12 | 同步企微标签当前原型：新增分组搜索、跨分组全局标签搜索、分组悬浮编辑删除及非空分组删除限制；名称统一15字 |
| V1.19 | 2026-09-12 | 标签管理更名为企微标签；移除系统标签来源；按左侧分组、右侧标签列表重构；新增企微企业/个人标签切换及标签维护规则 |
| V1.18 | 2026-09-09 | 按通用PRD需求说明结构重写标签管理逻辑；补充标签ID生成、唯一性与幂等规则；标签状态移除“已同步”；移除标签日志及同步历史展示要求 |
| V1.17 | 2026-09-03 | 标签来源最终收敛为系统、企业微信且均只读，移除新增标签；修正标签组幽灵引用、隐藏对象过滤和刷新后丢失问题 |
| V1.15 | 2026-08-31 | 客户列表“客户创建时间”更名为“客户添加时间”，继续使用主档 `created_at`，不迁移历史数据；客户总览筛选口径保持不变 |
| V1.14 | 2026-08-28 | 客户生命周期收口为线索/意向/成交并明确转换条件；撞单不阻断客户经营档案，仅冻结冲突身份覆盖/自动合并并同步建案；客户列表统一客户创建时间；基本档案补充客户地址并移除企微客户ID/UnionID展示；补充最终流程图 |
| V1.12 | 2026-08-28 | 客户列表字段设置仅展示齿轮图标；撞单管理移除风险统计、风险筛选和风险等级；标签库主列表移除生成方式、覆盖对象和有效期三列，详情、规则和历史数据保持不变 |
| V1.11 | 2026-08-27 | 客户总览统一数据权限并按客户创建时间筛选、默认最近一个月；客户列表新增字段设置、生命周期、客户标签、手机号授权预览及变更合并规则 |
