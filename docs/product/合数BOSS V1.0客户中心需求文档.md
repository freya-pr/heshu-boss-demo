...
企微标签仅维护企业微信标签，不再展示系统标签。详细页面、校验、同步、权限和异常规则以《合数BOSS V1.0客户中心——企微标签需求文档》V1.3为准。

#### 5.5.1 页面入口与初始状态

- 页面入口：客户中心—企微标签；默认进入企微企业标签。
- 可切换企微企业标签、企微个人标签；默认选中第一个标签组。
- 页面采用左侧标签分组、右侧标签列表结构。

#### 5.5.2 查询与展示

- 左侧“新增分组”旁的搜索图标用于按标签分组名称实时筛选，关闭搜索后恢复全部分组。
- 右侧“搜索”进入全局搜索视图，在当前企微标签类型的全部分组内，按标签名称、企微标签ID或标签分组名称实时模糊匹配；点击“取消”返回分组视图。
...
企业标签支持新增分组和在选中分组内批量新增标签。分组名称最长15字且同主体同类型唯一；新增分组必须同时创建至少一个标签，支持逐个添加或按行批量新建。分组行固定展示“…”入口，点击后显示编辑、删除；编辑回显名称和适用部门。非空分组禁止删除并提示先清空内容，空分组二次确认后可删除。“新增分组”右侧“…”进入批量删除模式，仅空分组可勾选，确认后批量删除。标签单次最多20个，名称以回车、换行、逗号或顿号分隔；单个名称最长15字，同批次及同组不可重复。编辑、删除同步至企业微信；部分失败不回滚成功项。

#### 5.5.6 权限、异常与边界

- 页面查看、分组新增、标签新增、编辑、删除和同步为独立权限点；企微个人标签仅本人可见。
