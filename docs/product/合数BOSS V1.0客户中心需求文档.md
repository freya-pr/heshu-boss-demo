...
> **V1.16 当前生效补充：**标签管理只保留标签库和标签组，标签来源固定为系统、企业微信。标签库支持新增系统标签和同步企微标签，标签名称全局不允许重复；列表展示标签名称/ID、目前覆盖客户数、可选标签组、来源、状态和操作，不展示对象与分类。新增标签仅填写名称、可选标签组及说明，不配置有效期、标签对象、标签分类和使用权限。标签组支持新增、编辑，不配置标签对象、维护组织及排序。后文与本补充冲突的旧表述均不再生效。
...
标签库列表展示标签名称（同时展示标签ID）、对象、分类、来源、状态和操作；主列表不展示“生成方式、覆盖对象、有效期”三列。支持按名称/ID/来源系统搜索，并按对象、来源和状态筛选。生成依据、覆盖关系和生命周期仍作为标签详情、规则配置及治理数据保留，不删除底层字段或历史记录。
