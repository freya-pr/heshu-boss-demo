...
展示客户总量、生命周期分布、SABC等级、首次来源、添加方式、营期、归属组织、负责人、流失候选、成交和退款回退。支持按组织、负责人、等级、首次来源和添加方式筛选；添加方式按“通过链接添加、名片、扫描二维码”展示数量及占比。指标必须展示周期、口径、更新时间并支持按权限下钻客户列表，下钻时携带当前筛选条件。

### 5.2 客户列表与360°档案

列表字段至少包含客户编号、客户名称、生命周期、SABC等级、主手机号、企微状态、添加方式、首次来源、当前营期、负责人、关联孩子摘要、最近跟进、累计成交、创建时间和更新时间。添加方式使用 `customer_add_method` 字典解析，不允许页面自行维护另一套文案；列表、详情、新建/编辑、筛选、导出和接口返回必须保持一致。
