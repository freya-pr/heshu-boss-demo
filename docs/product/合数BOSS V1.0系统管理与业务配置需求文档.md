...
> **V2.7 当前生效补充：**业务配置—短信管理新增“短信规则”三级菜单，位于短信模板和发送结果之间。短信规则按规则名称、类型、关联抖店、签名和模板配置，类型固定为用户下单后、用户提醒、用户授权后；五项均必填。后文与本补充冲突的旧表述均不再生效。

> **V2.8 当前生效补充：**员工编号统一采用 `B` + 5 位全局流水号，例如 `B00001`；生成后不可修改或复用，并发环境使用数据库序列。后文与本补充冲突的 `B` + 6 位规则均不再生效。
...
> **V2.11 当前生效补充：**“发送结果”从业务配置—短信管理迁移至一级“审计管理”下，与“异常中心”并列；短信管理仅保留运营商管理、签名管理、短信模板和短信规则四个三级菜单。发送结果的业务字段、回执追踪及补发节点口径不变。

> **V2.12 当前生效补充：**业务配置下的“企微管理”仅展示企微配置、小程序配置、落地页配置和微信客服管理入口；“应用管理”不再作为独立三级菜单，改为在企微配置列表操作列点击“应用”进入，该详情页不新增工作台标签。
...
flowchart TB
    subgraph SYNC_AREA["第一部分｜合数BOSS与企业微信同步交互"]
        ...
        BOSS["合数BOSS<br/>组织员工正式主数据<br/>生成 B + 5位员工编号"]
...
| 邮箱 | 否 | 填写时校验格式与唯一性 |
| 员工编号 | 系统生成 | 新员工按 `B` + 5 位全局流水号生成，例如 `B00001`；生成后不可修改、不可复用；并发环境使用数据库序列，不使用最大值加一 |
...
- 补开弹窗展示员工编号、姓名、脱敏手机号、主组织、企微部门映射及拟生成UserID；
- 自有系统主控关闭时入口禁用，并提示“请先在企业微信创建成员，再同步至合数BOSS”。

前置校验：

1. 员工在职且登录账号启用；目标 `CorpID + UserID` 尚未绑定其他账号；
...
| 合数BOSS员工编码 `employee_no` | 页面展示、合数BOSS业务编号 | 一律由合数BOSS按 `B` + 5 位全局流水号生成并维护；旧系统编号只作历史别名，不继承为正式编号 |
...
| 场景 | 处理规则 |
|---|---|
| 历史员工首次导入 | 先进入暂存区，完成身份匹配和冲突处理后建立正式员工主档，由合数BOSS生成 `B` + 5 位员工编号 |
...
| 建立正式主档 | 经确认后创建或合并员工主档，由合数BOSS生成永久员工编号，建立账号、组织、岗位、角色和企微绑定 | 正式员工编号覆盖率100%，每个 `CorpID + UserID` 只绑定一个账号，账号可关联多个外部身份 |
...
- 左侧导航在“短信管理”下设置四个三级菜单：运营商管理、签名管理、短信模板、短信规则；每个三级菜单使用独立路由、查询条件和列表；“发送结果”作为审计查询入口放在一级“审计管理”下，不在短信管理内重复展示；
...
- **短信结果**：提交结果、供应商回执、送达状态、失败码和计费条数，按业务请求号追踪完整链路；下单短信额外保存订单编号、规则版本、首次/补发标识、补发节点、计划检查时间、实际检查时间、检查时加微状态及取消原因；
...
- 左侧导航在“企微管理”下设置企微配置、小程序配置、落地页配置和微信客服管理；“应用管理”不生成独立菜单，从企微配置列表操作列进入，进入后不新增工作台标签；落地页配置仅编辑既有记录，不提供新增入口；
...
- 小程序和店铺在 V1.0 必须达到“可配置”状态，不以空白占位页作为完成；至少支持新增、编辑、校验、启停、测试连接/链接和配置审计。落地页配置不提供新增入口，至少支持筛选、预览、编辑、URL校验、启停、停用后删除及配置审计。
...
| 员工编号 | 合数BOSS按 `B` + 5 位全局流水号生成并永久维护；旧SCRM员工标识只作为历史别名 |
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V2.12 | 2026-09-20 | 应用管理从独立三级菜单改为企微配置行内“应用”入口，页面不新增工作台标签；企微管理收敛为企微、小程序、落地页和微信客服入口 |
| V2.11 | 2026-09-16 | 新增一级审计管理归属说明；异常中心与短信发送结果并列纳入审计管理，短信管理保留运营商、签名、模板和规则四个三级菜单 |
