...
| 新增/编辑 | 配置岗位名称、岗位编码、范围类型、指定组织、是否包含下级、协作数据策略、排除项、说明和状态 |
...
| `CUSTOM_ORG` | 自定义组织 | 岗位配置一个或多个公司、部门、小组，并分别配置是否包含下级 | 跨部门管理岗 |
...
| `CUSTOM_ORG` | 自定义组织 | 管理员选择一个或多个公司、部门、小组，并分别配置是否包含下级 | 跨部门管理岗 |
...
- 编辑支持落地页名称（必填、50字以内）、H5地址（必填且以 `http://` 或 `https://` 开头）、主跳转方式和兜底优先级。主跳转方式固定单选“获客助手、小程序、微信客户”；其余方式作为兜底项，可上移、下移调整尝试顺序，主方式不可同时出现在兜底列表；
...
| `sys_position` | 数据岗位编码和名称唯一；保存范围类型、协作策略、是否包含下级、状态和版本；不保存菜单操作权限 |
