...
| 主要范围 | 系统管理承载组织、员工、企微身份、岗位、角色、菜单、系统参数、字典、日志、地区、同步和权限；业务配置承载短信与企微管理；审计管理承载异常中心和发送结果 |
...
本文中的“系统管理”“业务配置”和“审计管理”不是合数统一管理平台外层的公共系统管理。用户从统一平台应用中心进入合数BOSS后，才在合数BOSS左侧看到这些模块。业务配置位于系统管理上方，负责短信和企微接入配置；系统管理负责组织员工映射、角色菜单、系统参数、字典、日志与地区治理；审计管理位于系统管理下方，负责异常与短信发送结果追踪。
...
| P1 达到配置可用 | 业务配置：短信管理、企微管理；审计管理：异常中心、发送结果；系统管理：地区管理 |
...
| 合数BOSS主数据字段被企微修改 | 员工列表“一致性=待处理”、同步记录；严重冲突同时进入审计管理—异常中心 | 姓名、手机号、主组织、在职状态、业务角色、数据岗位等不得被企微静默覆盖；保留差异，按规则重新下发BOSS值或由管理员确认 |
| 重复绑定、身份无法唯一匹配、连续失败 | 审计管理—异常中心 | 阻断自动合并，展示原值/新值、影响对象、错误码、重试次数和处理动作 |
...
审计管理为独立一级菜单，位于系统管理下方，依次包含“异常中心”和“发送结果”。
...
- 发送结果从“业务配置—短信管理”迁移至“审计管理”，业务数据、筛选条件、权限和历史记录保持连续；
...
- 左侧导航在“短信管理”下设置四个三级菜单：运营商管理、签名管理、短信模板、短信规则；每个三级菜单使用独立路由、查询条件和列表；发送结果不在短信管理内展示，统一进入“审计管理—发送结果”；
...
- **发送结果联动**：发送任务及供应商回执统一写入“审计管理—发送结果”，字段和追踪规则见5.6.2；
