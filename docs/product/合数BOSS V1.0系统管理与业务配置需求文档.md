...
| 主要范围 | 系统管理承载组织、员工、企微身份、岗位、角色、菜单、系统参数、字典、日志、地区、同步和权限；业务配置承载短信与企微管理；审计管理承载业务异常、企微异常和短信日志 |

> **V2.13 当前生效补充：**审计管理的二级菜单固定为“业务异常、企微异常、短信日志”。业务异常承接数据同步、身份冲突、订单、退款、分配及其他核心业务异常；企微异常专门承接企微接口、回调、授权和通讯录同步异常；“发送结果”统一更名为“短信日志”。微信客服管理不再提供页面级“同步企微客服”按钮；“新增客服”放置在当前企微主体摘要区，待续期或失效主体下禁用。

> **V2.11 当前生效补充：**业务配置保留“短信管理”和“企微管理”。短信管理新增“运营商管理”，并将“发送结果”迁移到独立一级模块“审计管理”的“异常中心”下方；“配置管理”更名为“企微管理”，下设企微配置、小程序配置、落地页配置和微信客服管理，移除店铺配置。应用管理不再作为侧栏菜单，在企微配置列表操作列通过“应用”进入；入口文案统一为“应用”，目标页面标题仍为“应用管理”。后文与本补充冲突的旧菜单、路径及命名均不再生效。

> **V2.12 当前生效补充：**系统管理新增员工时，员工编号改为由管理员手工录入的 6 位数字，保存前执行全局重复性校验，重复时禁止创建。创建登录账号时同样执行全局重复性校验；该账号同时作为向企业微信创建或同步员工时的 `UserID`，不再由后端另行生成。后文与本补充冲突的“`B` + 5 位流水号”及“后端生成 UserID”规则均不再生效。
...
> **V2.7 历史补充（菜单位置由 V2.11 替代）：**业务配置—短信管理新增“短信规则”三级菜单。短信规则按规则名称、类型、关联抖店、签名和模板配置，类型固定为用户下单后、用户提醒、用户授权后；五项均必填。发送结果的现行入口以 V2.11 为准。

> **V2.8 历史补充（已由 V2.12 替代）：**曾定义员工编号为 `B` + 5 位全局流水号；当前生效规则以 V2.12 为准。

> **V2.9 当前生效补充：**合数BOSS账号允许同时绑定多个企业微信身份；账号侧不再建立唯一数量限制，同一 `CorpID + UserID` 仍只能绑定一个账号。个人中心统一使用“外部绑定”名称；系统管理继续负责员工主档、企微成员开通、身份冲突和同步治理。后文与本补充冲突的“双向一对一”“一账号一企微身份”表述均不再生效。

> **V2.10 当前生效补充：**短信规则类型选择“用户下单后”时，固定启用未加微补发策略：支付成功满 3 分钟仍未加微时补发一次；下单满 8 分钟再次检查，仍未加微时再补发一次。任一检查时点已加微则取消该订单尚未执行的补发；同一订单、同一补发节点只允许成功提交一次。发送结果必须标识首次发送或具体补发节点。后文与本补充冲突的旧发送时点均不再生效。

## 1. 文档目标

本文件细化 V1.0 **合数BOSS应用内部**的系统管理与业务配置需求，作为合数BOSS产品原型、前后端开发、数据建模、测试验收和外部系统接入的共同基线。

本文中的“系统管理”“业务配置”和“审计管理”不是合数统一管理平台外层的公共系统管理。用户从统一平台应用中心进入合数BOSS后，才在合数BOSS左侧看到这些模块。业务配置位于系统管理上方，负责短信和企微接入配置；系统管理负责组织员工映射、角色菜单、系统参数、字典、日志与地区治理；审计管理位于系统管理下方，负责业务异常、企微异常与短信日志追踪。
...
| 企微、短信、微信客服、小程序和落地页配置 | 合数BOSS业务配置 |
| 企微及运行异常、短信发送结果 | 合数BOSS审计管理 |
| 企业微信原始组织员工信息 | 企业微信 |
...
| 层级 | V1.0 定义 |
|---|---|
| P0 必须可用 | 系统管理：组织、员工、岗位、角色、菜单、系统参数、字典、日志；企微配置列表提供“应用”入口并承接应用管理与合数BOSS接入权限 |
| P1 达到配置可用 | 业务配置：短信管理、企微管理；审计管理：业务异常、企微异常、短信日志；系统管理：地区管理 |
...
flowchart TB
    subgraph SYNC_AREA["第一部分｜合数BOSS与企业微信同步交互"]
        ...
        BOSS["合数BOSS<br/>组织员工正式主数据<br/>录入6位员工编号并查重"]
        OUTBOX["Outbox与同步适配器<br/>幂等、顺序、重试"]
        PROVISION["员工创建内开通企微<br/>账号查重并作为 UserID<br/>创建、绑定、发送邀请"]
        ACTIVE{"成员是否已激活"}
    ...
    subgraph PERSON_AREA["第二部分｜系统管理人员属性关联"]
        ...
        IDENTITY["当前外部绑定<br/>账号 → 多个 CorpID + UserID<br/>外部身份全局唯一"]
    ...
    IDENTITY -->|"当前有效企微身份"| WECOM
```

全景图的核心口径：

- 企业微信只在初始化阶段提供组织员工基线，并在运行期负责企微身份、激活、客户联系及接待状态；
- 合数BOSS生成并永久维护员工主键；员工编号由管理员录入 6 位数字并经全局查重后保存。正式运行后合数BOSS是姓名、手机号、在职状态、主组织和岗位的唯一主写源；
- 岗位决定“能看哪些数据”，角色决定“能做哪些操作”，账号状态决定当前能否登录；三者必须同时满足；
- 一个登录账号可绑定多个生效企微身份；同一 `CorpID + UserID` 只能绑定一个账号；
...
新增员工使用一个弹窗完成，按“员工身份—任职与权限—账号设置—企微开通”组织信息。“创建企微成员”不再作为独立的页面头部功能；创建员工时可选“同时创建或绑定企微成员”。员工编号由管理员手工录入；创建的登录账号直接作为企微 `UserID`，系统不再另行生成企微 UserID。
...
| 邮箱 | 否 | 填写时校验格式与唯一性 |
| 员工编号 | 是 | 管理员手工录入，必须为恰好 6 位数字；在合数BOSS员工主档内全局唯一，前端失焦或提交时可发起查重，服务端保存前必须再次校验；重复时禁止创建并提示“员工编号已存在” |
...
| 创建登录账号 | 否 | 默认开启；开启后必须填写账号，账号在合数BOSS内全局唯一，同时作为企微同步的 `UserID`；保存前由服务端执行重复性校验，重复时禁止创建；关闭时只创建员工主档 |
| 账号状态 | 条件必填 | 创建登录账号时选择立即启用或暂不启用；初始密码仅在创建成功后展示一次 |
| 同时创建或绑定企微成员 | 否 | 新增时默认开启；仅自有系统主控开启时可用，且必须已创建登录账号。系统先以账号作为 `UserID` 查询企微，并结合手机号核对：已存在且未绑定则绑定，不存在才创建并邀请；已绑定其他账号则进入身份冲突 |

保存逻辑：

1. 前端先完成必填、员工编号 6 位数字、字段长度、手机号和邮箱格式校验，提交中禁用保存按钮；
2. 服务端再次校验员工编号、登录账号、手机号、邮箱和外部身份唯一性，并校验主组织处于启用状态且类型为部门或小组；
3. 员工主档、主组织关系、角色关系和登录账号在同一事务中保存，任一步失败则全部回滚；
4. 保存成功后生成永久 `employee_id`，并将已校验的 6 位员工编号保存为只读 `employee_no`，写入来源 `MANUAL`、创建人、创建时间和 trace_id；
...
异常与拦截：员工编号或登录账号重复时，明确提示冲突字段并禁止保存；手机号或邮箱重复时提示已关联员工并引导查看现有档案；主组织已停用或在提交期间被停用时拒绝保存；无角色分配权限的管理员只能使用其可授权角色集合；无敏感字段权限的操作者保存后只看到脱敏手机号。所有成功、失败和重复拦截均记录审计日志。

验收标准：默认带入当前选中的启用部门或小组；选中公司时主组织留空并要求选择；员工编号非 6 位数字、员工编号重复或账号重复时均禁止保存且不关闭弹窗；连续点击不会生成重复员工；保存成功后员工编号、组织、岗位、角色和账号状态与详情一致；勾选企微开通后，企微 `UserID` 与创建的登录账号一致，并显示创建中、待激活、已绑定、失败或冲突中的准确状态；刷新页面后数据仍存在。
...
- 补开弹窗展示员工编号、姓名、脱敏手机号、主组织、企微部门映射及将作为企微 `UserID` 的登录账号；
- 自有系统主控关闭时入口禁用，并提示“请先在企业微信创建成员，再同步至合数BOSS”。

前置校验：

1. 员工在职且登录账号启用；账号在合数BOSS内全局唯一，目标 `CorpID + UserID`（其中 `UserID` = 登录账号）尚未绑定其他账号；
2. 姓名、手机号、主组织完整，主组织已映射至启用的企微部门；
3. 单一CorpID配置有效，通讯录写权限、成员查询权限和邀请能力校验通过；
4. 操作人拥有 `system:employee:wecom:create` 权限；
5. 先按登录账号对应的 `UserID`、手机号和历史身份映射查询企微成员：未命中才创建；命中未绑定成员需管理员确认后绑定；账号与手机号分别命中不同成员，或命中成员已绑定其他BOSS账号时，停止并进入身份冲突队列。

执行流程：

1. 创建带唯一请求号的企微开通任务，记录员工、账号、CorpID、目标部门和目标UserID；
2. 目标 `UserID` 直接取创建的登录账号；调用企微接口前再次校验本地账号及 `CorpID + UserID` 未重复，创建成功后立即保存 `CorpID + UserID` 外部身份及当前绑定；
3. 保存绑定后发送激活邀请；邀请接口失败时保留已创建成员，将任务标为“邀请失败”并允许重发，不删除成员；
4. 企业微信回调或定时差异扫描更新激活状态；只有企微已激活且绑定关系有效时，页面显示“已激活并绑定”；
...
现状中旧系统没有独立员工编号，历史业务关系主要使用企业微信 `UserID`。新系统不得继续把外部平台 ID 直接作为员工主键，采用以下四类标识：

| 标识 | 示例用途 | 规则 |
|---|---|---|
| 合数BOSS内部员工主键 `employee_id` | 合数BOSS数据库关联 | 合数BOSS生成，永不复用，不随姓名、组织、手机号或外部账号变化 |
| 合数BOSS员工编码 `employee_no` | 页面展示、合数BOSS业务编号 | 由管理员在新增员工时录入恰好 6 位数字；全局唯一，保存后不可修改或复用；旧系统编号只作历史别名 |
| 合数BOSS登录账号 `account` | 登录、企微成员同步 | 创建时手工录入并全局查重；向企微创建或同步员工时直接作为 `UserID` |
| 外部身份 | 企业微信 `CorpID + UserID`、历史SCRM员工标识 | 保存在员工外部身份表，记录来源、CorpID、状态和生效时间，不直接替代内部主键 |

当前企微身份与合数BOSS登录账号采用账号一对多、外部身份全局唯一的生效绑定：一个账号可以绑定多个企微 `CorpID + UserID`，同一企微身份同一时刻只能绑定一个账号。历史 SCRM 中的企微 `UserID`、解绑记录和已失效身份必须保留为只读历史映射，确保客户负责人、活码、跟进和历史操作能够找到合数BOSS员工，但不得参与当前登录、权限计算和新业务归属。
...
| 场景 | 处理规则 |
|---|---|
| 历史员工首次导入 | 先进入暂存区，完成身份匹配和冲突处理后建立正式员工主档；已有符合 6 位数字且不重复的员工编号可经确认导入，否则由管理员补录后才能进入正式主档 |
| 多个历史编号 | 旧SCRM员工ID和旧企微UserID进入别名/外部身份表，关联同一个 `employee_id`，不得直接作为正式员工编号 |
| 同一员工新增或调整企微绑定 | 账号可并行保留多个生效企微身份；新增前校验目标 `CorpID + UserID` 未绑定其他账号，解绑或迁移时写入历史 |
...
| 字段 | 正式主责 | 冲突处理 |
|---|---|---|
| 合数BOSS员工编码 | 合数BOSS | 管理员录入 6 位数字，保存前全局查重；创建后永久不复用，旧系统员工标识只作历史别名 |
| 合数BOSS登录账号 / 企微 UserID | 合数BOSS | 创建账号时全局查重；同步企微时以该账号作为 UserID，冲突时禁止覆盖 |
| 企业微信身份及接待状态 | 企业微信 | 一个 `account_id` 可关联多个身份；`CorpID + UserID` 跨账号唯一，冲突不得覆盖 |
...
| 建立正式主档 | 经确认后创建或合并员工主档，对管理员录入的 6 位员工编号和登录账号查重，建立组织、岗位、角色和企微绑定 | 正式员工编号覆盖率100%且无重复，登录账号无重复，每个 `CorpID + UserID` 只绑定一个账号，账号可关联多个外部身份 |
...
| 合数BOSS主数据字段被企微修改 | 员工列表“一致性=待处理”、同步记录；严重冲突同时进入审计管理—业务异常 | 姓名、手机号、主组织、在职状态、业务角色、数据岗位等不得被企微静默覆盖；保留差异，按规则重新下发BOSS值或由管理员确认 |
| 重复绑定、身份无法唯一匹配 | 审计管理—业务异常 | 阻断自动合并，展示原值/新值、影响对象、错误码、重试次数和处理动作 |
| 企微接口、回调或通讯录同步连续失败 | 审计管理—企微异常 | 展示企微接口、错误码、脱敏请求摘要、重试次数和处理轨迹 |
...
| 数据范围 | 支持本人数据、负责小组、负责部门及下级、自定义组织、当前公司全部五种基础范围 |
...
| `MANAGED_GROUP` | 负责小组 | 当前员工作为有效小组负责人所负责的全部小组；负责多个小组时取并集 | 小组管理岗 |
| `MANAGED_DEPARTMENT` | 负责部门及下级 | 当前员工负责的部门、部门直属员工和全部下级小组 | 部门管理岗 |
| `CUSTOM_ORG` | 自定义组织 | 岗位配置一个或多个公司、部门、小组，不提供独立范围扩展开关 | 跨部门管理岗 |
...
| `MANAGED_GROUP` | 本人负责小组 | 当前用户作为有效小组负责人所负责的全部小组；负责人兼管多个小组时取并集 | 小组管理岗 |
| `MANAGED_DEPARTMENT` | 本人负责部门及下级 | 当前用户作为有效部门负责人所负责的部门，以及部门下全部小组和直接归属部门的员工数据 | 部门管理岗 |
...
- 岗位列表的数据范围摘要分别显示“本人”“负责小组”“负责部门及下级”“当前公司全部”或“自定义N个组织”，不得统一显示为含义模糊的“本组织及下级”。
...
审计管理为独立一级菜单，位于系统管理下方，依次包含“业务异常”、“企微异常”和“短信日志”。

#### 5.6.1 业务异常与企微异常

- 页面采用“异常概览 + 查询区 + 异常列表 + 详情抽屉”结构；概览分别展示待处理、重试中和转人工数量；
- 业务异常汇总数据同步、身份冲突、分配执行、活码、问卷关联、订单和退款等核心业务异常；企微异常汇总企微授权、接口、回调验签、事件解析和通讯录同步异常；
...
- 原“发送结果”统一更名为“短信日志”，位于“审计管理”，业务数据、筛选条件、权限和历史记录保持连续；
- 列表展示提交结果、供应商回执、送达状态、失败码和计费条数，并按业务请求号追踪完整链路；
- 下单短信额外保存订单编号、规则版本、首次/补发标识、补发节点、计划检查时间、实际检查时间、检查时加微状态及取消原因；
- 菜单迁移只改变入口，不改变发送任务、回执处理、补发幂等及审计口径。

### 5.7 短信管理（P1）

- 左侧导航在“短信管理”下设置四个三级菜单：运营商管理、签名管理、短信模板、短信规则；每个三级菜单使用独立路由、查询条件和列表；短信日志不在短信管理内展示，统一进入“审计管理—短信日志”；
- **运营商管理**：维护运营商/短信服务商编码、名称、接入信息、状态、优先级和更新时间；敏感凭证加密保存且不回显，停用后不得承接新发送任务，历史发送记录仍可追溯；
- **签名管理**：签名名称、供应商、审核状态、适用场景、状态和有效期；只有审核通过且启用的签名可用；
- **短信模板**：模板编码、名称、内容、变量、签名、供应商模板编号、审核状态和启停状态；发送前校验变量；
- **短信规则**：列表展示规则编码、规则名称、类型、签名、短信模板、关联抖店、补发策略、状态和更新时间；支持关键词/状态查询、重置、新增、详情和编辑。新增/编辑填写规则名称、类型、关联抖店、签名和模板，均为必填；类型固定为用户下单后、用户提醒、用户授权后。签名、模板和抖店候选项必须有效且在当前用户权限范围内；同一抖店、类型在同一时点只能有一条生效规则，重复时阻止保存并提示冲突；所有新增、编辑、启停记录操作人、前后值和时间；
- **用户下单后补发**：首次短信仍按用户下单事件发送。系统分别以支付成功时间和下单时间建立补发检查任务：支付成功后第 3 分钟检查当前加微状态，未加微则执行第一次补发；下单后第 8 分钟再次检查，仍未加微则执行第二次补发。两个节点独立按各自事件时间计算，不因回调到达顺序改变时间基准；任一节点检查到已加微时不得发送，并取消尚未执行的后续补发。每个订单的 `PAY_SUCCESS_3_MIN`、`ORDER_CREATED_8_MIN` 节点必须使用独立幂等键，重复订单、支付或定时任务回调不得重复提交；
- **短信日志联动**：发送任务及供应商回执统一写入“审计管理—短信日志”，字段和追踪规则见5.6.2；
- V1.0 **复用历史短信服务**，通过适配层接入，不更换供应商，不重复建设供应商结算与智能路由能力。

### 5.8 应用管理（企微配置入口，P0）

- 应用管理不再作为业务配置侧栏菜单；用户从“业务配置—企微管理—企微配置”列表操作列点击“应用”进入，目标页面标题仍为“应用管理”；
- “应用”入口与企微配置记录绑定，跳转时携带对应配置主键或 `CorpID`，默认只展示当前企微配置关联的应用；
...
- 左侧导航将原“配置管理”更名为“企微管理”，下设企微配置、小程序配置、落地页配置、微信客服管理；每个三级菜单使用独立路由、列表和管理表单，不在企微管理页面内使用页签切换；不设置店铺配置菜单；
- **企微配置**：单一 CorpID，包含企业标识、AgentId、回调地址、通讯录/客户联系权限、授权状态和最近校验时间；敏感凭证加密且不回显。列表操作列提供“详情、编辑、应用”，其中“应用”跳转至当前配置对应的应用管理页面；
- **小程序配置**：AppID、应用名称、环境、业务域名、授权状态和负责人；密钥仅支持更新或轮换；
- **落地页配置**：页面标题独立显示“落地页配置”；列表依次展示落地页编码、落地页名称、H5地址、跳转方式、兜底方式优先级、状态、创建时间和操作。落地页编码为稳定标识，H5地址必须为合法的 `http://` 或 `https://` 地址；
- 落地页配置不提供新增入口，数据由既有业务链路或外部配置来源产生。用户可按落地页名称和启用/停用状态筛选，并对现有记录执行预览、编辑和启停；停用状态允许删除；
- 每条落地页仅允许选择一个主跳转方式，枚举限定为“获客助手、小程序、微信客服”。兜底方式用于配置除主跳转方式以外的其他方式及执行优先级；主方式不可用时按配置顺序依次尝试；
- 编辑支持修改落地页名称、H5地址、唯一跳转方式和兜底方式优先级，不允许修改落地页编码。预览以手机端样式展示用户实际点击后看到的H5页面，并显示落地页编码、地址和主跳转结果；
- 通用配置支持草稿、校验、发布和停用；生产配置变更要求权限、二次确认、审计和版本回退。
- 小程序在 V1.0 必须达到“可配置”状态，不以空白占位页作为完成；至少支持新增、编辑、校验、启停、测试连接/链接和配置审计。落地页配置不提供新增入口，至少支持筛选、预览、编辑、URL校验、启停、停用后删除及配置审计。

### 5.10 企微管理—微信客服管理（P1）

- 页面顶部提供“企微ID”下拉选择，仅展示当前用户有权且已接入的企业微信主体；
- 切换企微ID后，客服分组、客服列表、事件消息、客服消息、统计数据及新建客服的归属全部按所选 `corp_id` 刷新，不得混合不同主体数据；
- 切换时清空页面内关键字和页签临时状态，默认回到客服列表；待续期或失效主体不允许新建客服；
- “新增客服”位于当前企微主体摘要区右侧，让新增动作与当前 `corp_id` 归属关系清晰可见；页面不提供独立“同步企微客服”按钮，数据同步由接口、回调和后台任务承接；
...
| 小组管理岗 | `MANAGED_GROUP` 负责小组 | 所负责小组成员当前负责的数据，以及小组待分配队列 | 由业务角色字段权限决定 |
| 部门管理岗 | `MANAGED_DEPARTMENT` 负责部门及下级 | 部门直属员工及下属小组的业务数据、部门待分配及异常 | 由业务角色字段权限决定；批量导出仍需独立授权 |
...
| `sys_employee` | 合数BOSS内部员工主键、员工编码、姓名和汇总状态；`employee_no` 必须为 6 位数字并建立全局唯一约束；不使用企微 ID 作为主键 |
| `sys_employee_external_identity` | `employee_id`、来源类型、租户/CorpID、外部用户标识、来源员工编号、状态、生效区间；有效外部身份联合唯一 |
| `sys_user_wecom_binding` | 当前企微绑定；取消 `account_id` 唯一约束，保留 `(CorpID, UserID)` 生效联合唯一；一个账号可关联多条外部身份 |
...
| `sys_user` | 登录账号；绑定员工；`account` 建立全局唯一约束，并作为向企微创建或同步员工时的 `UserID`；记录锁定和企微绑定状态 |
...
| `sys_position_org_scope` | 数据岗位的自定义组织、包含下级标记及显式排除项 |
...
| 创建企微成员 | 合数BOSS → 企业微信 | 主控开启时先校验登录账号全局唯一，再以该账号作为 `UserID` 查重并创建或绑定企微通讯录成员；按请求号幂等，创建成功后保存身份映射并发送激活邀请 |
...
- [ ] 新增员工的员工编号必须为恰好 6 位数字，员工编号和登录账号均完成前端提示与服务端强制查重，重复时不产生员工或账号记录；
- [ ] 历史企微 UserID 能够稳定映射到合数BOSS员工，正式员工编号覆盖率达到100%且不存在未解释重复；
- [ ] 初始组织员工数据仅来自业务确认后的企业微信基线，未确认来源数据未进入暂存、正式表或身份映射；
- [ ] 一个登录账号可存在多个生效企微绑定；同一 `CorpID + UserID` 最多绑定一个账号，跨账号重复数为0；
- [ ] 从BOSS开通企微覆盖新建、命中已有成员、身份冲突、邀请失败重试、待激活和激活成功；重复提交不会新增重复企微成员；
- [ ] 通过BOSS创建或同步的企微成员，其 `UserID` 与BOSS登录账号完全一致；账号或 `CorpID + UserID` 冲突时不覆盖已有成员或绑定；
...
- [ ] 普通员工只能查看本人负责或明确协作的数据；小组负责人只能查看本人负责小组；部门负责人可查看部门直属员工和全部下属小组；公司管理者及已授权中台角色可查看当前公司数据；
...
| 员工编号 | 由管理员在新增员工时录入恰好 6 位数字，保存前全局查重；保存后不可修改或复用，旧SCRM员工标识只作历史别名 |
| 登录账号与企微 UserID | 创建登录账号时执行全局查重；账号直接作为向企微创建或同步员工时的 `UserID` |
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V2.13 | 2026-09-24 | 审计管理拆分为业务异常、企微异常和短信日志；发送结果更名短信日志；微信客服新增入口移入企微主体摘要区并移除页面同步按钮 |
| V2.12 | 2026-09-21 | 员工编号改为手工录入 6 位数字并全局查重；创建登录账号时同步查重，且账号直接作为企微同步 UserID |
| V2.11 | 2026-09-17 | 重组菜单：短信管理新增运营商管理并将发送结果迁移至审计管理；配置管理更名企微管理并纳入微信客服，移除店铺配置；应用管理移除独立菜单，改由企微配置操作列“应用”进入 |
| V2.10 | 2026-09-10 | 用户下单后短信新增未加微补发：支付成功后3分钟、下单后8分钟分别检查并补发；补充加微即取消、节点幂等及发送结果追踪字段 |
| V2.9 | 2026-09-10 | 外部身份绑定调整为账号一对多、外部身份全局唯一；个人中心使用“外部绑定”，同步修订员工开通、身份模型、初始化及验收口径 |
| V2.8 | 2026-09-07 | 员工编号展示及生成规则统一为B+5位全局流水号 |
| V2.7 | 2026-09-03 | 短信管理新增短信规则三级菜单，补充列表、新增编辑字段、类型枚举、唯一性、权限、审计及与短信群发的匹配规则 |
| V2.6 | 2026-09-03 | 岗位数据范围名称统一为“本部门”“本部门及下级部门”，移除独立“范围扩展/包含下级组织”配置，并同步范围计算与页面验收口径 |
| V2.5 | 2026-09-01 | 修正落地页跳转方式术语：主跳转方式和兜底方式中的“微信客户”统一改为“微信客服” |
| V2.4 | 2026-09-01 | 重构落地页配置：取消新增入口；列表新增落地页编码、H5地址、唯一跳转方式和兜底优先级；支持手机端预览、编辑、启停及停用后删除，跳转方式限定为获客助手、小程序、微信客服 |
