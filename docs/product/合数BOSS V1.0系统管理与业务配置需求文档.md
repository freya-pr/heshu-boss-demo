...
- **短信规则**：列表展示规则编码、规则名称、类型、签名、短信模板、关联抖店、状态和更新时间；支持关键词/状态查询、重置、新增、详情和编辑。新增/编辑填写规则名称、类型、关联抖店、签名和模板，均为必填；类型固定为用户下单后、用户提醒、用户授权后。签名、模板和抖店候选项必须有效且在当前用户权限范围内；同一抖店、类型在同一时点只能有一条生效规则，重复时阻止保存并提示冲突；所有新增、编辑、启停记录操作人、前后值和时间；
