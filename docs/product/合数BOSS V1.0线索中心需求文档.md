...
| 8 | 营期/班级 | 营期ID、班级ID | 商品规则、订单或人工补充 | 变更保存历史，不改接入快照 |
...
默认采用“线索接入时间队列口径”：选择某一时段进入系统的线索并持续追踪其后续结果。页面可切换“业务发生时间口径”查看特定时段实际发生的加微、预约、成交和退款，但两种口径不得混算。所有指标支持按来源、平台、店铺、商品、IP、IP渠道、IP大类、营期、班级、组织和负责人下钻，并展示指标口径版本。
...
- 常用搜索展示综合关键词、当前待办状态、线索标记和转化状态；更多条件包含分配、解密、短信、加微、问卷、测评、跟进和订单等独立状态，以及来源、添加方式、负责人、直播组、店铺和“日期类型 + 日期区间”；所属直播组支持下拉选择并可输入名称搜索；
...
- 列表上方提供“批量操作”，承载分配、同步订单、短信群发、导入解密数据和导出非解密数据；分配与短信群发处理已勾选线索，同步订单为不依赖列表勾选数据的全局后台任务，导入解密数据按模板内容处理，导出非解密数据按当前查询结果处理；不提供同步手机号和查询异常订单入口。
...
- 点击“更多条件”展开分配、解密、短信、加微、问卷、测评、跟进和订单状态，以及线索来源、添加方式、负责人、所属直播组、店铺名称及业务日期；各环节状态可以组合查询；
...
|---|---|---|---|
| 分配 | 已勾选线索 | 人工指定、轮询分配 | 仅处理待分配线索；人工指定通过组织架构选择员工；轮询按最新营期活码人员名单执行 |
...
![批量分配—人工指定不受营期活码名单及分配上限限制](./assets/drainage-lead/14-manual-assignment-unrestricted-v115.png)

- 弹窗明确影响范围及预计数量，选择具体操作类型后才可确认；同步订单弹窗明确提示“无需勾选线索”，不得显示或使用已勾选线索数量；
- 人工指定先在左侧选择公司、部门或小组，再从右侧员工列表选择接收人；员工列表上方提供“搜索员工姓名或员工编号”输入框，在当前所选组织及其下级组织范围内实时筛选；员工卡片展示员工编号、岗位和当前负责的未转化线索数；人工指定不受营期活码人员名单和最大分配人数限制；
- 轮询分配展示“根据最新营期配置的活码人员名单轮询分配”，并展示本次采用的营期及当前可参与轮询人数；
...
| 状态 | 加微状态 | `wechat_status` | enum | 未加微、已加微、已删除 |
...
- 详情从右侧打开，不改变列表条件和页签；
- 展示客户称呼、微信昵称、当前待办状态、各环节独立状态、转化状态、来源、添加方式、负责人、员工编号、营期、订单、客户、关键时间及备注；
- 敏感字段按权限展示；
...
| 变更线索标记 | 有标记权限 | 有效/无效/重复间变更，必须填写原因 |
...
单条“更多”不得出现编辑线索、添加跟进、变更线索等级，以及分配、同步订单、导入解密数据、导出非解密数据及短信群发等整体数据操作。
...
| 能力 | 接口建议 | 关键参数/返回 |
|---|---|---|
| 列表查询 | `GET /api/v1/leads` | 分页、关键词、状态、来源、负责人、营期ID、店铺、日期类型与区间 |
...
| 轮询分配 | 根据最新营期配置的活码人员名单按稳定顺序轮询，校验最大分配人数，保存营期、名单版本和轮询游标 |

“员工负载分配”不再作为分配方式。原“员工负载”统一定义为员工在活码配置中的**最大可分配人数上限**：

- 活码人员配置保存所属营期、员工、有效活码、已分配人数、最大分配上限、启停状态和备用接待人；
- 员工搜索支持姓名模糊匹配和员工编号模糊匹配，忽略英文字母大小写；清空关键词后恢复当前组织范围内的完整员工列表，无匹配结果时展示引导性空状态；
- 人工指定仅校验员工是否在职且账号启用，不要求员工进入最新营期活码名单，也不校验或阻断最大分配人数；
- 人工指定员工卡片展示“当前负责的未转化线索数”作为管理参考，但该数值不影响能否选择；
- 轮询只从最新营期活码人员名单中选取在职、账号启用、活码有效且未达到上限的员工；
- 达到上限、未配置上限、离职、停用或活码失效的员工自动跳过；全部人员不可用时线索保留待分配并生成异常；
- 上限调整只影响后续分配，不自动回收已经分配的线索；配置变更必须保留操作人、时间和前后值。

每次分配保存分配方式、目标员工、执行结果、操作人和时间；轮询额外保存营期、候选人员名单版本、员工已分配人数与上限快照、轮询游标和目标活码；人工指定保存组织范围、搜索条件和人工选择结果，不保存或校验轮询上限。无可用员工时继续停留待分配队列，不得丢弃或自动标记成功。

改派必须记录原员工、新员工、原因、待办处理、客户关系影响和通知结果。已转客户后的负责人变更由客户中心负责，线索中心只保存来源分配历史。

### 5.4 线索配置

线索配置页面采用“**规则中心**”模式，不按每一种规则新增独立菜单。页面左侧按规则域分类，右侧使用统一规则列表和统一版本机制；V1.0 首批开放“分配与接待”“营期名单流转”和“等级规则”三类，后续可继续注册清洗、标签、回收、提醒等规则域，而不改变页面主体结构。

#### 5.4.1 页面结构

| 页面区域 | 功能要求 |
|---|---|
| 规则分类 | 展示全部规则、分配与接待、营期名单流转、等级规则及数量；选中分类后只展示该类规则 |
...
|---|---|
| 推荐触发 | 客户扫码、批量轮询任务 |
| 适用范围 | 活码、活码分组、营期、班级、渠道、店铺、组织或岗位 |
| 基础条件 | 活码启用；员工在职且账号启用；员工进入适用活码的轮询名单；设置接量日期时当前日期在有效区间内 |
| 容量条件 | 当前负责的未转化线索数小于该员工在活码中的最大分配上限 |
| 动作 | 按稳定轮询顺序或配置顺序选出接待员工，并保存负责人、活码、营期、名单版本、容量和游标快照 |
| 无人可分 | 线索保留待分配，生成异常记录，不得丢弃或伪造成功 |

人工指定只校验员工在职及账号启用，不受营期活码名单和最大分配上限约束；规则页面中的名单和上限只控制自动轮询。

#### 5.4.4 营期名单流转规则

该规则用于根据上一营期的员工表现，自动生成或更新下一营期的活码轮询人员名单。判断对象为员工，不是线索。

| 配置项 | 规则 |
|---|---|
| 来源营期 | 已结束或进入结算阶段的上一营期，从营期库选择 |
| 目标营期 | 待开始或进行中的下一营期，不得与来源营期相同 |
| 推荐触发 | 来源营期结束后每日 02:00 定时重算，同时保留管理员手动试算和补跑 |
| 核心指标 | 员工上一营期转化率=`已转化有效线索数 ÷ 有效分配线索数`；退款是否回退转化按统一转化口径执行 |
| 最小样本 | 必须同时设置最小有效分配量，默认建议30条，防止少量样本造成虚高转化率 |
| 资格条件 | 有效分配量达到下限、转化率达到阈值、员工在职且账号启用 |
| 执行动作 | 将符合条件的员工加入目标营期指定活码或活码分组的轮询名单，可同步默认接量上限 |
| 重复策略 | 已在目标名单则跳过或按明确配置更新上限；不得删除管理员手工加入的员工 |
| 幂等键 | 来源营期ID + 目标营期ID + 规则版本 + 员工ID；重复执行不得重复插入 |
| 失败处理 | 自动重试后进入异常中心，保留失败员工、原因和补跑入口 |

若来源营期统计口径或转化率定义发生变更，必须发布新口径版本，并在试算结果和名单记录中保留指标口径版本。

#### 5.4.5 试算、发布与版本

1. 草稿保存不影响线上业务；
2. 发布前必须执行规则试算，展示数据范围、命中对象、未命中对象、冲突、异常和样例明细；
3. 活码分配规则试算展示命中线索、候选员工和容量分布；营期名单流转规则试算展示员工有效分配量、转化率、是否入选及未入选原因；
...
- 活码列表支持按活码名称/编号、关联IP名称/编号、IP渠道、员工、所属营期和活码状态查询，展示二维码、活码名称与编号、活码类型、活码分组、关联IP、自动带出的IP渠道、所属营期、所属班级、接量日期区间、接待员工、接量容量、企微标签、内部标签、创建人、创建日期、扫码/加微数据和状态；
- 活码可选关联一个启用IP。选择后系统从IP主档自动带出只读的IP渠道名称与渠道编号，不允许在活码中另行选择或覆盖；未选择IP时按无IP归因的通用活码保存；
- **所属营期为选填项**。V1.0 从外部/历史系统同步的只读营期库中选择并支持名称搜索，不允许自由填写；V2.0 上线后改由“交付中心—营期管理”提供同协议数据源；不选择时按通用活码保存，不限定营期；
- **接量日期区间为选填项**。不设置时活码长期有效；设置后仅在开始日到结束日参与轮询，开始日前为“待生效”，结束日后自动变为“已结束”并退出轮询；
- **所属班级为非必填项**。先选择营期，再从该营期的班级下拉库中选择；不选择班级时活码仅归属营期，不影响保存和使用；切换营期后必须清空不属于新营期的班级；
...
| 所属营期 | 否 | 从营期库选择，保存营期ID和名称快照；不选择时按通用活码保存 |
| 所属班级 | 否 | 仅展示所选营期下班级；保存班级ID和名称快照 |
| 接量日期区间 | 否 | 不设置时长期有效；设置后开始日期不得晚于结束日期；已选择营期时必须位于营期允许接量范围内 |
...
![新建活码—营期、班级与接量日期配置](./assets/qr-code/01-live-code-create-drawer.png)

#### 5.5.2 状态和审计

- 状态包括待生效、启用、停用和已结束；日期自动状态与人工停用分别保存，不互相覆盖；
- 活码启停、营期/班级、接量日期、员工名单和上限修改必须记录修改前后值、操作人和时间；
...
数据分析页签至少支持接入量、有效量、分配量、加微量、转客户量、成交量、净GMV和转化率，并按时间、渠道、组织、营期和店铺筛选。指标可以下钻到权限范围内的线索明细；启停渠道不得改写历史统计。
...
| 区域 | 功能要求 |
|---|---|
| 全局筛选 | 日期区间、所属营期、渠道、店铺、IP渠道、IP名称、组织及负责人；营期默认当前有效营期且采用单选，各可选项支持输入搜索；查询条件对整页看板、下钻和导出统一生效；员工在职状态不作为筛选项 |
| 当前营期报表条 | 展示当前营期、统计日期、数据版本和更新时间，并提供“导出本期完整报表”入口 |
| 线索转化旅程 | 按有效线索、加微、问卷、预约、到课、完课、成交展示阶段人数和环节转化率，补充当前在线与退款结果信号 |
| 经营指标 | 按过程效率、结果质量、经营效能展示关键指标卡，并独立展示毛GMV、退款金额、净GMV和目标完成率 |
| 指标下钻 | 点击有效线索、加微、问卷、预约、到课、完课、成交或退款等蓝色人数，打开当前营期明细抽屉；抽屉继承权限与完整筛选快照 |
| 渠道分析 | 独立区块按来源渠道展示线索、加微、问卷、成交、转化率和净GMV |
| IP渠道分析 | 独立区块按IP渠道展示渠道编号、IP数、线索、加微、成交、退款、最终转化率和净GMV |
| 商品分析 | 独立区块按第三方商品展示商品ID、平台、店铺、IP名称、IP渠道、线索、成交、最终转化率和净GMV |
| 成交趋势 | 按当前营期和日期范围展示成交数、净GMV等趋势 |
| 指标口径 | 抽屉展示指标定义、公式、数据源、更新时间、是否待接入；外部数据未接入时显示“待接入”，不得按0计算 |
| 导出 | 顶部“导出本期完整报表”输出当前营期经营概览、渠道、IP渠道和商品数据区；各分析区和下钻抽屉支持独立导出 |
...
| 预约数 | 已订阅所选营期直播的去重客户数 | `COUNT(DISTINCT customer_id WHERE reserved=true)` | 直播系统 |
...
- 默认营期为线索交付营期；渠道、团队、负责人等维度采用事件发生时快照，人员改派不得重写历史统计；
- 退款成功时间距下单时间不超过30日时回冲原销售营期；原销售营期无带班记录时顺延到下一营期，此规则上线前由业务与财务共同确认；
- 比率分子、分母必须使用同一日期、营期、权限和去重范围；分母为0时展示“—”，不得展示0%；
...
- 点击转化旅程、指标卡片、渠道、IP渠道或商品分析中的蓝色人数，进入带相同筛选快照的明细，不得出现汇总有值而明细无法核对；
- [ ] 全局筛选对转化旅程、经营指标、渠道、IP渠道、商品、成交趋势、指标下钻和导出同时生效；
- [ ] 当前营期以卡片式经营看板展示，生命周期、成交退款、GMV和人服比按指标字典计算；
- [ ] 渠道、IP渠道和商品分析保持三个独立区块，均归属于当前所选营期；
- [ ] 毛GMV、退款金额、净GMV和目标完成率名称及公式无歧义；
- [ ] 外部数据缺失展示“待接入”，分母为0展示“—”；
- [ ] 汇总与下钻明细在相同权限、口径版本下可核对；
- [ ] 当前营期完整报表、各分析分区和下钻明细均支持导出，导出数量与页面当前筛选一致；
- [ ] 单营期完整报表同时包含经营概览、渠道分析、IP渠道分析和商品分析四个数据区；
...
- 导出文件名采用 `IP渠道分析_营期_日期区间_生成时间.csv`，并写入渠道编号及所有当前筛选条件。

#### 5.7.6 商品分析

- 统计主体为“平台编码 + 第三方店铺ID + 第三方商品ID”，同名商品不得直接合并；
- 表格字段为商品名称/第三方商品ID、平台、店铺、IP名称、IP渠道、线索数、加微数、成交数、退款数、最终转化率和净GMV；
- 商品归属IP和IP渠道采用业务事件发生时有效关系快照，后续修改IP配置不得重写历史报表；
- 点击线索、加微、成交或退款数字打开对应阶段明细；点击商品名称可进入商品列表并定位当前商品；
- 导出文件名采用 `商品分析_营期_日期区间_生成时间.csv`，长商品ID按文本写出，禁止科学计数法或截断。

#### 5.7.7 当前营期经营报表与导出

顶部“导出本期完整报表”按当前所选营期、页面筛选、数据权限和指标口径生成完整经营报表，至少包含以下四个数据区：

1. **经营概览**：人服比、线索数、加微数、问卷填写数、预约数、到课数、完课数、在线数、成交数、退款数、加微率、问卷填写率、预约率、到课率、完课率、在线率、转化率、退款率、最终转化率、2980退款率、退款金额、毛GMV、净GMV、GMV完成率、单人净GMV、团队转化离散率和营期；
2. **渠道分析**：来源渠道、线索、加微、问卷、成交、转化率和净GMV；
3. **IP渠道分析**：采用5.7.5字段；
4. **商品分析**：采用5.7.6字段。

导出文件头必须记录营期、日期区间、来源/平台、店铺、IP渠道、IP名称、组织、负责人、生成账号、生成时间、口径版本和权限范围。数据量较小时前台直接生成；超过系统参数阈值时转异步任务，完成后从任务结果下载。生产环境不得导出演示数据，外部数据未接入的指标写“待接入”而非0。

#### 5.7.8 指标下钻明细

- 下钻采用右侧抽屉，保留首页上下文，不新增不可追踪的临时统计页面；
- 标题显示“指标名称 + 当前数值”，顶部展示营期、日期、店铺、IP渠道、IP名称及数据权限快照；
- 默认字段为订单编号、客户名称、手机号（按字段权限掩码或明文）、来源、店铺、商品、IP渠道、IP名称、当前负责人、营期、事件时间和阶段；
...
| 加微状态 | `wechat_status` | `NOT_ADDED` 未加微、`ADDED` 已加微、`DELETED` 已删除 |
...
- [ ] 人工指定可按组织架构选择任意在职、启用员工且不受营期名单和人数上限限制；轮询按最新营期活码人员名单及人数上限执行；两种方式均可追溯和重试；
- [ ] 达到活码配置最大分配上限的员工仍可被管理员人工指定，但不得参与自动轮询；
- [ ] 规则按“触发—条件—动作”分类配置；活码分配规则和营期名单流转规则字段、试算结果及执行对象互不混用；
- [ ] 上一营期员工达到最小有效分配量和转化率阈值后可幂等加入下一营期名单；未达阈值、离职停用、重复人员和失败原因均可解释；
