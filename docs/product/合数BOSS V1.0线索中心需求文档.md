...
| 引流期次 | 管理接量期、转化期和追单期，支持批量生成、启停、引用校验和变更日志 | P0 |
...
| 变更期次 | 已勾选线索 | 选择目标引流期次并填写变更原因 | 仅允许选择启用期次；逐条校验权限并记录原期次、目标期次、操作人、时间和原因；部分失败不回滚成功项 |
...
| 变更期次 | 有期次变更权限 | 选择一个启用中的引流期次并填写原因；记录原期次、目标期次、操作人和时间，不改变所属直播组 |
...
- 查询条件：期次名称、期次阶段、启停状态、期次开始时间、期次结束时间；名称支持模糊搜索；
- 列表字段：期次名称、期次阶段、期次开始时间、期次结束时间、状态、活码引用数、业务数据引用数、创建人、创建时间、操作；
- 期次阶段枚举固定为：`接量期`、`转化期`、`追单期`；阶段用于表达业务目的，不由时间自动切换；
...
顶部提供“批量创建”，支持按固定周期生成一个月或一年的期次：

1. 选择生成范围：指定月份或指定年份；
2. 选择期次阶段，并输入每个周期固定天数；
3. 设置首个周期开始时间、名称前缀和默认启停状态；
4. 系统先生成预览，展示期次名称、开始时间、结束时间及冲突提示；
5. 用户确认后按时间顺序创建。月度和年度生成均不得跨越所选范围，年度日级周期最多生成366条；
...
- 目标仅可选择启用中的引流期次，支持输入期次名称搜索；
- 操作人必须填写变更原因。系统记录线索、原期次、目标期次、原因、操作人、时间和请求追踪号；
- 批量变更逐条校验权限和线索状态，允许部分成功，失败项返回具体原因，不回滚成功项；
- 变更期次只改变线索的引流期次归属，不改变所属直播组、负责人、客户关系和来源归因；
...
| 线索变更期次 | `POST /api/v1/leads/change-period` | 支持单条或批量变更 |
...
| 区域 | 功能要求 |
|---|---|
| 全局筛选 | 日期区间、所属期次、渠道、店铺、IP渠道、IP名称、组织及负责人；期次默认当前启用期次且采用单选，各可选项支持输入搜索；查询条件对整页看板、下钻和导出统一生效；员工在职状态不作为筛选项 |
