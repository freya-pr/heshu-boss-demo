...
> **V1.75 当前生效补充：**“查询异常订单”和“同步异常订单”为两个独立入口。查询时至少选择一个平台，系统生成唯一查询批次并展示该批次的异常订单清单；同步入口读取最近一次有效查询批次，只允许勾选状态为“待同步/同步失败”的异常订单，生成逐单补同步任务并回写处理结果。查询不触发同步；同一查询批次、平台和订单的重复补同步请求必须幂等。当前原型结果明确标注演示状态，不作为真实第三方同步成功凭据。

> **V1.73 当前生效补充：**活码统一按多人接待配置，不再区分或展示单人活码类型；移除备用员工、接量日期区间、列表及详情二维码、详情下载入口，以及页面顶部“轮询权重/当前接量”汇总指标，列表不再提供下载操作；取消“添加同一员工”的开关、详情展示和分配逻辑。新增/编辑按“基础信息—排班设置—接待员工（仅全天在线）—客户标记”排列；员工配置统一展示姓名、接量权重和只读当前接量。分时段排班先配置自定义规则人员，再配置不可删除的全天在线兜底人员；新增规则入口位于“自定义分时段规则”旁，每条规则及兜底规则内直接选择员工并设置接量权重，无需另行配置接待员工。后文与本补充冲突的旧表述均不再生效。
...
> **V1.62 当前生效补充：**引流线索“线索来源”筛选移除“认证赠送”和“店播/阿留直播间”两个选项，存量记录仍保留原来源快照并可通过历史数据查看，不回写、不改名。后文与本补充冲突的旧表述均不再生效。

> **V1.63 当前生效补充：**活码管理按已授权企业微信主体隔离。页面必须先明确当前企微主体及企微ID，切换主体后，活码分组数量、列表、汇总统计和新建活码归属同步刷新；不同主体的活码、统计和配置不得混查、混用或跨主体写入。后文与本补充冲突的旧表述均不再生效。

> **V1.64 当前生效补充：**商品短信配置的购买后、授权后和手动提醒均改为必选短信模板，模板来源于业务配置—短信管理的已审核启用模板，不再把触发动作或延迟时长作为模板选项。V1.64 的订单状态方案已被 V1.66 替代。后文与本补充冲突的旧表述均不再生效。

> **V1.65 当前生效补充：**渠道类型统一收口为“平台渠道、IP渠道”。渠道列表不提供新增入口，仅维护已有渠道；列表字段收口为渠道编码、渠道名称、渠道类型、状态和操作，仍支持详情、编辑和启停。后文与本补充冲突的旧表述均不再生效。

> **V1.66 当前生效补充：**抖店、小红书、快手、视频号、百家号/度小店的外部订单状态统一映射为内部订单状态。内部枚举为待支付、已支付、备货中、已发货、已取消、已完成、售后处理中、发货前退款完结、发货后退款完结、收货后退款完结、部分退款；不设置“状态异常”，未产生订单时为空。未知外部状态保留平台原始值、维持原内部状态并进入异常中心。后文与本补充冲突的旧表述均不再生效。

> **V1.67 当前生效补充：**渠道列表进一步移除详情和编辑，仅保留启用/停用操作；渠道编码、名称和类型均作为上游同步或既有主数据只读展示。本规则替代 V1.65 中保留详情、编辑的描述。后文与本补充冲突的旧表述均不再生效。

> **V1.68 历史补充（活码人员与排班规则由 V1.72 替代）：**引流线索订单状态固定为未支付、已支付、已取消、已完成、退款中、已退款，均标注为第三方实时回传，本规则替代 V1.66。活码新增/编辑及列表移除班级和使用明细；自定义分组支持修改、删除并收入口“更多”，默认分组不可删除。本版本曾包含备用员工等旧方案，现行活码规则以 V1.72 为准。

> **V1.69 当前生效补充：**活码管理列表“更多”移除“复制短链”，仅保留按当前状态启用或停用活码。后文与本补充冲突的旧表述均不再生效。

> **V1.70 当前生效补充：**引流线索批量“短信群发”不再人工选择签名、模板或编辑内容，只选择必填订单日期；默认带出当前日期。系统按所选日期、线索所属抖店及当前生效短信规则匹配发送配置，未匹配或匹配多条时不发送并记录失败原因。后文与本补充冲突的旧表述均不再生效。
...
| 活码管理 | 活码分组、新旧活码、排班与接待员工、接量权重、企微标签、内部标签、创建人、营期和效果 | P0 |
...
- 列表上方提供“批量操作”，承载分配、变更期次、同步订单、短信群发、导入解密数据和导出非解密数据；分配、变更期次与短信群发处理已勾选线索，同步订单为不依赖列表勾选数据的全局后台任务，导入解密数据按模板内容处理，导出非解密数据按当前查询结果处理；不提供同步手机号入口；新增“查询异常订单、同步异常订单”全局演示入口，不依赖勾选结果，至少选择一个平台，按勾选顺序展示独立任务；当前不执行真实第三方查询或同步。
...
- 线索来源当前可选：抖店、小红书、淘宝、百家号、视频号、有赞、快手、小鹅通、伴鱼、火花思维、自有系统、支付宝、公户、抖音、星橙、飞策；不再提供“认证赠送”和“店播/阿留直播间”选项；
...
| 查询异常订单 | 全局入口，无需勾选线索 | 至少选择一个查询平台，支持多选 | 生成唯一查询批次，按平台展示异常订单编号、异常原因和处理状态；查询不触发同步 |
| 同步异常订单 | 最近一次有效查询批次 | 勾选待同步或同步失败的异常订单 | 按查询批次和订单逐条生成补同步任务，成功或失败后回写处理结果；已成功订单不可重复勾选 |
...
分配、同步订单、查询异常订单、同步异常订单、短信群发、导入解密数据和导出非解密数据属于整体数据能力，只能从列表工具栏进入，不出现在单条线索“更多”中。“同步手机号”仍下线。异常订单查询与补同步通过查询批次关联，查询清单是同步入口的唯一订单来源。
...
| 订单 | 订单状态 | `order_status` | enum | 第三方映射结果：待支付、已支付、备货中、已发货、已取消、已完成、售后处理中、发货前退款完结、发货后退款完结、收货后退款完结、部分退款；未产生订单时为空 |
...
订单状态不是直接展示平台原始文案，而是由平台原始状态映射为合数BOSS统一状态。系统同时保存支付状态、履约状态、售后状态和页面汇总状态；列表及筛选使用汇总状态，详情和状态历史保留平台原始状态、映射版本及更新时间。
...
- [ ] 批量操作包含分配、变更期次、查询异常订单、同步异常订单、同步订单、短信群发、导入解密数据和导出非解密数据；同步订单无需勾选线索，可多选平台并按平台勾选顺序创建独立全局后台任务；页面无新增线索、同步手机号入口；异常订单补同步明确标注原型演示，未选择平台时阻止创建，按平台勾选顺序展示演示任务且不调用真实第三方服务，单条操作无语音提醒；
- [ ] 单条“更多”不出现整体数据操作；
...
- [ ] 订单状态筛选与列表统一展示11个内部映射状态；外部待付款/未确认映射为“待支付”，退款成功按履约快照区分三类退款完结，部分退款独立展示，未知外部状态不生成“状态异常”；
...
- 页面顶部展示当前企业微信主体名称和企微ID，并允许在当前账号有权管理的已授权主体中切换；切换后重置分组和查询条件，重新加载所选主体的数据；
- 企业微信主体是活码的数据隔离边界。活码分组数量、活码列表、汇总统计、详情和新建入口均按当前企微ID计算或写入；不得展示其他主体的活码，也不得跨主体选择分组、接待员工、企微标签或已有活码；
...
- 活码统一按多人接待配置，不展示活码类型；支持自动通过好友开关、全天在线/分时段排班以及轮询/顺序/随机分配；取消“添加同一员工”的开关、详情展示及分配逻辑，不再因相同客户优先分配至同一员工；
- 全天在线时，在“排班设置”下方单独配置接待员工、接量权重和只读当前接量；
- 分时段排班时先展示自定义规则，再展示不可删除的全天在线兜底；固定说明为“自定义分时段规则说明：选择分时段排班时，默认需要一条全天在线规则，不可删除，保证有员工全天在线”；“添加规则”位于自定义规则标题旁，每条规则内直接选择员工、工作日、在线时间并配置员工接量权重，无需另行配置接待员工；
- 全天在线兜底固定周一至周日 00:00—23:59，至少一名员工且直接可修改；自定义规则与兜底员工自动汇总为活码接待员工；
- 活码标签分为**企微标签**和**内部标签**。企微标签在扫码加微成功后同步写入企业微信；内部标签仅用于合数BOSS内部筛选、运营和数据分析，两类标签分别存储、分别展示，不允许混用；
- 列表操作仅保留“详情”“编辑”和“更多”；“更多”仅包含按状态显示的启用或停用，不提供下载、复制短链和使用明细；详情不展示二维码及下载按钮；页面顶部“导出接量情况”保留；
...
| 企业微信主体 | 自动 | 取当前页面所选企微ID和主体名称，新建时自动写入且不可在表单内改为其他主体 |
...
| 排班类型 | 是 | 全天在线、分时段排班 |
| 排班方式 | 是 | 轮询、顺序、随机 |
| 接待员工 | 全天在线时必填 | 至少一名在职启用员工；员工行直接配置接量权重并只读展示当前接量 |
| 自定义分时段规则 | 分时段时必填 | 至少保留一条；每条规则选择员工、工作日和有效时间，员工权重在规则内直接配置 |
| 全天在线兜底 | 分时段时必填 | 默认生成且不可删除，固定周一至周日 00:00—23:59；至少选择一名员工并配置接量权重 |
...
- 主体切换不修改活码归属；新建、编辑、启停和查看详情均须再次校验当前用户对活码所属企微ID的权限，越权请求统一拒绝并记录安全审计；
...
- [ ] 切换企业微信主体后，分组计数、汇总统计、列表和查询条件同步刷新，且只展示当前企微ID的数据；
- [ ] 新建活码自动归属当前企业微信主体，详情同时展示主体名称和企微ID；
- [ ] 用户无权访问的主体不出现在切换项中，构造跨主体查看或写入请求时服务端拒绝并留痕。

### 5.7 渠道管理

渠道管理使用单一菜单入口，页面内部采用“渠道列表 / 数据分析”两个页签。渠道列表展示已有渠道，不提供新增、详情或编辑入口；列表字段为渠道编码、渠道名称、渠道类型、状态和操作，操作仅保留启用/停用。渠道类型固定为“平台渠道、IP渠道”，筛选支持渠道名称、编码、负责人或来源的综合搜索，以及渠道类型和状态。渠道编码、名称和类型按上游同步或既有主数据只读展示，启停不得改写历史归因。

数据分析展示渠道从线索进入到成交的效果。渠道列表和数据分析共用渠道身份、归因版本、权限范围和筛选上下文，但保持独立页签参数与权限点；分析页签提供日期区间和导出，列表页不展示导出及新增按钮。
...
| 购买后提醒 | 必选短信模板；从“业务配置—短信管理—短信模板”的已审核启用模板中选择 |
| 授权后提醒 | 必选短信模板；从“业务配置—短信管理—短信模板”的已审核启用模板中选择 |
| 手动提醒 | 必选短信模板；从“业务配置—短信管理—短信模板”的已审核启用模板中选择 |
...
- 三个提醒字段表达对应场景发送的短信模板，不表达触发动作或延迟时间；当前演示模板为“短信模版一、短信模版二”，正式数据以短信管理返回的模板名称和模板ID为准；
...
| 订单状态 | `order_status` | `PENDING_PAY` 待支付、`PAID` 已支付、`PREPARING` 备货中、`SHIPPED` 已发货、`CANCELLED` 已取消、`COMPLETED` 已完成、`AFTER_SALE_PROCESSING` 售后处理中、`REFUNDED_BEFORE_SHIP` 发货前退款完结、`REFUNDED_AFTER_SHIP` 发货后退款完结、`REFUNDED_AFTER_RECEIPT` 收货后退款完结、`PARTIAL_REFUNDED` 部分退款；无订单时为空 |
...
- 菜单权限控制各二级页面；
- 按钮权限控制导入解密数据、导出非解密数据、手机号变更、客户档案合并、编辑、无效、跟进、分配、改派、规则发布和活码启停；
...
- **客户建档身份边界（推荐方案，待确认）：**引流线索进入后先保存线索及“待识别经营关系”，只有手机号或 UnionID 任一有效时才创建正式客户主档和客户编号；身份不足的数据不进入正式客户列表，后续补齐身份时再幂等创建或关联客户。
- **活码分配算法（推荐方案，待确认）：**先按活码和当前时间命中唯一排班规则，自定义时段优先、全天规则兜底；时段采用左闭右开 `[开始, 结束)`；轮询游标按“活码 + 排班规则”保存，随机采用权重随机；同一扫码事件以企业微信事件ID幂等，重试保持首次选人结果。
- **无效/重复线索（推荐方案，待确认）：**无效线索仅包括测试恶意、联系方式明确无效、业务明确排除三类并记录原因；同一平台、店铺、外部业务单号重复接收视为技术重复并幂等，同一客户在不同时间、渠道、期次产生的业务事件保留为多条有效线索，不因手机号相同直接合并或删除。
- 接量权重的默认值、允许范围及调整审批人；
- 各营期无可用员工时的兜底人员；
- 渠道首触、末触及辅助归因口径；
- 旧活码数据接口、字段和事件覆盖范围；
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V1.75 | 2026-09-07 | 异常订单查询生成查询批次和明细清单；补同步从查询批次勾选异常订单并逐单回写结果；记录客户建档、活码分配和无效/重复线索推荐方案待确认 |
| V1.74 | 2026-09-04 | 补充查询异常订单、同步异常订单的全局入口、平台顺序、空选择校验及原型演示边界 |
| V1.73 | 2026-09-04 | 移除活码列表下载操作；取消添加同一员工的开关、详情展示及分配逻辑；保留顶部导出接量情况 |
...
| V1.70 | 2026-09-03 | 引流线索短信群发改为必选订单日期，由系统按抖店及生效短信规则自动匹配签名和模板，移除人工选模板及内容预览 |
| V1.69 | 2026-09-03 | 活码列表“更多”移除复制短链，仅保留按当前状态启用或停用活码 |
| V1.68 | 2026-09-02 | 订单状态收敛为六个实时第三方回传枚举；活码移除班级与使用明细，并补充分组修改删除、排班、全天兜底、备用员工及添加同一员工规则 |
| V1.67 | 2026-09-02 | 渠道列表进一步移除详情和编辑操作，仅保留启用/停用；渠道编码、名称和类型只读展示 |
| V1.66 | 2026-09-02 | 将第三方订单状态映射方案并入引流线索：统一11个内部订单状态，外部待付款/未确认统一为待支付，退款按履约快照区分发货前、发货后、收货后退款完结，新增售后处理中和部分退款；不设置状态异常，未知状态保留原值并进入异常中心 |
| V1.65 | 2026-09-02 | 渠道类型收口为平台渠道、IP渠道；渠道列表移除新增入口，仅保留已有渠道的详情、编辑和启停，并将列表字段收口为编码、名称、类型、状态及操作 |
| V1.64 | 2026-09-02 | 引流线索订单状态改为第三方实时回传枚举，移除“无订单”并新增已取消、已完成；商品短信配置三个提醒场景统一改为选择已审核启用的短信模板 |
| V1.63 | 2026-09-02 | 活码管理新增企业微信主体切换与企微ID隔离；分组、列表、统计和新建归属按当前主体刷新，并补充跨主体权限校验、审计和验收规则 |
| V1.62 | 2026-09-01 | 引流线索来源筛选移除“认证赠送”和“店播/阿留直播间”，存量来源快照保持不变 |
