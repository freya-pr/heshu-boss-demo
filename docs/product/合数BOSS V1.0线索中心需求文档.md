...
| IP管理 | 维护老师IP主档、IP大类与大类编码、带编号的IP渠道；不维护关联商品/平台 | P1 |
| 商品列表 | 统一查看第三方商品、店铺归属与活码分配；不展示关联IP | P1 |
...
| 综合关键词 | 可清空输入框 | 模糊匹配订单编号、第三方商品ID、客户名称、客户编号、原始/解密手机号、微信昵称 |
| 当前待办状态 | 单选下拉 | 精确匹配待分配、待解密、待触达、待客户点击、待加微、待填问卷、待测评、已完成培育或无需处理 |
| 线索标记 | 单选下拉 | 有效线索、无效线索、重复线索 |
| 转化状态 | 单选下拉 | 未转化、已转化 |

点击“查询”按全部条件组合查询；点击“重置”同时清空常用条件和更多条件，并将日期类型恢复为线索创建时间。查询必须先经过数据权限过滤，再进行字段脱敏。
...
| 标签 | 线索标签 | `lead_tags` | array | 仅引流线索默认展示；标签关系归属于当前线索，不等同于客户标签 |
...
线索标签列最多直接展示2个标签，超过2个显示“+N”；无标签时展示“—”。点击标签区域打开全部线索标签弹窗，仅按“系统、企业微信”来源分组展示；两类来源均只读，不提供人工新增、编辑或删除。线索转客户时可按业务规则复制或关联标签，但不得覆盖原线索标签关系及来源记录。
...
| 修改加微状态 | 引流线索且有加微状态修正权限 | 回显当前状态；改为“已加企微”时必须搜索并选择有效客户，自动取客户添加时间作为只读加微时间并建立关联；改为“已加个微”时人工选择加微时间；改为“否”时清空当前加微时间；提交后刷新列表并追加旅程与审计 |
...
3. 改为“已加企微”时，必须按客户名称或手机号远程搜索并选择一个未合并的有效客户；提交时后端再次校验客户有效性，禁止仅依赖前端缓存。所选客户必须存在客户添加时间 `created_at`，系统自动将其写入并锁定为 `wechat_added_at`，页面不得手工修改。
4. “已加企微”提交成功后同步 `wechat_method=WECOM`，并在线索上保存关联客户ID、客户编号、客户名称及关联时间；客户选择、状态和加微时间均未变化时不重复写入。改为“已加个微”时同步 `wechat_method=PERSONAL_WECHAT`，由用户选择实际加微时间；改为“否”时清空当前加微方式和当前 `wechat_added_at`，但历史加微时间、UnionID、外部身份及既有旅程仍保留。
5. 每次有效变更必须在线索旅程和审计日志中追加修改前/后状态、修改前/后加微时间、关联客户前/后值、操作人、员工编号、操作时间、来源 `MANUAL`、原因/备注及 traceId；历史记录只追加不可覆盖。幂等判断同时包含状态、加微时间和关联客户。
6. 企业微信事件和第三方同步仍是正常状态来源，人工修改仅用于异常纠正。后续收到可信外部事件时可再次更新当前状态，并继续追加新旅程，不得因为曾人工修改而停止接收事件。

单条“更多”不得出现编辑线索、添加跟进、变更线索等级、变更线索标记，以及分配、同步订单、导入解密数据、导出非解密数据及短信群发等整体数据操作。
...
- [ ] 单条“更多”提供“修改加微状态”；已加企微/个微时加微时间必填，允许仅修正时间；改为“否”时清空当前加微方式和当前加微时间，但旅程中历史记录不删除；有效变更记录状态和时间前后值；
- [ ] 表格设置即时生效并可记忆，恢复默认有效；
...
| 字段 | 必填 | 规则 |
|---|---|---|
| 期次名称 | 是 | 50字以内；去除首尾空格并忽略大小写后，在全部期次中全局唯一，启用、停用记录均参与校验 |
...
顶部提供“批量创建”，支持从指定期次开始时间起选择按月或按年生成：

1. 选择生成范围“按月/按年”和“期次开始时间”；按月生成至开始日期所在月月末，按年生成至所在年年末；
2. 分别设置“固定周期”和“期次时长”。固定周期用于推算相邻期次开始时间，期次时长用于根据每期开始时间推算该期结束时间，两者不得混用；
3. 默认期次名称采用该期期次开始日期的 `YYYYMMDD` 格式，例如 `20260803`；
4. 系统先生成预览，展示期次名称、开始时间、结束时间及冲突提示；预览中的每个期次名称、开始时间和结束时间均可直接修改；
5. 用户确认后按预览顺序创建，实际保存名称以用户最后修改结果为准；
6. 预览内部或与任一既有期次出现同名时，阻断整批提交并逐条提示冲突项，不自动跳过、不部分创建；名称校验统一去除首尾空格并忽略大小写；
...
- [ ] 批量创建可按固定天数生成月度或年度期次，并能预览、跳过冲突和查看任务结果；
...
渠道管理使用单一菜单入口，页面内部采用“渠道列表 / 数据分析”两个页签。渠道列表仅展示渠道编码、渠道名称、渠道类型、来源映射、状态和操作；移除归因规则、负责人及上级渠道三列。数据分析展示渠道从线索进入到成交的效果。两个页签共用渠道身份、归因版本、权限范围和筛选上下文，但保持独立路由参数与权限点。
...
| 区域 | 功能要求 |
|---|---|
| 全局筛选 | 时间拆分为期次时间与线索创建时间；期次默认距当前日期最近的启用期次；期次、渠道、店铺、IP渠道、IP名称均支持多选；组织和负责人受统一数据权限约束；查询条件对整页看板、下钻和导出统一生效 |
| 经营数据 | 单选期次时展示一行；多选期次时展示汇总行及各期次行；不选择期次、仅按线索创建时间筛选时正常汇总且期次列留空。期次列固定，其他字段支持自由选择、自动记忆和恢复默认 |
| 转化链路 | 按有效线索、加微、填写问卷、成交展示阶段人数和环节转化率，移除预约直播、到课和完课节点 |
| 经营指标 | 在同一横向分组表内展示：过程转化含加微率、问卷填写率、预约率、到课率、完课率；结果质量含退款率、最终转化率、GMV完成率；经营效能含人服比、单人净GMV、团队转化离散率、当前在线；成交金额含毛GMV、退款金额、净GMV |
| 指标下钻 | 点击有效线索、加微、问卷、预约、到课、完课、成交或退款等蓝色人数，打开当前期次明细抽屉；抽屉继承权限与完整筛选快照 |
| 渠道分析 | 独立区块按来源渠道展示线索、加微、问卷、成交、转化率和净GMV |
| IP渠道分析 | 独立区块按IP渠道展示渠道编号、IP数、线索、加微、成交、退款、最终转化率和净GMV |
| 商品分析 | 页面移除商品分析模块及对应分区导出 |
| 成交趋势 | 按当前期次和日期范围同时展示成交订单数（个）和成交金额（万元） |
| 指标口径 | 抽屉展示指标定义、公式、数据源、更新时间、是否待接入；外部数据未接入时显示“待接入”，不得按0计算 |
| 导出 | 顶部导出当前统计主体的经营概览、渠道和IP渠道数据；各保留分析区和下钻抽屉支持独立导出 |

页面链路标题统一为“转化链路”，经营模块统一命名为“经营数据”。单选期次时展示一行期次数据；多选期次时第一行展示汇总数据，其后逐行展示各期次数据；不选择期次、仅按线索创建时间筛选时，以创建时间圈定线索并正常汇总其后续转化结果，期次列留空。筛选摘要、下钻和导出必须继承当前筛选，不通过模块改名表达统计差异。

经营数据“过程转化”分组默认展示有效线索数、加微率、问卷填写率、DAY1 到课率、DAY1 完课率、DAY2 到课率、DAY2 完课率、DAY3 到课率、DAY3 完课率。有效线索数按当前筛选去重统计并支持下钻；其余指标同时展示对应人数并支持下钻。列表右上角提供表头设置，期次列固定且不可取消，其他过程转化、结果质量、经营效能和成交金额字段可自由勾选；设置即时生效、自动记忆并支持恢复默认。已有用户保存过表头设置时，系统自动补入有效线索数。
...
- [ ] 全局筛选对转化链路、经营数据、渠道、IP渠道、成交趋势、指标下钻和导出同时生效；
...
店铺管理用于统一维护各外部交易或获客平台中的店铺身份，为线索、订单、商品及渠道归因提供标准店铺主数据。

#### 5.9.1 店铺列表

- 店铺档案查询条件包括综合搜索、平台类型、店铺状态，以及“日期类型 + 日期区间”；综合搜索支持店铺名称和第三方店铺ID；
- 店铺档案日期类型包括创建时间、更新时间和启停时间，分别读取店铺创建、最后修改和最后一次启停状态变更时间；
- 列表字段包括平台类型、店铺名称、第三方店铺ID、线索数/加微数、下单数/支付订单数、净GMV、创建人、创建时间、状态和操作；
- 平台类型包括抖音、有赞、小鹅通、淘宝、小红书和其他，后续通过字典扩展；
- 操作包括新增、编辑、启用和停用；移除独立店铺详情页和店铺权限入口；
...
| 店铺状态 | 是 | 启用、停用 |
| 抖店KEY/平台密钥 | 按平台要求 | 新增店铺时补充平台接口身份信息 |
| 抖店秘钥/接口秘钥 | 按平台要求 | 敏感字段加密保存并按权限脱敏 |
| 短信账号 | 否 | 用于店铺关联的短信业务身份 |
| 备注 | 否 | 店铺补充说明 |

- 店铺唯一身份由“平台类型 + 第三方店铺ID”共同确定；
- 编辑平台类型或第三方店铺ID前必须进行影响预览，已关联业务数据时记录变更前后值，不得覆盖历史快照；
- 新增、编辑和启停必须记录操作人、操作时间、修改前后值及请求追踪号。

#### 5.9.3 店铺权限（移除）

店铺配置不再维护“店铺负责人”和“权限范围”，页面移除对应字段、入口和权限弹窗；数据可见范围统一由系统数据权限控制。

#### 5.9.4 验收标准

- [ ] 可以按平台类型、店铺名称、第三方店铺ID、状态及创建/更新/启停日期查询；
- [ ] 店铺档案日期筛选只影响店铺范围，不得被误用为线索、订单、支付或退款统计日期；
- [ ] 同一平台的第三方店铺ID重复时阻止保存，不同平台允许使用相同外部ID；
- [ ] 新增和编辑表单包含店铺名称、店铺ID、KEY、秘钥、短信账号和备注，并完成必填与敏感字段校验；
- [ ] 页面不展示店铺负责人、权限范围、店铺权限或独立店铺详情入口；
- [ ] 停用店铺不再参与新业务归因，历史数据仍可按权限查询。

#### 5.9.5 经营统计范围与日期口径

店铺经营统计仅保留“线索归因口径”，移除“业务发生口径”。期次筛选统一使用“全部期次”概念，不再展示“全部营期”；列表、下钻及导出保持同一筛选快照。

| 统计方式 | 日期选择 | 计算规则 | 适用场景 |
|---|---|---|---|
| 线索归因口径（默认） | 固定为线索进入时间 | 先按线索进入时间圈定同一批店铺归因线索，再持续追踪这批线索后续加微、转客户、下单、支付和退款结果 | 比较不同店铺、渠道或营期引入线索的最终转化质量 |

- 列表顶部展示线索归因口径、线索进入时间、日期区间和期次，防止用户误读；
- 下钻、导出和接口响应必须携带 `statistics_mode=attribution`、`date_range`、`period_id` 和口径版本。
...
V1.0 店铺管理不提供独立店铺详情页或经营详情抽屉；关键经营指标直接在店铺列表展示并按统一线索归因口径下钻。
...
- 店铺改名或停用不得重写历史线索和订单的店铺快照。

### 5.10 IP管理

IP管理用于维护以老师为主体的业务IP主档。IP名称应填写老师姓名或对外使用名称，例如“阿留皮皮”，不再使用“研究院”“品牌中心”等机构名称代替老师IP。
...
筛选支持IP名称、IP编号或创建人综合搜索，以及IP大类（支持按名称或大类编码检索）、IP渠道和状态。列表顺序固定为IP大类、IP渠道、IP名称，其后展示创建信息、状态和操作；移除“当前IP”统计卡、关联商品、关联平台。列表操作仅保留编辑、启用和停用。

#### 5.10.2 IP配置（移除）

新增IP和IP配置属于同一功能，因此移除独立“IP配置”按钮和抽屉，统一由新增IP、编辑IP维护“IP大类—IP渠道—IP名称”关系。

- 同一IP编号同一时点只能存在一组生效的大类与渠道关系；修改关系必须记录修改前后值和生效时间；
- 停用IP后不能再被新活码选择，但历史活码、扫码、线索及订单仍按原IP编号和渠道快照追溯；
- 活码管理只引用IP编号并解析当前IP主档关系，禁止在活码侧维护第二套IP大类或IP渠道关系。

#### 5.10.3 新增、编辑与字典联动

- 新增和编辑表单字段顺序为IP大类、IP渠道、IP名称；三项必填，IP说明选填；
...
- [ ] 页面不展示“当前IP”统计卡和独立“IP配置”入口；新增、编辑统一按IP大类、IP渠道、IP名称顺序维护；
...
- [ ] IP编号自动递增且不可修改；IP列表不展示关联商品和关联平台。
- [ ] 新建或编辑活码选择IP后自动带出只读IP渠道；保存后列表和详情展示同一IP编号、渠道编号，变更留痕可追溯。

### 5.11 商品列表

商品列表是线索中心内的第三方商品查询页面，用于回答“哪个外部商品属于哪家店铺、是否已分配活码”。本模块不展示或维护关联IP。本模块与V2.0产品中心的“商品管理”边界不同：V1.0只承接第三方商品同步、归因和内部备注，不负责自有商品定价、权益或上下架策略。
...
- 支持商品名称、商品ID或店铺名称综合搜索，并支持店铺类型、商品状态和活码分配状态筛选；不提供关联IP筛选；
...
- [ ] 线索中心存在独立“商品列表”入口；
- [ ] 列表完整展示创建/更新时间、商品名称、是否分配活码、商品ID、商品状态、店铺名称和店铺类型，不展示关联IP；
...
- [ ] 表格字段设置可即时生效并在重新进入页面后恢复，订单编号、线索来源和操作始终展示，订单编号位于第一业务列。
- [ ] 引流线索默认展示线索标签，最多直接展示2个并以“+N”表示其余数量；点击后仅按“系统、企业微信”来源查看全部标签，不提供人工新增标签，三方品线索不展示该字段；
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V1.58 | 2026-08-31 | 期次名称改为全局唯一且批量重名整批阻断；修改为已加企微时必须关联有效客户并锁定客户添加时间；单条更多移除变更线索标记；线索概览默认新增有效线索数并兼容历史表头设置 |
| V1.56 | 2026-08-28 | 引流线索新增“线索标签”默认字段：列表最多展示2个及“+N”，点击按企业微信/BOSS人工/系统规则分组查看全部标签，并支持添加不重名的BOSS人工标签；标签关系归属于当前线索，三方品线索不展示该字段 |
| V1.53 | 2026-08-28 | 线索概览链路名称统一为“转化链路”；未筛选期次和期次时间、仅筛选线索创建时间时，切换为线索数据汇总并贯穿指标、下钻和导出 |
| V1.54 | 2026-08-28 | 经营模块统一命名为“经营数据”；单期一行，多期展示汇总及各期次行，不选期次时正常汇总且期次列留空，不再切换线索经营数据名称 |
...
| V1.57 | 2026-08-28 | 修改加微状态新增实际加微时间；已加企微/个微时必填，选择否时清空当前时间但保留历史，支持仅修正时间并记录前后值；线索标签来源收口为系统和企业微信 |
