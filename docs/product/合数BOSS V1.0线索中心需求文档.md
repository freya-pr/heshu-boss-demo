...
3. 选择与当前相同的状态时不重复写入，提示“加微状态未变化”；选择其他状态后提交，列表状态和当前待办状态应立即重新计算并刷新。
4. 改为“已加企微”时同步 `wechat_method=WECOM`；改为“已加个微”时同步 `wechat_method=PERSONAL_WECHAT`；改为“否”时清空当前加微方式，但不得删除历史加微时间、UnionID、外部身份或既有旅程。
5. 每次有效变更必须在线索旅程和审计日志中追加记录：修改前状态、修改后状态、操作人、员工编号、操作时间、来源 `MANUAL`、原因/备注及 traceId；历史记录只追加不可覆盖。
...
- [ ] 单条“更多”提供“修改加微状态”，弹窗回显并支持“否、已加企微、已加个微”；相同状态不重复写入，有效变更刷新列表并写入只追加旅程与审计；改为“否”不清除历史身份和加微记录；
- [ ] 表格设置即时生效并可记忆，恢复默认有效；
...
| 期次经营数据 | 采用横向分组表展示当前期次；左侧固定期次名称，表头按过程转化、结果质量、经营效能、成交金额分组，并提供“导出数据”入口 |
| 线索转化旅程 | 按有效线索、加微、问卷、成交展示阶段人数和环节转化率，移除预约直播、到课和完课节点 |
...
| 导出 | 顶部“导出本期完整报表”输出当前期次经营概览、渠道、IP渠道和商品数据区；各分析区和下钻抽屉支持独立导出 |
...
- 点击转化旅程、期次经营数据、渠道、IP渠道或商品分析中的蓝色人数，进入带相同筛选快照的明细，不得出现汇总有值而明细无法核对；
- [ ] 全局筛选对转化旅程、经营指标、渠道、IP渠道、商品、成交趋势、指标下钻和导出同时生效；
- [ ] 当前期次按“过程转化、结果质量、经营效能、成交金额”横向分组展示；生命周期、成交退款、GMV和人服比按指标字典计算；
- [ ] 渠道、IP渠道和商品分析保持三个独立区块，均归属于当前所选期次；
- [ ] 毛GMV、退款金额、净GMV和目标完成率名称及公式无歧义；
- [ ] 外部数据缺失展示“待接入”，分母为0展示“—”；
- [ ] 汇总与下钻明细在相同权限、口径版本下可核对；
- [ ] 当前期次完整报表、各分析分区和下钻明细均支持导出，导出数量与页面当前筛选一致；
- [ ] 单期次完整报表同时包含经营概览、渠道分析、IP渠道分析和商品分析四个数据区；
...
- 导出文件名采用 `商品分析_期次_日期区间_生成时间.csv`，长商品ID按文本写出，禁止科学计数法或截断。

#### 5.8.7 当前期次经营报表与导出

顶部“导出本期完整报表”按当前所选期次、页面筛选、数据权限和指标口径生成完整经营报表，至少包含以下四个数据区：

1. **经营概览**：人服比、线索数、加微数、问卷填写数、预约数、到课数、完课数、在线数、成交数、退款数、加微率、问卷填写率、预约率、到课率、完课率、在线率、转化率、退款率、最终转化率、2980退款率、退款金额、毛GMV、净GMV、GMV完成率、单人净GMV、团队转化离散率和期次；
2. **渠道分析**：来源渠道、线索、加微、问卷、成交、转化率和净GMV；
3. **IP渠道分析**：采用5.8.5字段；
4. **商品分析**：采用5.8.6字段。

导出文件头必须记录期次、日期区间、来源/平台、店铺、IP渠道、IP名称、组织、负责人、负责人组织、生成账号、生成时间、口径版本和权限范围。数据量较小时前台直接生成；超过系统参数阈值时转异步任务，完成后从任务结果下载。生产环境不得导出演示数据，外部数据未接入的指标写“待接入”而非0。
