...
- 活码统一按多人接待配置，不展示活码类型；支持自动通过好友开关、全天在线/分时段排班以及轮询/顺序/随机分配，可开启“添加同一员工”；
...
- 列表支持下载活码、查看详情、编辑和启停；不提供复制短链和使用明细，详情不展示二维码及下载按钮；
...
- 主体切换不修改活码归属；新建、编辑、启停、下载和查看详情均须再次校验当前用户对活码所属企微ID的权限，越权请求统一拒绝并记录安全审计；
