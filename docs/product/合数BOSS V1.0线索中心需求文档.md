...
> **V1.79 当前生效补充：**重绘对外讲解版六段式流程，统一为“多源接入—治理建档—归因与分配—触达与加微—培育与转化—分析与优化”。移除“班级”等已废弃口径，明确基础归因先于活码匹配，分配、身份处理和订单同步可并行，异常任务在各阶段可修复后重试。

> **V1.78 文档拆分说明：**线索中心已按引流线索、线索配置、引流期次、活码管理、渠道管理、店铺管理、IP管理、商品列表和短信配置拆分为9份可独立评审、开发和验收的细化模块需求。本总文档继续维护跨模块主流程、统一状态、数据模型、接口、权限、验收及发布回滚规则。“线索概览”归属首页，不纳入本次拆分。

> **V1.77 当前生效补充：**商品短信配置匹配到“用户下单后”短信规则时，执行统一的未加微补发策略：支付成功后 3 分钟检查未加微则补发；下单后 8 分钟再次检查，仍未加微则再补发。任一时点已加微即取消剩余补发，同一订单同一补发节点必须幂等。具体任务与发送结果字段以系统管理与业务配置需求文档 V2.10 为准。

> **V1.76 当前生效补充：**引流线索订单状态统一使用 11 个内部映射枚举：待支付、已支付、备货中、已发货、已取消、已完成、退款中、发货前退款完结、发货后退款完结、收货后退款完结、部分退款。`AFTER_SALE_PROCESSING` 页面显示为“退款中”；内部编码、业务定义和映射逻辑不变。未产生订单时为空，未知外部状态保留原始值、维持最近一次有效内部状态并进入异常中心。

> **V1.75 当前生效补充：**“查询异常订单”和“同步异常订单”为两个独立入口，活码列表不提供下载。查询时至少选择一个平台，系统生成唯一查询批次并展示异常订单清单；同步入口读取有效查询批次，只允许勾选待同步或同步失败的异常订单，生成逐单补同步任务并回写结果。同一查询批次、平台和订单的重复补同步请求必须幂等。当前原型结果标注为演示状态，不作为真实第三方同步成功凭据。

> **V1.73 评审补充：**新增 5.6.3—5.6.8，解释活码详情字段、分配链路、统计口径、异常与配置变更影响。已明确的 V1.72 规则继续生效；2026-09-04 已确认“自定义时段内无人可接待时，自动转给全天兜底人员”。其余新增算法、匹配优先级、兜底边界和统计口径仍为建议待确认，不是已上线能力。原型差异见 5.6.8，本次仅补充文档。

> **V1.72 历史补充（活码操作范围由 V1.75 替代）：**活码统一按多人接待配置，不再区分或展示单人活码类型；移除备用员工、接量日期区间、列表及详情二维码、详情下载入口，以及页面顶部“轮询权重/当前接量”汇总指标。新增/编辑按“基础信息—排班设置—接待员工（仅全天在线）—客户标记”排列；员工配置统一展示姓名、接量权重和只读当前接量。分时段排班先配置自定义规则人员，再配置不可删除的全天在线兜底人员；每条规则及兜底规则内直接选择员工并设置接量权重。本版本的列表下载规则已被 V1.75 替代。

> **V1.71 当前生效补充：**引流线索列表字段“首单商品名称”统一更名为“商品名称”，底层字段 `first_product_name` 暂保持兼容。引流线索不再使用“所属直播组”概念，筛选、列表与字段设置、详情、单条及批量变更、查询接口参数和验收项均移除直播组；“变更期次”仅允许选择启用中的所属期次并记录变更审计。后文与本补充冲突的旧表述均不再生效。

> **V1.70 历史补充（活码操作范围由 V1.75 替代）：**引流线索批量“短信群发”不再人工选择签名、模板或编辑内容，只选择必填订单日期；默认带出当前日期。系统按所选日期、线索所属抖店及当前生效短信规则匹配发送配置，未匹配或匹配多条时不发送并记录失败原因。本版本的活码操作范围已被 V1.75 替代。
...
> **V1.64 历史补充（活码人员与排班规则由 V1.72 替代）：**引流线索订单状态固定为未支付、已支付、已取消、已完成、退款中、已退款，均由第三方实时回传并映射；未产生订单时为空。活码新增/编辑及列表移除班级和使用明细；分组支持修改、删除并收入口“更多”。本版本曾包含备用员工等旧方案，现行活码人员、权重和分时段排班规则以 V1.72 为准。
...
| 二级菜单 | 主要职责 | 优先级 |
|---|---|---|
| [引流线索](./合数BOSS V1.0线索中心-引流线索需求文档.md) | 广告、活动、直播、落地页等引流线索及扩展字段；承载分配、改派和分配历史 | P0 |
| [引流期次](./合数BOSS V1.0线索中心-引流期次需求文档.md) | 管理待开始、接量期、转化期、追单期和已停用阶段，支持批量生成、启停、引用校验和变更日志 | P0 |
| [线索配置](./合数BOSS V1.0线索中心-线索配置需求文档.md) | 以规则中心分类管理活码分配和营期名单流转，支持条件组合、试算、冲突检测、发布停用和版本回退 | P0 |
| [活码管理](./合数BOSS V1.0线索中心-活码管理需求文档.md) | 企业微信主体及企微ID切换、活码分组、新旧活码、排班与接待员工、接量权重、企微标签、内部标签、创建人、营期和效果 | P0 |
| [渠道管理](./合数BOSS V1.0线索中心-渠道管理需求文档.md) | 渠道列表维护渠道、来源映射、归因口径和启停；数据分析页签展示接入、有效、分配、加微、转客户及成交分析 | P1 |
| [店铺管理](./合数BOSS V1.0线索中心-店铺管理需求文档.md) | 店铺、主体、应用、组织和线索来源映射 | P1 |
| [IP管理](./合数BOSS V1.0线索中心-IP管理需求文档.md) | 按IP大类、IP渠道、IP名称顺序维护IP主数据，不关联商品或平台 | P1 |
| [商品列表](./合数BOSS V1.0线索中心-商品列表需求文档.md) | 统一查看第三方商品、店铺归属、活码分配与关联IP | P1 |
| [短信配置](./合数BOSS V1.0线索中心-短信配置需求文档.md) | 按商品与活码配置购买后、授权后及手动提醒策略 | P1 |

导入、同步、任务进度、失败下载和数据对账属于引流线索页面操作，不单独生成菜单。三方品线索沿用统一线索主模型及独立扩展字段，统一在 V1.5 开放菜单和页面能力。

上表模块名称均可直接进入对应的独立细化需求文档。本总文档继续负责端到端流程、跨模块状态、数据模型、接口、权限、验收及发布回滚规则；各模块文档自身包含完整的页面、字段、交互、业务、异常和验收说明，不依赖额外导航文档。

IP管理放在店铺管理之后，商品列表紧随IP管理；此前 V1.23 的“移除 IP 管理”仅作为历史决策保留，由 V1.28 起失效。
...
```mermaid
flowchart LR
    A["1. 多源线索接入<br/>API / Webhook / 文件导入 / 手工导入"] --> B["2. 治理与线索建档<br/>校验 / 标准化 / 幂等去重 / 有效性"]
    B --> C["3. 归因与负责人分配<br/>平台·店铺·商品·IP·期次 / 活码·排班·权重"]
    C --> D["4. 触达与客户连接<br/>身份解密 / 短信 / 企微或个微加微 / 唯一客户建档"]
    D --> E["5. 培育与业务转化<br/>问卷 / 测评分级 / 预约·直播 / 下单·支付·退款"]
    E --> F["6. 分析与持续优化<br/>处理效率 / 转化漏斗 / 归因经营 / 异常补偿"]
    C -. "分配、身份处理、订单同步可并行" .-> D
    F -. "反向优化规则与运营策略" .-> C
```

对外演示可直接使用高清图：[线索中心对外六段式流程 V1.79](./assets/线索中心-对外六段式流程-V1.79.svg)。

| 阶段 | 回答的问题 | 核心输出 |
|---|---|---|
| 多源线索接入 | 数据从哪里来，是否可靠进入系统 | 接入任务、幂等键、来源记录、原始快照 |
| 治理与线索建档 | 数据是否合格、重复或有效，是否建立可经营线索 | 线索编号、标准字段、主重复关系、有效性结果和异常记录 |
| 归因与负责人分配 | 线索归属哪个来源，应由谁接待 | 基础归因快照、匹配活码、排班规则、负责人/部门、权重与分配历史 |
| 触达与客户连接 | 线索能否被触达，是否完成加微和唯一客户关联 | 身份/解密状态、短信任务、加微事件、客户编号及服务关系 |
| 培育与业务转化 | 客户到达哪个培育或交易阶段，最终是否有效转化 | 问卷/测评、S/A/B/C等级、预约直播事件、订单支付退款事实和转化状态 |
| 分析与持续优化 | 哪个处理或经营环节存在损失，如何优化规则 | 系统监控、转化漏斗、归因经营指标、异常补偿结果和策略优化输入 |

> 六个阶段用于对外讲解，不表示后台任务必须严格串行。线索建档后，分配处理、身份/手机号处理和来源订单同步可并行；任一阶段失败均应形成可修复、可重试、可对账的异常任务。
...
flowchart LR
    ...
    ATTR1 --> QR["匹配活码<br/>期次+IP+店铺/渠道 → 通用活码"]
    QR --> K3{"K3 人工指定还是自动轮询"}
...
| 8 | 引流期次 | 期次ID、期次名称 | 引流期次、商品规则、订单或人工补充 | 保存期次归属快照；变更保留历史，不改接入快照 |
...
默认采用“线索接入时间队列口径”：选择某一时段进入系统的线索并持续追踪其后续结果。页面可切换“业务发生时间口径”查看特定时段实际发生的加微、预约、成交和退款，但两种口径不得混算。所有指标支持按来源、平台、店铺、商品、IP、IP渠道、IP大类、引流期次、组织和负责人下钻，并展示指标口径版本。

### 3.8 讲解口径

**对外讲解（约1分钟）**

合数BOSS先将API、Webhook、文件和人工导入的线索统一接入，通过校验、标准化、幂等去重和有效性判断建立线索主档；再完成平台、店铺、商品、IP和引流期次等基础归因，按活码排班及权重分配负责人。系统同时推进身份处理、短信触达、加微和唯一客户建档，并持续记录问卷测评、预约直播、下单支付及退款。最终通过处理效率、转化漏斗和归因经营分析定位损失，反向优化分配、触达和运营策略。
...
- 常用搜索展示综合关键词、当前待办状态、线索标记和转化状态；更多条件包含分配、解密、短信、加微、问卷、测评、跟进和订单等独立状态，以及来源、添加方式、负责人、所属期次、店铺和“日期类型 + 日期区间”；所属期次支持下拉选择并可输入名称搜索；
...
|---|---|
| 推荐触发 | 客户扫码、批量轮询任务 |
| 适用范围 | 活码、活码分组、引流期次、IP、渠道、店铺、组织或岗位 |
...
| IP编码 | 由用户在新增或编辑时录入，最长30个字符；去除首尾空格后按不区分大小写的口径全局唯一校验；在IP名称下方以次级编码展示，不单独占列 |
...
筛选支持IP名称、IP编码或创建人综合搜索，以及IP大类（支持按名称或大类编码检索）、IP渠道和状态。列表操作包括编辑、启用和停用。
...
- 新增和编辑必填IP编码、IP名称、IP大类、IP渠道；IP说明选填；
- IP编码由用户维护，不再由系统按流水号自动生成；创建和编辑保存时均须校验非空、长度与全局唯一性，重复时阻断保存并提示“IP编码已存在，请重新输入”；
...
- [ ] 新增和编辑均可维护IP编码；编码为空、超长或与其他IP重复时不得保存，商品和平台关系可下钻查看。
...
| 订单状态 | `order_status` | `PENDING_PAY` 待支付、`PAID` 已支付、`PREPARING` 备货中、`SHIPPED` 已发货、`CANCELLED` 已取消、`COMPLETED` 已完成、`AFTER_SALE_PROCESSING` 退款中、`REFUNDED_BEFORE_SHIP` 发货前退款完结、`REFUNDED_AFTER_SHIP` 发货后退款完结、`REFUNDED_AFTER_RECEIPT` 收货后退款完结、`PARTIAL_REFUNDED` 部分退款；无订单时为空 |
...
| 唯一正式课全额退款 | `conversion_status`、`order_status` | → 未转化，并按退款时履约快照更新为发货前/发货后/收货后退款完结 | 客户、旅程及历史状态记录 |
...
- [ ] 人工指定仅校验员工在职和账号启用，不受活码排班名单及接量权重影响；
...
- [ ] 无可用员工时线索保留在待分配队列；
- [ ] 新旧活码均能接收并关联扫码、加微结果；
- [ ] 员工停用后不再接待新客户；命中规则无可用员工时进入待配置异常；
...
- 新活码异常时停用新活码并恢复旧活码；当前排班规则无可用员工时进入待分配；
- 解密数据导入不创建记录；发现误导入时只能由受权人员按批次执行手机号字段修复并追加反向审计，不允许删除线索旅程或直接覆盖审计历史；
- 回滚不得删除线索、分配、企微事件、客户关联和审计历史。

## 12. 待确认事项

- **客户建档身份边界（推荐方案，待确认）：**引流线索进入后先保存线索及待识别经营关系，只有手机号或 UnionID 任一有效时才创建正式客户主档和客户编号；身份不足的数据不进入正式客户列表，补齐身份时再幂等创建或关联客户。
- **活码分配算法（推荐方案，待确认）：**自定义时段优先、全天规则兜底；时段采用左闭右开 `[开始, 结束)`；轮询游标按“活码 + 排班规则”保存，随机采用权重随机；同一扫码事件以企业微信事件ID幂等，重试保持首次选人结果。
- **无效/重复线索（推荐方案，待确认）：**无效线索限定为测试恶意、联系方式明确无效、业务明确排除三类；同一平台、店铺、外部业务单号重复接收按技术重复幂等，同一客户不同时间、渠道或期次的业务事件保留多条有效线索，不因手机号相同直接删除。
- 接量权重的默认值、允许范围及调整审批人；
- 分时段规则和全天在线兜底人员的业务配置负责人；
- 渠道首触、末触及辅助归因口径；
- 旧活码数据接口、字段和事件覆盖范围；
- 引流与三方品扩展字段最终清单。
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V1.79 | 2026-09-14 | 按现行线索中心逻辑重绘对外六段式流程；调整为多源接入、治理建档、归因与分配、触达与加微、培育与转化、分析与优化，移除班级口径并补充并行与异常补偿说明 |
| V1.78 | 2026-09-14 | 按9个实际子板块拆分线索中心专项需求文档并新增统一导航；总文档保留跨模块流程、状态、数据、接口、权限和发布规则 |
| V1.77 | 2026-09-10 | 商品短信配置补充用户下单后未加微补发：支付成功后3分钟、下单后8分钟分别检查；加微即取消剩余任务，同订单同节点幂等 |
| V1.76 | 2026-09-09 | 恢复并确认订单状态为11个内部映射枚举；`AFTER_SALE_PROCESSING` 页面名称由“售后处理中”统一调整为“退款中” |
| V1.75 | 2026-09-07 | 异常订单查询生成批次和明细，补同步从查询批次勾选订单并回写结果；确认活码列表无下载；记录三项推荐方案待确认 |
| V1.73 | 2026-09-04 | 补充活码详情字段实际说明、分配链路与算法建议、统计口径、配置变更/异常边界、原型差异及验收；已确认自定义时段无人可接待时自动转全天兜底；移除添加同一员工的开关、详情字段、历史优先分支及关联统计/审计项；其余新增策略待确认，未改原型 |
| V1.72 | 2026-09-04 | 活码统一多人接待，移除单人活码、备用员工、接量日期、二维码及详情下载、顶部权重/接量指标；接量上限改为接量权重，分时段规则与全天兜底均在规则内直接配置人员和权重 |
| V1.71 | 2026-09-04 | 引流线索“首单商品名称”更名为“商品名称”；移除所属直播组的筛选、字段、详情及单条/批量变更，变更期次仅处理所属期次 |
| V1.64 | 2026-09-02 | 订单状态收敛为六个实时第三方回传枚举；活码移除班级和使用明细，分组支持在更多中修改/删除，并补充自动通过好友、排班方式、全天兜底、分时段工作周期、备用员工及添加同一员工规则 |
| V1.63 | 2026-09-02 | 将第三方订单状态映射方案并入引流线索：统一11个内部订单状态，外部待付款/未确认统一为待支付，退款按履约快照区分发货前、发货后、收货后退款完结，新增退款处理中状态和部分退款；不设置状态异常，未知状态保留原值并进入异常中心 |
...
| V1.70 | 2026-09-03 | 引流线索短信群发改为必选订单日期，由系统按抖店及生效短信规则自动匹配签名和模板；活码移除复制短链入口 |
...
| V1.62 | 2026-09-02 | 活码管理新增企业微信主体与企微ID全局切换；统计、分组、列表、筛选和新建归属随主体刷新，既有活码禁止普通编辑跨主体迁移，详情、导出、事件和审计统一携带 `corp_id` |
| V1.64 | 2026-09-16 | IP编码改为用户在新增和编辑时维护，保存时校验必填、长度与全局唯一性，取消系统自增编号规则 |
| V1.65 | 2026-09-16 | 新增和编辑活码时关联IP改为必填；未选择时阻断保存，IP渠道仍由所选IP自动带出且只读 |
