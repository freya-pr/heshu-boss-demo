...
| 修改加微状态 | 引流线索且有加微状态修正权限 | 回显当前状态与加微时间；改为“已加企微/已加个微”时加微时间必填，改为“否”时清空当前加微时间；提交后刷新列表并追加旅程与审计 |
...
3. 当状态为“已加企微”或“已加个微”时，必须选择实际加微时间 `wechat_added_at`；可仅修正时间而不改变状态。状态和加微时间均未变化时不重复写入。
4. 改为“已加企微”时同步 `wechat_method=WECOM`；改为“已加个微”时同步 `wechat_method=PERSONAL_WECHAT`；改为“否”时清空当前加微方式和当前 `wechat_added_at`，但历史加微时间、UnionID、外部身份及既有旅程仍保留。
5. 每次有效变更必须在线索旅程和审计日志中追加修改前/后状态、修改前/后加微时间、操作人、员工编号、操作时间、来源 `MANUAL`、原因/备注及 traceId；历史记录只追加不可覆盖。
...
6. 与既有期次重名或时间完全重复的记录跳过并返回原因，已成功创建项不因其他项失败而回滚；
...
经营数据“过程转化”分组除加微率、问卷填写率外，新增 DAY1 到课率、DAY1 完课率、DAY2 到课率、DAY2 完课率、DAY3 到课率、DAY3 完课率，各指标同时展示对应人数并支持下钻。列表右上角提供表头设置，期次列固定且不可取消，其他过程转化、结果质量、经营效能和成交金额字段可自由勾选；设置即时生效、自动记忆并支持恢复默认。
