...
> **V1.74 当前生效补充：**引流线索批量菜单新增“查询异常订单、同步异常订单”两个独立入口；查询不触发同步，同步独立创建补同步演示任务。操作不依赖列表勾选结果，至少选择一个平台，按勾选顺序展示独立任务。当前仅为原型演示，不执行真实第三方查询或同步，结果必须明确标注演示状态，不能作为真实同步成功凭据。
...
| 查询异常订单 | 全局入口，无需勾选线索 | 至少选择一个查询平台，支持多选 | 按平台勾选顺序创建独立查询任务；查询不触发同步；当前原型展示查询演示任务结果，不执行真实第三方查询 |
| 同步异常订单 | 全局入口，无需勾选线索 | 至少选择一个同步平台，支持多选 | 独立发起异常订单补同步，按平台勾选顺序展示任务；当前原型展示同步演示任务结果，不执行真实第三方同步 |
...
分配、同步订单、短信群发、导入解密数据和导出非解密数据属于整体数据能力，只能从列表工具栏进入，不出现在单条线索“更多”中。“同步手机号”仍下线；“查询异常订单、同步异常订单”作为独立全局演示入口恢复，仅展示平台任务创建结果，不执行真实查询或同步。
