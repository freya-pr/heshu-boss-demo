...
> **V1.68 当前生效补充：**引流线索订单状态固定为未支付、已支付、已取消、已完成、退款中、已退款，均标注为第三方实时回传，本规则替代 V1.66。活码新增/编辑及列表移除班级和使用明细；自定义分组支持修改、删除并收入口“更多”，默认分组不可删除。活码支持自动通过好友、全天在线/分时段排班、轮询/顺序/随机、备用员工及优先添加同一员工；选择分时段排班时系统必须先生成一条覆盖周一至周日 00:00—23:59 的不可删除全天兜底规则，再允许新增自定义工作周期。后文与本补充冲突的旧表述均不再生效。
...
- 支持下载活码、查看详情、编辑和启停；列表“更多”仅保留启用/停用，不提供复制短链和使用明细；
