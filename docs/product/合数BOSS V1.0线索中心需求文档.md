...
> **V1.64 当前生效补充：**引流线索订单状态直接展示第三方平台实时回传结果，当前枚举为未支付、已支付、已取消、已完成、退款中、已退款；未产生订单时字段为空，不再使用“无订单”状态。商品短信配置的购买后、授权后和手动提醒均改为必选短信模板，模板来源于业务配置—短信管理的已审核启用模板，不再把触发动作或延迟时长作为模板选项。后文与本补充冲突的旧表述均不再生效。
...
| 订单 | 订单状态 | `order_status` | enum | 第三方平台实时回传：未支付、已支付、已取消、已完成、退款中、已退款；未产生订单时为空，不使用“无订单”枚举 |
...
渠道管理使用单一菜单入口，页面内部采用“渠道列表 / 数据分析”两个页签。渠道列表展示并维护已有渠道，不提供新增渠道入口；列表字段为渠道编码、渠道名称、渠道类型、状态和操作，操作保留详情、编辑、启用/停用。渠道类型固定为“平台渠道、IP渠道”，筛选支持渠道名称、编码、负责人或来源的综合搜索，以及渠道类型和状态。渠道编码作为稳定身份，名称或类型调整不得改写历史归因。
...
| 订单状态 | `order_status` | 第三方平台实时回传：`UNPAID` 未支付、`PAID` 已支付、`CANCELLED` 已取消、`COMPLETED` 已完成、`REFUNDING` 退款中、`REFUNDED` 已退款；无订单时为空 |
