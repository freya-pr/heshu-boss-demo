...
|---|---|---|---|---|
| 首页 | 线索概览 | 合数BOSS唯一默认首页与线索全链路营期经营台账 | 以营期为一行展示核心过程与结果指标；支持全部营期对比、营期行展开渠道/IP渠道/商品分析、指标明细下钻、当前列表导出和单营期完整报表导出 | V1.0 |
| 线索中心 | 引流线索 | 引流业务线索列表及详情 | 详情、旅程、是否挖需单向勾选、变更手机号、无效/重复标记、人工/批量分配、改派、短信提醒/群发、无需勾选线索并按平台勾选顺序同步订单、导入解密数据、导出非解密数据；不提供编辑线索、添加跟进和变更线索等级 | V1.0 |
...
| 区域 | 主要内容 |
|---|---|
| 全局筛选 | 数据权限范围、营期、日期区间、来源/平台、店铺、IP渠道、IP名称、组织和负责人；营期默认“全部营期”，查询条件对列表、展开区、下钻和导出统一生效 |
| 营期经营列表 | 一行对应一个营期，展示名称/编号/日期/状态、有效线索、加微及比率、问卷及比率、预约/到课/完课、成交/退款及最终转化率、毛/净GMV和人服比 |
| 营期展开分析 | 点击行首展开按钮，在行下以“渠道分析 / IP渠道分析 / 商品分析”页签查看该营期归因数据；同一时间仅展开一个营期 |
| 指标下钻 | 点击列表蓝色人数打开该营期明细抽屉，展示订单、客户、来源、店铺、商品、IP归因、负责人、事件时间和当前节点 |
| 导出 | 页面级导出当前营期对比列表；行级导出该营期经营概览、渠道、IP渠道和商品四个数据区；明细抽屉可独立导出 |

**关键规则**

- 数据范围与当前用户的数据岗位、组织关系和特别授权一致，页面能力与业务角色权限一致；
- 指标必须展示统计周期、口径版本和更新时间，不得将不同口径数据直接相加；
- 点击营期列表中的蓝色人数或展开分析中的指标数字，打开继承营期、筛选与权限快照的明细抽屉；
- “导出当前列表”必须与当前筛选后的营期行一致；行级导出必须包含该营期经营概览、渠道分析、IP渠道分析和商品分析四个数据区；
- 毛GMV、退款金额和净GMV命名及公式必须固定，禁止出现两个同名GMV；
- 加载失败时保留最近一次成功数据并标记更新时间，不将失败展示为 0。

**V1.0 完成标准**

全局筛选、营期经营列表、营期展开分析、指标下钻和分层导出可用；主管与员工看到的数据范围正确，列表汇总、下钻明细和导出在同一口径下能够核对。完整字段、公式和交互见线索中心详细需求5.7节。
...
| 首页—线索概览 | 按营期经营列表对比线索—加微—问卷—预约—到课—完课—成交及退款、GMV、人服比；展开行查看渠道、IP渠道和商品分析；支持指标下钻和分层导出 |
...
7. 在“首页—线索概览”按营期逐行展示线索到成交的关键过程与经营结果；支持店铺、IP渠道及IP名称筛选，营期行可展开渠道、IP渠道和商品分析，关键数量可下钻，并可导出当前对比列表或单营期完整经营报表，完整口径见线索中心详细需求5.7节。
...
10. 行操作固定展示“详情、旅程、更多”；建立客户档案、变更线索标记、改派、短信提醒和备注等单条动作收进更多；不提供编辑线索、添加跟进和变更线索等级，取消人工“确认加微”，加微结果由企业微信事件或数据同步回写。
