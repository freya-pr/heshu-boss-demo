...
> **V1.78 当前生效补充：**引流线索订单状态固定为未支付、已支付、已取消、已完成、退款中、已退款，标注为第三方实时回传，本规则替代 V1.76。活码移除班级和使用明细，分组支持修改/删除并收入口“更多”；新增/编辑支持自动通过好友、全天在线/分时段排班、轮询/顺序/随机、备用员工和优先添加同一员工，分时段排班必须先生成不可删除的全天兜底规则。标签管理仅保留标签库和标签组；标签库支持新增标签、同步企微，名称全局不可重复，标签组非必填，列表不展示对象和分类；标签组支持新增/编辑且不含对象、维护组织和排序。后文与本补充冲突的旧表述均不再生效。
...
| 线索中心 | 活码管理 | 按企业微信主体隔离的新旧员工活码、分组、IP归因、标签及接待配置 | 切换有权主体及企微ID；分组修改/删除；新增、编辑、启停、下载、关联IP、标签、全天/分时段排班、全天兜底、备用员工及添加同一员工 | V1.0 |
