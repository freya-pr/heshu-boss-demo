...
flowchart LR
    ...
    subgraph SUPPORTBOX["业务域八｜平台支撑中心｜阶段一重点"]
        ...
        AUDIT_BASE["审计管理｜异常中心 · V1.0<br/>发送结果 · V1.0"]
...
| 审计管理 | 异常中心 | 企业微信及同步异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 审计管理 | 发送结果 | 供应商发送回执 | 结果、错误码、计费条数、补发节点和关联任务 | V1.0 |
...
系统管理、业务配置和审计管理均属于合数BOSS应用内部，不是合数统一管理平台外层的公共系统管理。统一平台负责统一登录、应用入口和会话；合数BOSS“系统管理”负责组织员工映射、岗位数据范围、角色功能权限、菜单、日志和地区等平台治理能力；“业务配置”负责短信、企微、小程序、落地页和微信客服等业务接入能力；“审计管理”负责异常中心和短信发送结果。业务配置展示在系统管理上方，审计管理展示在系统管理下方，当前版本不包含支付管理和群机器人。
...
产品责任人为张铭钰；技术责任人为陈庆焕并指定企微接口开发人。企微基线上线前执行一次全量导入；后续使用实时增量下发、企微回调和每日全量差异扫描。正常回调显示在员工管理—同步记录；字段冲突、重复绑定或连续失败进入审计管理—异常中心。旧差异未处理时不会仅因其存在而重复收到同一回调，因此夜间差异扫描是必须的兜底机制。
...
复用历史短信服务，提供运营商管理、签名管理、短信模板和短信规则。模板、签名启停均受权限控制；发送结果迁移至“审计管理—发送结果”，保存供应商流水、状态、错误码、重试次数和关联业务对象。
