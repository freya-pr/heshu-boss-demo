...
> **V1.86 当前生效补充：**以“查询异常订单、同步异常订单”两个独立入口及“活码列表无下载”为现行规则。查询生成唯一批次并展示异常订单清单；同步从有效查询批次勾选待同步/失败订单，逐单生成补同步任务并回写结果，重复请求必须幂等。员工编号统一显示为 `B` + 5 位。菜单配置不得展示已删除的商品分析、活码班级/下载、渠道新增/详情/编辑、店铺负责人/权限、IP关联商品/平台及落地页新增能力。标签只保留只读标签库、企微同步和标签组维护，不建设已删除的标签规则、映射及治理能力。
...
| V1.86 | 2026-09-07 | 确认异常订单查询批次—清单—勾选—补同步—结果回写闭环；员工编号统一B+5位；清理菜单及标签已删除范围；记录三项推荐方案待确认 |
