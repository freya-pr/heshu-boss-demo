...
> **V1.93 当前生效补充：**客户中心模块名称统一为“企微标签”，按当前企微企业标签原型管理标签组与标签，不再展示系统标签来源或企微个人标签；支持分组搜索、新增、编辑、空分组单条/批量删除、适用部门多选，以及标签全局搜索、单条/批量新增、编辑、删除、使用客户下钻和同步企微标签。员工编号改为管理员录入的 6 位数字并全局查重；登录账号同样全局查重并直接作为企微 `UserID`。引流线索保留“查询异常订单、同步异常订单”两个独立入口。后文与本补充冲突的只读标签、`B + 5位` 自动编号、后端生成企微 UserID 和移除异常订单查询入口等描述均不再生效。

> **V1.94 当前生效补充：**审计管理菜单统一调整为“业务异常、企微异常、短信日志”，不再使用“异常中心、发送结果”作为当前菜单名。微信客服管理的“新增客服”放置到当前企微主体摘要区，移除页面级“同步企微客服”按钮。详细规则以系统管理与业务配置需求文档 V2.13 为准。

> **V1.92 当前生效补充：**IP编码改为新增、编辑时人工必填并按不区分大小写规则校验全局唯一，IP页面不再展示大类编码；新增、编辑活码时关联IP均为必填。业务配置重组为短信管理和企微管理：短信管理新增运营商管理，发送结果迁移至独立“审计管理”；配置管理更名企微管理并纳入微信客服管理、移除店铺配置；应用管理不再作为侧栏菜单，改由企微配置操作列“应用”进入。详细规则分别以线索中心需求文档 V1.78、系统管理与业务配置需求文档 V2.11 为准。

> **V1.89 当前生效补充：**新增“个人中心”一级菜单并置于“业务配置”上方，包含“用户信息、我的下载”。用户信息提供基本设置、密码设置和外部绑定；原“企微绑定”更名为“外部绑定”，一个登录账号允许绑定多个企业微信身份，同一 `CorpID + UserID` 仍只能绑定一个登录账号。我的下载按服务人、部门、申请人/文件名称、申请时间和生成状态查询；列表补充申请人及所属部门，操作列固定右侧；状态仅为已生成、生成中、生成失败，不包含已过期。详细规则以[个人中心需求文档](./合数BOSS%20V1.0个人中心需求文档.md)为准。

> **V1.90 当前生效补充：**业务配置—短信管理的“用户下单后”规则固定补充未加微补发逻辑：支付成功后 3 分钟检查，未加微则补发；下单后 8 分钟再次检查，仍未加微则再补发。任一时点已加微即取消剩余补发，同一订单同一节点必须幂等。发送结果区分首次发送与两个补发节点。详细规则以系统管理与业务配置需求文档 V2.10 为准。

> **V1.91 当前生效补充：**个人中心—我的下载点击“记录”后，以列表展示下载时间、文件内容名称、下载人姓名和状态；不再展示申请任务详情，也不展示下载IP地址。无历史记录时显示“暂无下载记录”。详细规则以个人中心需求文档 V1.1 为准。

> **V1.88 当前生效补充：**引流线索订单状态使用11个内部映射枚举，其中 `AFTER_SALE_PROCESSING` 页面显示由“售后处理中”调整为“退款中”；内部编码、业务定义和第三方映射逻辑不变。详细状态及验收以线索中心需求文档 V1.76 为准。

> **V1.87 历史补充（标签交互由 V1.93 替代）：**曾按通用标签管理结构补充标签ID、状态和同步规则；当前企微标签页面及交互以 V1.93 和客户中心标签管理专项需求为准。

> **V1.86 历史补充（员工编号由 V1.93 替代）：**查询异常订单、同步异常订单两个独立入口及“活码列表无下载”继续生效；本版本曾规定员工编号为 `B + 5位`，现已由 V1.93 的手工6位数字规则替代。

> **V1.84 评审补充：**活码详情字段含义、主体/活码匹配—排班—人员资格—选人—幂等提交—加微归因链路，统一见 [线索中心需求文档](./合数BOSS%20V1.0线索中心需求文档.md) 5.6.3—5.6.8（V1.73）。2026-09-04 已确认取消“添加同一员工”的配置与优先分配逻辑，新分配请求统一按当前排班及分配方式选人，同一请求重试仍保持幂等。自定义时段内无人可接待时自动转给全天兜底人员，兜底仍无人可用则保留待分配并生成异常。其余多码冲突、兜底边界、时段边界、顺序/随机算法、权重作用域及统计口径仍待确认，不视为已实现。本轮仅补充需求说明，未修改原型及分享版。

> **V1.83 历史补充（活码操作范围由 V1.86 替代）：**活码统一按多人接待配置，不展示单人活码类型；移除备用员工、接量日期区间、列表及详情二维码、详情下载入口和顶部“轮询权重/当前接量”指标。新增/编辑将“排班设置”移至“接待员工”之前；全天在线单独配置接待员工，分时段排班则先配置自定义规则人员、再配置不可删除的全天在线兜底人员，每条规则内直接选择员工并设置接量权重、只读展示当前接量。本版本的列表下载规则已被 V1.86 替代。
...
> **V1.81 历史补充（标签部分由 V1.93 替代）：**短信规则、短信群发自动匹配、岗位数据范围和活码移除复制短链继续生效；本版本的系统/企业微信两类只读标签方案已由 V1.93 的企微标签管理方案替代。
...
> **V1.73 历史补充（标签部分由 V1.81 替代，活码人员与排班规则由 V1.83 替代）：**引流线索订单状态固定为未支付、已支付、已取消、已完成、退款中、已退款，标注为第三方实时回传。渠道管理不提供新增入口，渠道类型仅为平台渠道、IP渠道，操作仅保留启用/停用。活码移除班级和使用明细，分组支持修改/删除并收入口“更多”；本版本曾包含备用员工等旧方案，现行活码规则以 V1.83 为准。
...
> **V1.65 历史补充（标签管理部分由 V1.93 替代）：**企微客户全量/增量同步、客户档案结构及修改加微状态的时间与审计要求继续生效；标签管理现行规则以 V1.93 为准。
...
|---|---|---|
| 组织、员工、岗位及角色 | 整改后的企业微信基线；合数BOSS正式主数据 | 企微基线经校验确认后建档；合数BOSS生成员工编号并用于权限、负责人、活码及继承；一个账号可绑定多个企微身份，同一企微身份只能绑定一个账号；岗位配置数据范围，角色配置功能权限 |
...
flowchart LR
    ...
    subgraph CUSTOMERBOX["业务域二｜线索与客户中心｜阶段一重点"]
        direction TB
        HOME["首页｜线索概览 · V1.0"]
        LEAD["线索中心｜引流线索 · V1.0<br/>三方品线索 · V1.5<br/>线索配置 · V1.0<br/>活码管理 · V1.0<br/>渠道管理（列表/分析）· V1.0<br/>店铺管理 · V1.0<br/>IP管理 · V1.0<br/>商品列表 · V1.0<br/>短信配置 · V1.0"]
        CUSTOMER["客户中心｜客户总览 · V1.0<br/>客户列表 · V1.0<br/>撞单管理 · V1.0<br/>企微标签 · V1.0"]
    ...
    subgraph AFTERBOX["业务域七｜售后客服中心｜V1.0 + V2.0"]
        ...
        WECHAT["业务配置｜企微管理 / 微信客服管理 · V1.0"]
    end

    subgraph SUPPORTBOX["业务域八｜平台支撑中心｜阶段一重点"]
        direction TB
        PERSONAL_BASE["个人中心｜用户信息 · V1.0<br/>我的下载 · V1.0"]
        BUSINESS_BASE["业务配置｜短信管理 · V1.0<br/>企微管理 · V1.0<br/>百家号配置 · V2.0"]
        AUDIT_BASE["审计管理｜业务异常 · V1.0<br/>企微异常 · V1.0<br/>短信日志 · V1.0"]
    ...
    class SYS_ORG,HOME,LEAD,CUSTOMER,QUESTION,WECHAT,PERSONAL_BASE,BUSINESS_BASE,SYS_BASE,AUDIT_BASE phase1;
...
| 线索中心 | 活码管理 | 按企业微信主体管理新旧员工活码、分组、IP归因、标签及接待配置 | 企微ID全局切换、分组修改/删除、新增、编辑、启停、必填关联IP、标签、全天/分时段排班、规则内人员及接量权重、全天兜底 | V1.0 |
| 线索中心 | 渠道管理 | 通过渠道列表/数据分析页签统一展示渠道基础信息及转化效果 | 不提供新增；渠道类型仅平台渠道/IP渠道；列表操作仅启用/停用，分析保留筛选、漏斗、下钻及导出 | V1.0 |
| 线索中心 | 店铺管理 | 外部平台店铺身份及线索归因 | 新增补充店铺ID、KEY、秘钥、短信账号和备注；移除负责人、权限范围、独立详情及业务发生口径；期次统一为全部期次 | V1.0 |
| 线索中心 | IP管理 | 以老师名称维护IP编码、IP大类、IP渠道和IP名称关系 | IP编码人工必填并全局唯一；页面不展示大类编码；新增、编辑、启停 | V1.0 |
| 线索中心 | 商品列表 | 统一承接第三方商品与店铺、活码关系 | 创建/更新时间、商品名称、商品ID、商品状态、是否分配活码、店铺名称/类型、同步、导出和备注；移除关联IP | V1.0 |
| 线索中心 | 短信配置 | 按商品与活码配置业务短信提醒策略 | 短信分组、商品/活码绑定、购买后提醒、授权后提醒、手动提醒、批量设置、新增、编辑、启停；发送结果进入审计管理 | V1.0 |
...
| 客户中心 | 企微标签 | 按企业微信标签组维护企微企业标签并与当前企业主体同步 | 分组搜索、新增、编辑、空分组单条/批量删除、适用部门多选；标签全局搜索、单条/批量新增、编辑、删除、使用客户下钻和同步企微标签 | V1.0 |
| 问卷管理 | 问卷列表 | 问卷模板、题目结构、投放链接与答卷统一管理 | 名称/状态筛选、新增编辑、题目分组、五类题型、企微标签、二维码与双链接、答卷统计、模板导入、权限导出、线索/客户关联与SABC评级输入 | V1.0 |
| 个人中心 | 用户信息 | 当前登录用户的个人资料与账号安全设置 | 个人资料、基本设置、密码设置、外部绑定；账号可绑定多个企业微信身份 | V1.0 |
| 个人中心 | 我的下载 | 查询导出任务、生成进度和下载记录 | 服务人、部门、申请人/文件名称、申请时间及状态筛选；下载、记录和刷新 | V1.0 |
...
| 审计管理 | 业务异常 | 数据同步与核心业务异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 审计管理 | 企微异常 | 企微接口、授权、回调与同步异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 审计管理 | 短信日志 | 供应商发送回执 | 结果、错误码、计费条数、补发节点和关联任务 | V1.0 |
| 业务配置 / 短信管理 | 运营商管理 | 短信运营商及服务商配置 | 新增、编辑、启停、接入信息和优先级 | V1.0 |
...
| 业务配置 / 企微管理 | 企微配置 | 企业微信接入配置 | 校验、发布、凭证轮换、版本回退；操作列“应用”进入应用管理 | V1.0 |
| 业务配置 / 企微管理 | 小程序配置 | 小程序接入配置 | 校验、发布、凭证轮换和版本回退 | V1.0 |
| 业务配置 / 企微管理 | 落地页配置 | 落地页地址、跳转及兜底配置 | 筛选、手机端预览、编辑、链接校验、启停及停用后删除 | V1.0 |
| 业务配置 / 企微管理 | 微信客服管理 | 企业微信客服接入 | 客服列表、客服事件消息、客服消息 | V1.0 |
...
| 个人中心 | [个人中心详细需求](./合数BOSS V1.0个人中心需求文档.md) |
...
| 页面 | 主要职责 |
|---|---|
| 引流线索 | 展示公共线索字段及投放、素材、落地页等引流扩展字段；IP大类默认展示并可在字段设置中隐藏；线索标签最多展示2个及“+N”，支持分源查看全部和添加BOSS人工标签 |
| 三方品线索（V1.5） | 展示公共线索字段及合作方、产品、批次等三方品扩展字段 |
| 线索配置 | 分类管理活码分配和营期名单流转规则；统一保存触发、条件、动作、适用范围、优先级、冲突策略、试算、执行快照和规则版本 |
| 活码管理 | 顶部按授权企业微信主体及企微ID切换；统计、分组、列表和新建归属随主体刷新；新旧活码、活码分组、关联IP、自动带出只读IP渠道、排班规则、接待员工、接量权重、企微标签、内部标签、创建人和期次均按 `corp_id` 隔离 |
| 渠道管理 | 渠道列表维护渠道定义与来源映射；数据分析页签展示线索到客户及成交转化效果 |
...
11. 列表上方批量操作承载分配、变更期次、同步订单、查询异常订单、同步异常订单、短信群发、导入解密数据和导出非解密数据；其中分配、变更期次和短信群发需要先勾选线索，同步订单与查询异常订单属于全局后台任务；查询异常订单至少选择一个平台并生成唯一查询批次，同步异常订单只处理有效批次中待同步或同步失败的订单并逐单回写结果；短信群发按订单日期、关联抖店和当前生效短信规则自动匹配签名与模板；导入只按订单编号唯一命中并覆盖既有线索手机号，导出只输出当前查询结果中未解密且有订单编号的数据；页面不提供同步手机号，单条操作不提供语音提醒。
...
| 客户列表 | 固定客户编号、客户名称和操作列；生命周期仅为线索/意向/成交；支持客户添加时间、标签、手机号预览/变更；基本档案展示客户地址且不展示企微客户ID/UnionID |
| 撞单管理（V1.0） | 手机号与 UnionID 分别命中不同客户时不阻断经营档案与业务记录，只冻结冲突身份覆盖/自动合并并自动建案；支持证据核验、裁决和审计 |
| 企微标签 | 仅展示企微企业标签，不展示系统标签来源和企微个人标签；支持标签组搜索、新增、编辑、空分组单条/批量删除及适用部门多选；支持标签全局搜索、单条/批量新增、编辑、删除、使用客户下钻和同步企微标签 |
...
手机号/UnionID 唯一客户逻辑、客户档案、撞单管理、标签和等级均可形成闭环；客户中心无“跟进记录”“等级管理”“公海客户”和“商机管理”二级菜单，等级可从客户列表编辑并完整留痕；手机号与 UnionID 跨档案冲突可完成阻断、建案、领取、证据核验、人工裁决、审计和反向恢复。
...
系统管理、业务配置和审计管理均属于合数BOSS应用内部，不是合数统一管理平台外层的公共系统管理。统一平台负责统一登录、应用入口和会话；合数BOSS“系统管理”负责组织员工映射、岗位数据范围、角色功能权限、菜单、日志和地区等平台治理能力；“业务配置”负责短信、企微、小程序、落地页和微信客服等业务接入能力；“审计管理”负责业务异常、企微异常和短信日志。业务配置展示在系统管理上方，审计管理展示在系统管理下方，当前版本不包含支付管理和群机器人。
...
手工新增员工时，员工编号、员工名称、真实姓名、手机号、主组织、一个数据岗位和至少一个业务角色必填，邮箱选填；登录账号默认同步创建，也可仅创建员工主档。员工编号由管理员录入恰好6位数字，保存前执行全局查重，保存后不可修改或复用；创建登录账号时同样执行全局查重。手机号在有效员工范围内全局唯一，邮箱填写时校验唯一性；主组织只能选择启用的部门或小组，公司不可直接作为员工主组织。员工主档、主组织关系、岗位关系、角色关系和账号必须事务保存，失败整体回滚。新增弹窗内提供“同时创建或绑定企微成员”开关：系统以登录账号作为企微 `UserID` 查重，已有且未绑定则建立绑定，不存在才创建并邀请。员工主档先提交，企微任务异步执行；企微失败不得删除已创建员工，应进入同步记录并支持重试，冲突或连续失败进入异常中心。历史未绑定员工使用列表行操作“补开企微”。创建成功与激活成功必须分开显示；一个账号可绑定多个企微身份，同一企微身份只归属一个账号，重复请求不得重复创建；新增员工不自动承接客户、线索、订单或活码。详细字段、异常提示、安全要求和验收标准见《[合数BOSS系统管理1.0 PRD](./合数BOSS V1.0系统管理与业务配置需求文档.md)》5.2.0.1—5.2.0.2 节。
...
数据岗位统一使用五级范围：本人数据、本人负责小组、本人负责部门及下级、当前公司全部和自定义组织。本人数据岗只查看本人负责或明确协作的数据；小组管理岗查看员工被登记为负责人的全部小组；部门管理岗查看负责部门的直属员工及全部下属小组；公司管理者和中台业务人员必须绑定“当前公司全部”岗位或获得有期限的特别授权后才能查看全公司数据。系统管理员角色默认只有系统配置功能，不自动拥有全部线索、客户和订单数据。
...
筛选只能缩小当前员工由数据岗位及特别授权计算出的有效范围，不能扩大权限。本人数据岗仅返回本人数据；小组管理岗返回所负责小组；部门管理岗返回部门直属员工和下属小组；公司数据管理岗与获批中台特别授权可在当前公司内选择任意组织。所有组织、负责人候选项、列表数据、汇总数量、统计指标和导出结果均由服务端数据权限能力裁剪，前端隐藏不作为权限依据。
...
2. 合数BOSS员工编码 `employee_no`：由管理员录入恰好6位数字，全局唯一，保存后不可修改或复用；
3. 外部身份：企微 `CorpID + UserID` 作为在线外部身份，单独保存来源租户、状态及生效时间；旧SCRM员工ID可作为只读历史别名。

历史员工与重新入职统一遵循“一个自然人、一个员工主档、一个永久员工编号”原则：企微员工基线先进入暂存区，经身份匹配和冲突处理后，由管理员补齐或确认6位正式员工编号；旧SCRM员工ID和历史企微UserID只作为别名或外部身份保存。同一员工重新入职时恢复原 `employee_id/employee_no` 并新增任职记录，不创建新员工编号；原角色、岗位、客户、活码和待办不自动恢复，必须按新任职重新授权或分配。无法确认是否同一自然人时必须进入人工冲突处理队列。
...
- 一个合数BOSS登录账号可绑定多个生效企微身份，同一 `CorpID + UserID` 只能绑定一个登录账号；解绑或跨账号迁移先归档旧关系再变更，业务归属按 `employee_id` 聚合，历史操作保留当时实际企微身份。
...
flowchart LR
    ...
    ACCOUNT --> BINDING["外部身份绑定<br/>账号 → 多个 CorpID + UserID"]
    SWITCH{"自有系统主控开关"}
    SWITCH -->|"开启"| OWN
...
产品责任人为张铭钰；技术责任人为陈庆焕并指定企微接口开发人。企微基线上线前执行一次全量导入；后续使用实时增量下发、企微回调和每日全量差异扫描。正常回调显示在员工管理—同步记录；字段冲突和重复绑定进入审计管理—业务异常，企微接口、回调或通讯录同步连续失败进入审计管理—企微异常。旧差异未处理时不会仅因其存在而重复收到同一回调，因此夜间差异扫描是必须的兜底机制。
...
- 数据范围统一支持本人、本人负责小组、本人负责部门及下级、当前公司全部和自定义组织；
...
菜单归属：`审计管理 / 业务异常`与`审计管理 / 企微异常`；同级第三个菜单为“短信日志”。

业务异常展示数据同步、员工身份冲突、分配执行、活码、客户身份、问卷关联、第三方同步、订单和退款异常；企微异常展示企微授权、接口、回调和通讯录同步异常。两页均支持查询、详情、重试、忽略、转人工、补偿和关闭；关闭必须填写原因，所有处理保留审计。
...
复用历史短信服务，提供运营商管理、签名管理、短信模板和短信规则。模板、签名启停均受权限控制；短信日志进入“审计管理—短信日志”，保存供应商流水、状态、错误码、重试次数和关联业务对象。

左侧导航在短信管理下设置“运营商管理、签名管理、短信模板、短信规则”四个三级菜单，分别使用独立路由、查询、列表和操作。短信规则新增/编辑必须填写规则名称、类型、关联抖店、签名和模板；类型固定为用户下单后、用户提醒、用户授权后。同一抖店与类型只允许一条生效规则，变更全量审计。选择“用户下单后”时固定展示补发策略：支付成功后 3 分钟和下单后 8 分钟分别检查加微状态，仅未加微时发送；加微后取消剩余任务，同一订单同一节点不得重复提交。

##### J. 企微管理与应用入口

菜单归属：`业务配置 / 企微管理`。

- 企微管理下设置企微配置、小程序配置、落地页配置和微信客服管理，不设置店铺配置；
- 应用管理不设置独立侧栏菜单；企微配置列表操作列提供“应用”入口，点击后进入标题为“应用管理”的页面，支持外部应用新增、编辑、启停、连接校验、凭证轮换及调用范围；
- 企微、小程序和落地页达到“可配置、可校验、可发布、可回退”；
- 落地页配置使用独立页面标题；列表展示落地页编码、落地页名称、H5地址、唯一跳转方式、兜底方式优先级、状态及创建时间。主跳转方式仅可选择获客助手、小程序或微信客服中的一个，兜底方式按优先级依次尝试；支持手机端预览、编辑、启停和停用后删除，不提供新增落地页入口；
- 百家号配置属于 V2.0，不作为 V1.0 门禁；
- 配置发布形成版本，失败时可回退到上一有效版本。

##### K. 微信客服管理（P1）

菜单归属：`业务配置 / 企微管理 / 微信客服管理`。

页面顶部提供企微ID切换；切换后，客服分组、客服列表、客服事件消息、客服消息、统计及新建客服均按所选 `corp_id` 刷新和归属，不得混合不同企微主体数据。复用企业微信客服能力；V1.0 重点是账号与事件接入、关联业务对象、状态查询和异常定位，不强制建设完整会话内容归档。

“新增客服”放置在当前企微主体摘要区，待续期或失效主体下禁用；页面不设独立“同步企微客服”按钮，数据同步由接口、回调和后台任务承接。
...
3. 一个企微身份只绑定一个合数BOSS登录账号，一个登录账号允许保留多个生效企微绑定；
...
个人中心位于业务配置上方，包含用户信息和我的下载。右上角用户名区域提供个人中心快捷入口并直接进入用户信息页。

- 用户信息：左侧只读展示当前用户、角色、组织、岗位及登录信息；右侧提供基本设置、密码设置和外部绑定。一个账号可绑定多个企业微信身份，同一 `CorpID + UserID` 仍全局唯一，解绑保留历史审计。
- 我的下载：支持服务人、部门、申请人/文件名称、申请时间快捷区间/自定义区间及生成状态组合查询；状态仅为已生成、生成中、生成失败。列表展示申请人、所属部门、申请时间、文件内容名称、下载次数和状态，下载/记录操作列固定在最右侧；下载记录弹窗按时间倒序展示下载时间、文件内容名称、下载人姓名和状态，不展示下载IP地址。
- 文件生成说明不承诺通过企业微信通知；用户通过刷新主动查询最新状态。只有已生成任务允许下载，所有查看及下载均受当前账号数据权限约束。

完整页面、字段、权限、异常、验收及回滚规则见《[合数BOSS V1.0个人中心需求文档](./合数BOSS%20V1.0个人中心需求文档.md)》。
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V1.94 | 2026-09-24 | 审计管理拆分业务异常、企微异常和短信日志；微信客服新增入口移至当前企微主体摘要区并移除页面同步按钮 |
| V1.93 | 2026-09-22 | 企微标签按当前原型统一分组、适用部门、标签增删改、全局搜索、客户下钻及同步交互；员工编号改为手工6位数字并查重，账号作为企微UserID；补齐异常订单查询与同步入口及整体流程口径 |
| V1.92 | 2026-09-17 | 同步IP编码人工必填且唯一、活码必选IP；重组业务配置/审计管理菜单，新增运营商管理，迁移发送结果，配置管理更名企微管理，移除店铺配置和独立应用菜单，并新增企微配置“应用”入口 |
| V1.91 | 2026-09-10 | 个人中心下载记录改为历史列表，展示下载时间、文件名称、下载人和状态；移除下载IP地址并补充无记录空状态 |
| V1.90 | 2026-09-10 | 下单短信新增未加微补发策略：支付成功后3分钟、下单后8分钟分别检查并补发；加微即取消后续任务，同订单同节点幂等，发送结果区分补发节点 |
| V1.89 | 2026-09-10 | 新增个人中心一级菜单及专项需求；用户信息包含基本设置、密码设置和外部绑定，账号侧支持多个企业微信身份；我的下载补充服务人/部门/时间/状态筛选、申请人及所属部门、右侧固定操作列，并删除已过期状态与企微通知承诺 |
| V1.88 | 2026-09-09 | 引流线索订单状态保持11个内部映射枚举，`AFTER_SALE_PROCESSING` 页面名称统一调整为“退款中” |
| V1.87 | 2026-09-09 | 标签管理按通用PRD需求说明结构补全入口、筛选、字段、操作、编码、权限与异常逻辑；新增标签ID生成及幂等规则；状态移除“已同步”；移除标签日志及同步历史展示要求 |
| V1.86 | 2026-09-07 | 确认异常订单查询批次—清单—勾选—补同步—结果回写闭环；员工编号统一B+5位；清理菜单和标签已删除范围；记录三项推荐方案待确认 |
| V1.84 | 2026-09-04 | 引用线索中心V1.73活码详情与分配逻辑评审补充；确认自定义时段无人可接待时自动转全天兜底；取消添加同一员工的配置与优先分配逻辑；其余算法与原型差异保留待确认，不新增菜单或修改原型 |
...
| V1.81 | 2026-09-03 | 新增短信规则并调整短信群发为订单日期自动匹配；标签管理统一两类来源只读并修正标签组引用、过滤和持久化逻辑；同步岗位范围和活码操作最终口径 |
| V1.73 | 2026-09-02 | 同步订单六状态、渠道只读启停、活码分组及排班兜底、标签库与标签组最终精简规则 |
| V1.72 | 2026-09-02 | 引流线索统一第三方订单状态映射：待付款/未确认归入待支付，补充备货、发货、售后处理中、三阶段退款完结及部分退款；不设置状态异常，详细规则并入线索中心需求文档 |
...
| V1.34 | 2026-08-19 | 历史版本曾允许同一员工保留多个企微身份；该规则曾由 V1.40 的双向一对一规则替代，当前又由 V1.89 的账号一对多、外部身份全局唯一规则替代 |
...
| V1.40 | 2026-08-21 | 历史版本采用账号与企微身份双向一对一规则；该账号侧唯一限制已由 V1.89 替代，外部身份跨账号唯一及完整历史继续有效 |
...
| V1.54 | 2026-09-01 | 落地页配置取消新增入口；新增落地页编码、H5地址、唯一跳转方式及兜底优先级展示，支持手机端预览、编辑、启停和停用后删除 |
| V1.55 | 2026-09-01 | 落地页主跳转方式与兜底方式中的“微信客户”统一更正为“微信客服” |
| V1.71 | 2026-09-02 | 活码管理新增授权企业微信主体及企微ID切换；统计、分组、列表、筛选、新建归属、详情、导出和事件统一按 `corp_id` 隔离，既有活码禁止普通编辑跨主体迁移 |
