...
flowchart LR
    ...
    H -- "未命中" --> I["创建唯一客户编码"]
    H -- "命中同一客户" --> J["关联并补充身份"]
    H -- "命中不同客户" --> K["阻断自动建档并生成撞单案件<br/>进入客户中心—撞单管理"]
    I --> L["客户档案与负责人"]
    J --> L
    L --> M["问卷发送、回收与SABC定级"]
    M --> N["测评预约与完成"]
...
| 线索中心 | 引流线索 | 引流业务线索列表及详情 | 详情、旅程、是否挖需单向勾选、变更手机号、修改加微状态（否/已加企微/已加个微）、无效/重复标记、人工/批量分配、改派、短信提醒/群发、无需勾选线索并按平台勾选顺序同步订单、导入解密数据、导出非解密数据；不提供编辑线索、添加跟进和变更线索等级 | V1.0 |
...
| 线索中心 | 渠道管理 | 通过渠道列表/数据分析页签统一管理渠道基础信息、归因规则及转化效果 | 列表、新增、编辑、详情、启停、渠道/时间/组织筛选、漏斗、下钻及导出 | V1.0 |
| 线索中心 | 店铺管理 | 外部平台店铺身份、负责人、访问范围及经营归因 | 基础档案、新增、编辑、启停、权限、创建/更新/启停日期筛选、线索归因/业务发生口径、业务日期、线索/加微、订单/支付、净GMV、经营详情、归因异常 | V1.0 |
| 线索中心 | IP列表 | 以老师名称维护IP主档、页内IP配置、IP大类、稳定编码、归因渠道及商品/平台关系 | IP名称下展示自增IP编号、IP大类下展示大类编码、IP配置绑定名称/大类/渠道、商品下钻、平台折叠、新增、编辑、启停 | V1.0 |
| 线索中心 | 商品列表 | 统一承接第三方商品与店铺、活码和IP关联 | 创建/更新时间、商品名称、商品ID、商品状态、是否分配活码、店铺名称/类型、关联IP、同步所有商品、分平台顺序同步、同步任务记录、导出、修改备注 | V1.0 |
...
| 客户中心 | 客户列表 | 唯一客户档案 | 查看、编辑、身份补录、添加方式、首次来源、负责人、标签、首次建档继承线索等级、独立编辑客户等级和等级历史 | V1.0 |
| 客户中心 | 撞单管理 | 手机号与 UnionID 跨档案命中的强身份冲突处理 | 自动建案、幂等阻断、领取处理、双档案证据对比、指定主档合并、确认不同人、审计与反向恢复 | V1.0 |
| 客户中心 | 标签管理 | 统一管理系统、BOSS人工、企微、外部SCRM、规则和AI标签 | 标签库、标签组、自动规则、外部映射、标签治理、新增/编辑/停用、覆盖下钻、历史保留 | V1.0 |
...
| 客户中心 | [客户中心详细需求](./合数BOSS V1.0客户中心需求文档.md) |
...
| 区域 | 主要内容 |
|---|---|
| 全局筛选 | 数据权限范围、当前营期、日期区间、来源/平台、店铺、IP渠道、IP名称、组织和负责人；查询条件对经营数据表、下沉分析、下钻和导出统一生效 |
| 营期经营数据 | 按参考样式以单行横向分组表展示当前营期。过程转化：加微率、问卷填写率、预约率、到课率、完课率；结果质量：退款率、最终转化率、GMV完成率；经营效能：人服比、单人净GMV、团队转化离散率、当前在线；成交金额：毛GMV、退款金额、净GMV。比例指标下方保留对应人数或目标值 |
| 下沉分析 | 原渠道分析、IP渠道分析、商品分析及趋势等下沉内容、筛选、交互和统计口径保持不变，位于营期经营数据表下方 |
| 指标下钻 | 点击可下钻指标打开当前营期明细抽屉，展示订单、客户、来源、店铺、商品、IP归因、负责人、负责人组织、事件时间和当前节点 |
| 导出 | “导出数据”输出当前营期经营数据及既有下沉分析；明细抽屉可按当前指标、筛选和权限独立导出，明细包含负责人及负责人组织 |

**关键规则**

- 数据范围与当前用户的数据岗位、组织关系和特别授权一致，页面能力与业务角色权限一致；
- 指标必须展示统计周期、口径版本和更新时间，不得将不同口径数据直接相加；
- 点击经营数据表中的可下钻指标或下沉分析中的指标数字，打开继承营期、筛选与权限快照的明细抽屉；
- “导出数据”必须与当前筛选、统计口径和数据权限一致，并包含当前营期经营数据、渠道分析、IP渠道分析和商品分析；
...
全局筛选、营期经营数据横向分组表、原有下沉分析、指标下钻和分层导出可用；主管与员工看到的数据范围正确，汇总、下钻明细和导出在同一口径下能够核对，明细可追溯至负责人及其组织快照。完整字段、公式和交互见首页与线索中心详细需求。
...
引流线索字段编码、多维状态和转化联动已直接合并到《[合数BOSS V1.0——线索中心详细需求](./合数BOSS V1.0线索中心需求文档.md)》5.1及第6节。系统使用当前待办状态表达下一步优先事项，分配、解密、短信、加微、问卷、测评、跟进和订单状态分别保存事实；线索标记为有效、无效或重复；转化状态只保留未转化、已转化。建立客户档案不得直接标记为已转化。
...
| 首页—线索概览 | 当前营期以横向分组表展示过程转化、结果质量、经营效能和成交金额；原渠道、IP渠道和商品下沉分析保持不变；支持指标下钻和分层导出，明细包含负责人及负责人组织 |
...
7. 在“首页—线索概览”以当前营期横向分组表展示线索到成交的关键过程与经营结果；支持店铺、IP渠道及IP名称筛选，原渠道、IP渠道和商品下沉分析保持不变，关键指标可下钻，并可导出完整经营报表；下钻与导出明细同时包含负责人及负责人组织，完整口径见首页与线索中心详细需求。
8. 列表常用搜索包含综合关键词、当前待办状态、线索标记和转化状态；综合关键词支持订单编号、第三方商品ID、客户和手机号等信息，不提供线索编号搜索；更多条件包含分配、解密、短信、加微、问卷、测评、跟进和订单等独立状态，以及来源、添加方式、负责人、所属直播组、店铺和可选业务日期；所属直播组支持下拉及输入搜索；为兼容现有接口和存量数据，V1.0 可继续沿用 `camp_id/camp_name` 技术字段，但页面及业务文档统一展示“所属直播组”；
9. 每条线索提供生命旅程日志，以只追加时间轴记录接入、清洗、解密、分配/改派、触达、加微、问卷、测评、客户建档、状态/标记变化、订单同步、转化及退款回退。
10. 行操作固定展示“详情、旅程、更多”；建立客户档案、变更手机号、修改加微状态、变更线索标记、改派、短信提醒和备注等单条动作收进更多；不提供编辑线索、添加跟进和变更线索等级。“确认加微”不作为快捷动作，加微通常由企业微信事件或数据同步回写，但允许具备 `lead:wechat_status:update` 权限的人员通过“修改加微状态”纠正异常数据；状态固定为否、已加企微、已加个微，全部人工修改只追加旅程和审计，不删除历史身份数据。
...
| 页面 | 主要职责 |
|---|---|
| 客户总览 | 客户规模、生命周期、来源、添加方式、等级、归属和转化概览，支持按当前权限下钻 |
| 客户列表 | 唯一客户编码、身份、添加方式、首次来源、档案、负责人和客户画像；首次建档继承线索等级，支持独立编辑客户等级和查看等级历史 |
| 撞单管理（V1.0） | 手机号与 UnionID 分别命中不同客户时自动建案，支持双档案证据核验、指定主档合并、确认不同人、完整审计与反向恢复 |
| 标签管理 | 标签库、标签组、自动规则、外部标签映射和标签治理；六类来源统一展示、分源编辑，已使用标签只能停用并保留历史 |
...
- 手机号与 UnionID 分别命中两个不同有效客户时，必须原子性阻断当前自动建档并按“业务来源+来源记录号+手机号哈希+UnionID哈希”幂等生成撞单案件；
