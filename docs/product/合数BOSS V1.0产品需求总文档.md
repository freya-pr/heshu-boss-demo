...
| 线索中心 | 引流线索 | 引流业务线索列表及详情 | 线索标签最多展示2个及“+N”，仅按系统/企业微信来源查看；详情、旅程、是否挖需、变更手机号、修改加微状态及加微时间、分配改派、短信、同步订单及解密导入导出 | V1.0 |
...
| 客户中心 | 客户列表 | 唯一客户档案 | 生命周期仅为线索/意向/成交；字段设置仅展示齿轮图标并固定客户编号/客户名称/操作；客户创建时间、系统/企微标签筛选、手机号预览与变更；基本档案展示客户地址且不展示企微客户ID/UnionID；企微客户全量/增量同步 | V1.0 |
...
| 经营数据 | 单选期次时展示一行期次数据；多选期次时第一行展示汇总数据，其后逐行展示各期次数据；不选择期次、仅按线索创建时间筛选时正常汇总后续转化结果，期次列留空。过程转化包含加微率、问卷填写率以及DAY1/2/3到课率和完课率；期次列固定，其他字段可通过表头设置自由选择、自动记忆并恢复默认 |
...
| 客户列表 | 固定客户编号、客户名称和操作列；字段设置仅展示齿轮图标；生命周期仅为线索/意向/成交；支持客户创建时间、系统/企微标签、手机号预览/变更；基本档案展示客户地址且不展示企微客户ID/UnionID；保留线索、问卷、时间轴及企微客户同步 |
