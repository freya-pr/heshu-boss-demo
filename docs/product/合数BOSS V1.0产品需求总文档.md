...
> **V1.66 当前生效补充：**客户生命周期仅保留线索、意向、成交：引流线索进入即自动保存并建立客户经营档案，发生可追溯挖需动作后转意向，存在支付成功且有效的成交订单后转成交。手机号与UnionID分别命中不同客户时，不阻断经营档案、来源线索和后续业务记录，只冻结冲突身份覆盖与自动合并并同步创建撞单案件。客户列表统一“客户创建时间”；基本档案展示客户地址，不展示企微客户ID和UnionID。

> **V1.67 当前生效补充：**引流期次名称去除首尾空格并忽略大小写后全局唯一，批量预览内部或与既有期次重名时整批阻断，不自动跳过。引流线索改为“已加企微”时必须搜索并关联未合并的有效客户，以客户添加时间锁定加微时间并保存客户关系；单条“更多”移除“变更线索标记”。线索概览经营数据默认新增可下钻的“有效线索数”，并自动补齐历史表头设置。客户列表“客户创建时间”更名为“客户添加时间”，底层仍使用 `created_at`，客户总览筛选口径不变。
...
flowchart LR
    ...
    B --> C["自动保存线索并建立客户经营档案<br/>生命周期：线索"]
...
分配、解密和订单同步是并行环节，不设固定先后顺序。页面不展示“当前待办”字段；分配、解密、短信、加微、问卷、测评、跟进和订单状态分别保存并独立筛选，从而兼容异步回写、人工补录和跨渠道完成动作。完整字段枚举及状态联动以线索中心详细需求为准。
...
|---|---|---|---|---|
| 首页 | 线索概览 | 合数BOSS唯一默认首页与线索全链路营期经营台账 | 当前营期以横向分组表展示过程转化、结果质量、经营效能和成交金额；原渠道/IP渠道/商品下沉分析、指标明细下钻与导出能力保持不变 | V1.0 |
| 线索中心 | 引流线索 | 引流业务线索列表及详情 | 线索标签最多展示2个及“+N”，仅按系统/企业微信来源查看；详情、旅程、是否挖需、变更手机号；修改为已加企微时关联有效客户并锁定客户添加时间；分配改派、短信、同步订单及解密导入导出 | V1.0 |
...
| 线索中心 | 渠道管理 | 通过渠道列表/数据分析页签统一管理渠道基础信息及转化效果 | 渠道列表移除归因规则、负责人、上级渠道三列；保留新增、编辑、详情、启停、筛选、漏斗、下钻及导出 | V1.0 |
...
| 客户中心 | 客户列表 | 唯一客户档案 | 生命周期仅为线索/意向/成交；字段设置仅展示齿轮图标并固定客户编号/客户名称/操作；客户添加时间、系统/企微标签筛选、手机号预览与变更；基本档案展示客户地址且不展示企微客户ID/UnionID；企微客户全量/增量同步 | V1.0 |
| 客户中心 | 撞单管理 | 手机号与 UnionID 跨档案命中的强身份冲突处理 | 不阻断客户经营档案与业务记录；冻结冲突身份覆盖/自动合并并幂等建案，支持领取、证据对比、指定主档合并、确认不同人、审计与反向恢复；不展示风险内容 | V1.0 |
| 客户中心 | 标签管理 | 统一查看系统与企业微信标签 | 仅保留标签库和标签组；来源锁定为系统/企业微信，两类标签均只读；不提供自动规则、外部映射和标签治理 | V1.0 |
...
| 业务配置 / 配置管理 | 落地页配置 | 落地页及回传配置 | 新增、链接校验、发布和启停 | V1.0 |
...
| 客户中心 | [客户中心详细需求（唯一正式来源）](./合数BOSS V1.0客户中心需求文档.md) |
...
| 区域 | 主要内容 |
|---|---|
| 全局筛选 | 数据权限范围、期次、期次时间、线索创建时间、来源渠道、店铺、IP渠道、IP名称、归属组织和负责人；期次、渠道、店铺、IP渠道和IP名称支持多选，默认选择距当前日期最近的启用期次并带出其起止时间；查询条件对经营数据、分析、下钻和导出统一生效 |
| 转化链路 | 按有效线索、加微、填写问卷和成交展示人数及环节转化率；不再展示预约直播、到课、完课三个链路节点 |
| 经营数据 | 单选期次时展示一行期次数据；多选期次时第一行展示汇总数据，其后逐行展示各期次数据；不选择期次、仅按线索创建时间筛选时正常汇总后续转化结果，期次列留空。过程转化默认包含可下钻的有效线索数、加微率、问卷填写率以及DAY1/2/3到课率和完课率；期次列固定，其他字段可通过表头设置自由选择、自动记忆并恢复默认，历史设置自动补入有效线索数 |
...
引流线索字段编码、多维状态和转化联动已直接合并到《[合数BOSS V1.0——线索中心详细需求](./合数BOSS V1.0线索中心需求文档.md)》5.1及第6节。页面不展示“当前待办”字段或筛选；分配、解密、短信、加微、问卷、测评、跟进和订单状态分别保存并独立筛选，线索标记为有效、无效或重复，转化状态只保留未转化、已转化。建立客户档案不得直接标记为已转化。

**页面组成**

| 页面 | 主要职责 |
|---|---|
| 引流线索 | 展示公共线索字段及投放、素材、落地页等引流扩展字段 |
| 三方品线索（V1.5） | 展示公共线索字段及合作方、产品、批次等三方品扩展字段 |
| 线索配置 | 分类管理活码分配和营期名单流转规则；统一保存触发、条件、动作、适用范围、优先级、冲突策略、试算、执行快照和规则版本 |
| 活码管理 | 新旧活码、活码分组、关联IP、自动带出只读IP渠道、接待员工、备用人员、企微标签、内部标签、创建人、营期和班级 |
| 渠道管理 | 渠道列表维护渠道定义与来源映射；数据分析页签展示线索到客户及成交转化效果 |
| 首页—线索概览 | 统一展示转化链路与经营数据；支持单期、多期及无期次汇总、DAY1/2/3指标、表头设置、渠道/IP渠道分析、成交订单数与金额趋势、指标下钻和分层导出；移除商品分析 |
| 店铺管理 | 店铺按平台类型和第三方店铺ID建立唯一身份，配置负责人、权限并分析线索、加微、订单、GMV、退款和归因异常；店铺档案日期与经营业务日期分离，经营数据支持线索归因与业务发生两种口径 |
...
4. 支持详情、旅程、无效/重复标记、是否挖需、变更手机号、导出非解密数据、批量分配和改派；引流线索页不提供人工新增、编辑线索、添加跟进或人工变更线索等级入口；
...
8. 列表常用搜索包含综合关键词、线索标记和转化状态，不展示“当前待办”；更多条件包含分配、解密、短信、加微、问卷、测评、跟进和订单等独立状态，以及来源、添加方式、负责人、所属直播组、店铺和可选业务日期；
9. 每条线索提供生命旅程日志，以只追加时间轴记录接入、清洗、解密、分配/改派、触达、加微、问卷、测评、客户建档、状态/标记变化、订单同步、转化及退款回退。
10. 行操作固定展示“详情、旅程、更多”；建立客户档案、变更手机号、修改加微状态、改派、短信提醒和备注等单条动作收进更多；不提供编辑线索、添加跟进、变更线索等级和变更线索标记。“确认加微”不作为快捷动作，加微通常由企业微信事件或数据同步回写，但允许具备 `lead:wechat_status:update` 权限的人员通过“修改加微状态”纠正异常数据；状态固定为否、已加企微、已加个微。改为已加企微时必须选择未合并的有效客户，以客户添加时间自动锁定加微时间并保存客户关联；全部人工修改只追加旅程和审计，不删除历史身份数据。
11. 列表上方批量操作只承载分配、同步订单、短信群发、导入解密数据和导出非解密数据；其中分配和短信群发需要先勾选线索，同步订单属于全局后台任务，无需勾选线索且不读取当前列表勾选结果；同步订单允许多选已接入平台，并按用户勾选先后顺序为各平台创建独立后台任务；短信群发必须选择已审核启用的签名与模板，并排除无有效手机号、退订和黑名单；导入固定模板必须包含订单编号和手机号，只按订单编号唯一命中并覆盖既有引流线索手机号，不创建线索；导出基于当前查询结果，仅输出未解密且有订单编号的线索订单；页面移除同步手机号和查询异常订单，单条操作移除语音提醒。
...
- 人工指定展示组织树，并支持按员工姓名或员工编号实时搜索当前组织及下级组织人员；员工卡片展示员工编号、岗位和当前负责的未转化线索数；人工指定不受营期活码名单和最大分配人数限制；
- 轮询仅从最新营期活码人员名单中选择员工，并校验最大分配人数；达到上限、未配置活码、离职或停用员工不参与轮询；
...
两类线索可稳定进入系统并保留差异字段；人工指定可按组织选人，轮询可按最新营期活码名单执行并受最大分配上限约束；新旧活码事件均能回写；搜索条件可以组合并完整重置；关键变更可通过生命旅程日志追溯；失败记录可重试或人工处理。
...
| 撞单管理（V1.0） | 手机号与 UnionID 分别命中不同客户时不阻断经营档案与业务记录，只冻结冲突身份覆盖/自动合并并自动建案；支持证据核验、指定主档合并、确认不同人、审计与恢复，不展示风险内容 |
| 标签管理 | 仅保留标签库和标签组；标签来源仅为系统和企业微信，两类均只读并保留稳定ID和同步历史 |
...
- 手机号与 UnionID 分别命中两个不同有效客户时，不阻断客户经营档案、来源线索和后续业务记录；必须原子性冻结冲突身份覆盖与自动合并，并按“业务来源+来源记录号+手机号哈希+UnionID哈希”幂等生成撞单案件；
...
手机号/UnionID 唯一客户逻辑、三阶段生命周期、客户档案、撞单管理、标签和等级均可形成闭环；客户中心无“跟进记录”“等级管理”“公海客户”和“商机管理”二级菜单；手机号与 UnionID 跨档案冲突不阻断经营档案和业务记录，可完成冻结冲突身份、建案、领取、证据核验、人工裁决、审计和反向恢复。
...
岗位新增/编辑字段包括岗位名称、只读岗位编码、范围类型、自定义组织、是否包含下级、协作数据策略、排除项、说明和状态。有生效员工的岗位不可删除，停用岗位后不可新增绑定；修改数据范围前需预览受影响员工、组织和业务数据范围。
...
岗位授予数据范围类型，实际小组和部门通过组织管理中的有效负责人关系动态解析；“小组负责人、部门负责人、公司数据管理员”属于数据岗位，不作为业务角色。负责人调岗或负责人关系变更后刷新权限缓存，无需重写业务角色或历史业务数据；部门没有小组时仍可查看部门直属员工负责的数据；一人负责多个组织时范围取并集。自定义范围可选择多个公司、部门或小组并配置是否包含下级。
...
左侧导航在短信管理下设置“签名管理、短信模板、发送结果”三个三级菜单，分别使用独立路由、查询、列表和操作，不在页面内使用页签。
...
4. 线索通过组织到人的人工指定或最新营期活码名单轮询完成分配；人工指定不受营期活码名单及人数上限限制，轮询必须校验最新营期名单和最大分配人数；
...
| 营期活码人员名单或最大分配上限维护不及时 | 轮询无可用员工或员工承接过量 | 营期开启前完成名单与上限校验；无可用员工时保留待分配并通知管理员 |
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V1.67 | 2026-08-31 | 引流期次名称改为全局唯一且批量重名整批阻断；已加企微必须关联有效客户并锁定客户添加时间；单条更多移除变更线索标记；线索概览默认新增有效线索数；客户列表时间字段更名为客户添加时间 |
| V1.66 | 2026-08-28 | 客户生命周期收口为线索/意向/成交并明确引流进入、挖需和有效成交订单三个转换条件；撞单调整为不阻断客户经营档案和业务记录，仅冻结冲突身份覆盖/自动合并并同步建案；客户列表统一客户创建时间，基本档案补充客户地址并移除企微客户ID/UnionID展示；最终客户流程图写入客户专项文档 |
| V1.65 | 2026-08-28 | 客户档案补充基本档案、线索、问卷测评和时间轴字段；客户列表新增企微客户全量/增量同步，明确 external_userid、多员工跟进关系、幂等补偿及手机号限制；标签来源收口为系统/企业微信且仅保留标签库/标签组；修改加微状态增加加微时间 |
