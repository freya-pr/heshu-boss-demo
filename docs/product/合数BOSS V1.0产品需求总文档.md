...
> **V1.94 当前生效补充（2026-09-12）**：客户中心“标签管理”统一更名为“企微标签”。页面采用左侧标签分组、右侧标签明细的管理结构；支持分组搜索、新增分组、分组编辑/删除、空分组批量删除、单个及批量新增标签、标签编辑/删除、全局标签搜索，以及点击“使用客户”跳转至已按标签筛选的客户列表。标签来源不展示“系统”。客户列表移除顶部唯一身份、客户来源和添加方式说明卡片，但不改变底层客户识别及归因规则。

> **V1.95 当前生效补充（2026-09-14）**：企微标签仅保留企微企业标签，移除企微个人标签入口、数据及切换逻辑。新增和编辑标签分组时，适用部门均支持组织树多选；“全部部门”与具体部门互斥且至少选择一项。分组列表中的适用部门可点击进入编辑，多部门以顿号连接展示。
...
flowchart LR
    ...
    subgraph CUSTOMERBOX["业务域二｜线索与客户中心｜阶段一重点"]
        direction TB
        HOME["首页｜线索概览 · V1.0"]
        LEAD["线索中心｜引流线索  · V1.5<br/>线索配置 · V1.0<br/>活码管理 · V1.0<br/>渠道管理（列表/分析）· V1.0<br/>店铺管理 · V1.0<br/>IP管理 · V1.0<br/>商品列表 · V1.0<br/>短信配置 · V1.0"]
...
| 客户中心 | 企微标签 | 管理企业微信企业标签分组及标签 | 支持分组搜索、适用部门多选、新增、编辑、删除和空分组批量删除；支持单个/批量新增标签、编辑、删除、全局搜索及使用客户下钻；不展示系统来源或企微个人标签 | V1.0 |
...
| 审计管理 | 发送结果 | 短信供应商回执、送达结果与失败信息 | 查询、详情 | V1.0 |
...
| 业务配置 / 企微管理 / 企微配置 | 应用管理 | 接入合数BOSS的外部应用 | 从企微配置行内“应用”进入；新增、编辑、启停、连接校验和凭证轮换 | V1.0 |
| 业务配置 / 企微管理 | 企微配置 | 企业微信接入配置 | 校验、发布、凭证轮换、版本回退和应用入口 | V1.0 |
...
| 企微标签 | 仅维护企微企业标签；左侧维护标签分组和多个适用部门，右侧维护当前分组标签；支持分组搜索、新增、编辑、删除及空分组批量删除，支持单个/批量新增标签、全局搜索和使用客户下钻；不展示系统来源或企微个人标签 |
...
左侧导航在短信管理下设置“运营商管理、签名管理、短信模板、短信规则”四个三级菜单，分别使用独立路由、查询、列表和操作。“发送结果”迁移至一级“审计管理”下，与异常中心并列。短信规则新增/编辑必须填写规则名称、类型、关联抖店、签名和模板；类型固定为用户下单后、用户提醒、用户授权后。同一抖店与类型只允许一条生效规则，变更全量审计。选择“用户下单后”时固定展示补发策略：支付成功后 3 分钟和下单后 8 分钟分别检查加微状态，仅未加微时发送；加微后取消剩余任务，同一订单同一节点不得重复提交。

##### J. 应用与配置管理

菜单归属：企微管理位于一级菜单“业务配置”下；应用管理不作为独立菜单，从企微配置列表行内入口进入。

- 应用管理：从企微配置行内“应用”进入，页面不新增工作台标签；支持外部应用新增、编辑、启停、连接校验、凭证轮换及调用范围；
- 企微管理：企微、小程序和落地页达到“可配置、可校验、可发布、可回退”，微信客服保留独立入口；
...
| 版本 | 日期 | 内容 |
|---|---|---|
| V1.95 | 2026-09-14 | 企微标签移除企微个人标签模块，仅保留企业标签；标签分组适用部门支持组织树多选、全部部门互斥、必选校验、多部门回显及点击部门直接编辑 |
| V1.94 | 2026-09-12 | 标签管理更名为企微标签并按最新交互重构：补充分组搜索、新增/编辑/删除、空分组批量删除、单个/批量新增标签、全局标签搜索和使用客户下钻；移除系统来源及客户列表顶部说明卡片 |
