...
> **V1.71 当前生效补充：**引流线索来源筛选移除“认证赠送”和“店播/阿留直播间”，存量来源快照保持不变。业务配置—配置管理—落地页配置完善独立三级页面，页面内不再展示配置切换页签；支持名称/状态筛选、手机端预览、编辑H5地址及“获客助手/小程序/微信客户”主跳转方式与兜底优先级、启停，并仅允许删除停用记录。后文与本补充冲突的旧表述均不再生效。
...
| 线索中心 | 引流线索 | 引流业务线索列表及详情 | 新增IP大类和线索标签默认列，IP大类读取归因快照对应字典名称；线索标签最多展示2个及“+N”，支持按系统/企业微信来源查看且均只读；详情、旅程、是否挖需、变更手机号/期次/直播组、修改加微状态、分配改派、短信、同步订单及解密导入导出 | V1.0 |
...
| 业务配置 / 配置管理 | 落地页配置 | 落地页地址、主跳转方式与兜底优先级 | 名称/状态筛选、手机端预览、编辑H5地址、调整获客助手/小程序/微信客户跳转优先级、启停及停用后删除 | V1.0 |
