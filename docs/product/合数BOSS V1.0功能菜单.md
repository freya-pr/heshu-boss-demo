...
flowchart TD
    ...
    SMS --> SMSCFG["签名 / 模板 / 规则"]
    BIZCFG --> WECOM["企微管理"]
    WECOM --> WXCFG["企微配置（操作列含应用入口）"]
    WECOM --> OTHERCFG["小程序 / 落地页配置"]
    WECOM --> CS["微信客服管理"]
...
| 线索中心 | 活码管理 | 活码分组、新旧活码、必填关联IP并自动带出IP渠道、企微/内部标签、排班人员与接量权重、创建人和效果；统一多人接待，无备用员工及接量日期 | P0 |
| 线索中心 | 渠道管理 | 渠道列表维护基础信息、归因和状态；数据分析页签展示渠道线索到客户及成交转化 | P1 |
| 线索中心 | 店铺管理 | 店铺档案、负责人、权限、线索/加微、订单/支付、净GMV、经营详情和归因异常 | P1 |
| 线索中心 | IP管理 | 维护IP编码、IP名称、IP大类和IP渠道；IP编码人工必填并校验全局唯一，页面不展示大类编码 | P1 |
| 线索中心 | 商品列表 | 统一展示第三方商品、店铺、活码分配状态和关联IP，支持同步、筛选、导出和修改备注 | P1 |
| 线索中心 | 短信配置 | 按商品与活码配置购买后、授权后及手动提醒；下单规则支持支付后3分钟、下单后8分钟未加微补发，签名模板与发送回执复用业务配置短信管理 | P1 |
| 客户中心 | 客户总览 | 客户数量、生命周期、来源、添加方式、等级、归属和转化概览；支持下钻明细 | P1 |
...
| 客户中心 | 企微标签 | 按企业微信标签组维护企微企业标签，并与当前企业主体同步；支持标签组搜索、新增、编辑、空分组单条/批量删除、适用部门多选，以及标签的全局搜索、单条/批量新增、编辑、删除和使用客户下钻 | P1 |
| 问卷管理 | 问卷列表 | 问卷列表、详情、发送、回收、答卷、同步及客户/线索关联 | P0 |
| 个人中心 | 用户信息 | 查看并维护本人资料、密码及外部绑定；一个账号允许绑定多个企业微信身份 | P0 |
| 个人中心 | 我的下载 | 按服务人、部门、申请人/文件名称、申请时间和生成状态查询导出任务；下载记录列表展示时间、文件、下载人和状态，不展示下载IP | P1 |
...
| 业务配置 / 短信管理 | 运营商管理 | 短信运营商及服务商接入信息、状态和优先级 | P1 |
| 业务配置 | 短信管理 | 运营商管理、签名管理、短信模板、短信规则和下单未加微补发；短信日志进入审计管理 | P1 |
| 业务配置 / 企微管理 | 企微配置 | 企业微信接入配置；操作列“应用”进入应用管理页面 | P0 |
| 业务配置 / 企微管理 | 小程序配置 | 小程序接入配置 | P1 |
| 业务配置 / 企微管理 | 落地页配置 | 落地页地址、跳转及兜底配置 | P1 |
| 业务配置 / 企微管理 | 微信客服管理 | 企微ID切换、客服列表、事件消息和客服消息；切换后数据与新建归属均按所选主体刷新 | P1 |
| 系统管理 | 日志查询 | 登录、操作、权限、配置和接口日志 | P0 |
| 系统管理 | 地区管理 | 标准地区树及启停 | P1 |
| 审计管理 | 业务异常 | 数据同步、身份冲突、分配、订单、退款等核心业务异常 | P1 |
| 审计管理 | 企微异常 | 企微接口、授权、回调和通讯录同步异常 | P1 |
| 审计管理 | 短信日志 | 短信提交结果、供应商回执、错误码、计费及补发节点 | P1 |
...
- 应用管理不作为左侧菜单展示，由企微配置列表操作列“应用”进入；
- 支付默认切换和退款密码/凭据设置。
