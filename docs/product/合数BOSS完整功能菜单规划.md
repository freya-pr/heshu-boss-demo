...
- “交付查询”统一更名为“交付中心”；
- 个人中心、业务配置与系统管理均属于合数BOSS应用内部；个人中心位于业务配置上方，业务配置位于系统管理上方。
...
flowchart LR
    ...
    subgraph SUPPORTBOX["业务域八｜平台支撑中心｜阶段一重点"]
        direction TB
        BUSINESS_BASE["业务配置｜短信管理 · V1.0<br/>企微管理 · V1.0<br/>百家号配置 · V2.0<br/>审计管理｜业务异常 · V1.0<br/>企微异常 · V1.0<br/>短信日志 · V1.0"]
...
| 线索中心 | 活码管理 | 新旧员工活码、分组、IP归因、标签和多人接待配置 | 分组筛选、新增、编辑、启停、关联IP、排班规则、规则内员工与接量权重、全天在线兜底、创建人及双类标签 | V1.0 |
...
| 个人中心 | 用户信息 | 当前登录用户的资料与安全设置 | 基本设置、密码设置、多个外部身份绑定 | V1.0 |
| 个人中心 | 我的下载 | 当前权限范围内的导出任务与下载记录 | 服务人、部门、申请人/文件名称、申请时间、状态筛选、刷新、下载、记录 | V1.0 |
...
| 业务配置 | 短信管理 | 复用历史短信服务并按抖店配置发送规则 | 运营商管理、签名管理、短信模板、短信规则及未加微补发 | V1.0 |
| 业务配置 | 企微管理 | 企微、小程序、落地页和微信客服配置 | 企微配置操作列以“应用”进入应用管理；不设独立应用菜单和店铺配置 | V1.0 |
| 业务配置 | 百家号配置 | 百家号接入、授权和线索映射 | 新增、授权、编辑、启停和同步 | V2.0 |
| 业务配置 / 企微管理 | 微信客服管理 | 企业微信客服接入 | 企微ID切换、客服列表、客服事件消息和客服消息 | V1.0 |
| 审计管理 | 业务异常 | 数据同步与核心业务异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 审计管理 | 企微异常 | 企微接口、授权、回调与同步异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 审计管理 | 短信日志 | 短信任务与供应商回执 | 结果、错误码、计费、重试及补发节点 | V1.0 |
...
| 活码管理 | 按企业微信主体、活码分组、企微/内部标签和期次配置多人接待，承接新旧活码及扫码/加微事件 | 分组筛选、新增、编辑、启停、排班人员与接量权重、全天在线兜底、创建人、查看效果 | V1.0 |
...
| 标签管理 | 仅管理系统、企业微信两类只读标签及标签组；标签组可新增、编辑、启停并持久保留 | V1.0 |
### 4.4 订单数据能力（非菜单）
...
| 二级菜单 | 功能说明 | 版本 |
|---|---|---|
| 短信管理 | 运营商管理、签名管理、短信模板、短信规则及未加微补发 | V1.0 |
| 企微管理 | 企微、小程序、落地页和微信客服；应用管理由企微配置操作列进入 | V1.0 |
| 百家号配置 | 管理百家号接入、授权和线索来源映射 | V2.0 |
| 微信客服管理 | 企微ID切换、客服列表、客服事件消息和客服消息 | V1.0 |
| 审计管理 | 业务异常、企微异常、短信日志、重试和处理记录 | V1.0 |

## 5. 版本范围汇总

| 版本 | 主要范围 |
|---|---|
| V1.0 | 首页—线索概览、线索中心（不含三方品线索）、客户中心（含撞单管理）、营期、问卷列表、个人中心、业务配置及系统管理基础 |
| V1.5 | 三方品线索 |
...
| 线索中心 | 活码管理 | 新旧员工活码、分组、IP归因、标签及多人接待配置 | 分组筛选、新增、编辑、启停、关联IP、排班规则、规则内员工与接量权重、全天在线兜底、创建人及双类标签 | V1.0 |
...
| 系统管理 | 系统参数 | 全局设置与运行参数 | 查询、新增、编辑、启停、类型校验、敏感值保护和版本历史 | V1.0 |
| 系统管理 | 字典管理 | 业务枚举与显示口径 | 字典类型、字典项、排序、颜色、启停和历史值兼容 | V1.0 |
| 业务配置 | 短信管理 | 复用历史短信服务 | 运营商管理、签名管理、短信模板、短信规则及未加微补发 | V1.0 |
| 业务配置 | 企微管理 | 企微、小程序、落地页和微信客服接入 | 企微配置操作列“应用”入口、企微ID切换、配置校验与发布 | V1.0 |
| 审计管理 | 业务异常 | 数据同步与核心业务异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 审计管理 | 企微异常 | 企微接口、授权、回调与同步异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 审计管理 | 短信日志 | 短信任务与供应商回执 | 结果、错误码、计费、重试及补发节点 | V1.0 |
| 系统管理 | 日志查询 | 登录、操作、权限、配置和接口日志 | 查询、详情、审批导出和归档 | V1.0 |
