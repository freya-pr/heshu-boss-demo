...
| 线索中心 | 引流线索 | 引流业务线索列表，使用公共主模型和引流扩展字段 | 详情、旅程、编辑、无效、跟进、批量分配、短信群发、查询异常订单、同步异常订单、按平台勾选顺序同步订单、导入解密数据、导出非解密数据 | V1.0 |
...
| 线索中心 | IP管理 | 按IP大类、IP渠道、IP名称维护老师IP主档 | 移除当前IP、IP配置、关联商品/平台；新增、编辑、启停 | V1.0 |
| 线索中心 | 商品列表 | 第三方商品与店铺、活码关联主档 | 创建/更新时间、商品名称/ID/状态、活码分配、店铺名称/类型、同步、任务记录、导出、备注；移除关联IP | V1.0 |
| 线索中心 | 短信配置 | 商品与活码维度的业务短信提醒策略 | 短信分组、商品/活码绑定、购买后/授权后/手动提醒、批量设置、新增、编辑、启停 | V1.0 |
| 客户中心 | 客户总览 | 客户经营数据概览 | 统一权限与客户创建时间筛选（默认最近一个月），查看生命周期/添加方式分布并下钻明细 | V1.0 |
...
| 二级菜单 | 功能说明 | 主要页面动作 | 版本 |
|---|---|---|---|
| 引流线索 | 管理广告投放、落地页、表单等引流来源线索，展示投放、素材和归因扩展字段 | 详情、旅程、编辑、无效、跟进、批量分配、短信群发、查询异常订单、同步异常订单、按平台勾选顺序同步订单、导入解密数据、导出非解密数据 | V1.0 |
...
| 短信管理 | 签名管理、短信模板、短信规则、发送结果；复用历史短信服务 | V1.0 |
...
| 线索中心 | 引流线索 | 引流业务线索列表及详情 | 详情、旅程、编辑、无效、跟进、批量分配、短信群发、查询异常订单、同步异常订单、按平台勾选顺序同步订单、导入解密数据、导出非解密数据 | V1.0 |
...
| 系统管理 | 字典管理 | 业务枚举与显示口径 | 字典类型、字典项、排序、颜色、启停和历史值兼容 | V1.0 |
| 业务配置 | 异常中心 | 企业微信及同步异常 | 查询、详情、重试、忽略、转人工、关闭 | V1.0 |
| 业务配置 | 短信管理 | 复用历史短信服务 | 签名管理、短信模板、短信规则、发送结果 | V1.0 |
| 业务配置 | 应用管理 | 接入合数BOSS的外部应用 | 新增、编辑、启停、连接校验和凭证轮换 | V1.0 |
