...
> **V1.72 当前生效补充：**活码统一按多人接待配置，不再区分或展示单人活码类型；移除备用员工、接量日期区间、列表及详情二维码、详情下载入口，以及页面顶部“轮询权重/当前接量”汇总指标，列表仍保留下载操作。新增/编辑按“基础信息—排班设置—接待员工（仅全天在线）—客户标记”排列；员工配置统一展示姓名、接量权重和只读当前接量。分时段排班先配置自定义规则人员，再配置不可删除的全天在线兜底人员；新增规则入口位于“自定义分时段规则”旁，每条规则及兜底规则内直接选择员工并设置接量权重，无需另行配置接待员工。后文与本补充冲突的旧表述均不再生效。
...
> **V1.70 当前生效补充：**引流线索批量“短信群发”不再人工选择签名、模板或编辑内容，只选择必填订单日期；默认带出当前日期。系统按所选日期、线索所属抖店及当前生效短信规则匹配发送配置，未匹配或匹配多条时不发送并记录失败原因。活码列表移除复制短链入口，仅保留下载活码、查看详情和状态管理。后文与本补充冲突的旧表述均不再生效。
...
| 订单 | 订单状态 | `order_status` | enum | 第三方实时回传映射结果：未支付、已支付、已取消、已完成、退款中、已退款；未产生订单时为空 |
...
订单状态由平台实时回传的原始状态映射为合数BOSS统一状态。列表及筛选只使用未支付、已支付、已取消、已完成、退款中、已退款六个汇总状态；详情和状态历史保留平台原始状态、映射版本及更新时间。无法映射时保留最近一次有效内部状态，并进入异常中心，不新增页面枚举。
...
| `AFTER_SALE_PROCESSING` | 售后处理中 | 退款、退货退款等售后申请正在处理 |
...
| 退款或退货申请处理中 | 售后中 | 取消中/售后处理中 | 售后中 | 退款中 | 退款中 | 售后处理中 |
...
汇总状态优先级为：售后处理中 ＞ 全额退款完结 ＞ 部分退款 ＞ 已取消 ＞ 已完成 ＞ 已发货 ＞ 备货中 ＞ 已支付 ＞ 待支付。全额退款成功后必须读取退款发生时的履约状态快照：从未发货映射为发货前退款完结，已发货但未签收/未完成映射为发货后退款完结，已签收/已核销/已完成映射为收货后退款完结。
...
- [ ] 订单状态筛选与列表统一展示未支付、已支付、已取消、已完成、退款中、已退款六个实时第三方回传映射状态；未知外部状态不生成新的页面枚举；
...
- 列表支持下载活码、查看详情、编辑和启停，不提供复制短链入口；详情不展示二维码，也不提供下载活码按钮；
...
| 订单状态 | `order_status` | `UNPAID` 未支付、`PAID` 已支付、`CANCELLED` 已取消、`COMPLETED` 已完成、`REFUNDING` 退款中、`REFUNDED` 已退款；由第三方实时回传并映射，无订单时为空 |
...
| V1.63 | 2026-09-02 | 将第三方订单状态映射方案并入引流线索：统一11个内部订单状态，外部待付款/未确认统一为待支付，退款按履约快照区分发货前、发货后、收货后退款完结，新增售后处理中和部分退款；不设置状态异常，未知状态保留原值并进入异常中心 |
