...
> **V1.78 当前生效补充：**新增和编辑活码时“关联IP”为必填，只允许选择启用IP并自动带出只读IP渠道。IP管理新增人工录入的“IP编码”，字段置于IP名称上方、全局唯一，保存时前后端均须查重；列表与表单不展示IP大类编码。引流线索列表保留“查询异常订单”和“同步异常订单”两个独立入口，现行批量操作与验收均以本补充为准。
...
| 活码管理 | 企业微信主体及企微ID切换、活码分组、新旧活码、排班与接待员工、接量权重、企微标签、内部标签、创建人、营期和效果 | P0  |
| 渠道管理 | 渠道列表维护渠道、来源映射、归因口径和启停；数据分析页签展示接入、有效、分配、加微、转客户及成交分析       | P1  |
| 店铺管理 | 店铺、主体、应用、组织和线索来源映射                                       | P1  |
| IP管理 | 按IP大类、IP渠道、IP名称顺序维护IP主数据，不关联商品或平台                        | P1  |
...
| 5   | IP主播   | IP编码             | 商品—IP配置；名称解析仅兜底   | 多义或未命中进入归因异常           |
...
- 列表上方提供“批量操作”，承载分配、变更期次、同步订单、查询异常订单、同步异常订单、短信群发、导入解密数据和导出非解密数据；分配、变更期次与短信群发处理已勾选线索，同步订单和查询异常订单为不依赖列表勾选数据的全局后台任务，同步异常订单从有效查询批次勾选待同步或同步失败订单，导入解密数据按模板内容处理，导出非解密数据按当前查询结果处理；不提供同步手机号入口。
...
| 查询异常订单 | 全局任务，无需勾选线索 | 至少选择一个已接入平台                     | 生成唯一查询批次和异常订单明细；查询结果为后续补同步依据，不作为真实同步成功凭据                  |
| 同步异常订单 | 查询批次内勾选订单    | 待同步、同步失败                          | 只读取有效查询批次；逐单生成补同步任务并回写结果，同一查询批次、平台和订单重复请求必须幂等             |
...
- [ ] 批量操作包含分配、变更期次、同步订单、查询异常订单、同步异常订单、短信群发、导入解密数据和导出非解密数据；同步订单无需勾选线索，可多选平台并按平台勾选顺序创建独立全局后台任务；查询异常订单至少选择一个平台并生成唯一批次，补同步只处理有效批次中待同步或同步失败的订单；页面无新增线索和同步手机号入口，单条操作无语音提醒；
- [ ] 单条“更多”不出现整体数据操作；
...
- 活码必须关联一个启用IP。选择后系统从IP主档自动带出只读的IP渠道名称与渠道编号，不允许在活码中另行选择或覆盖；未选择IP时禁止保存，并提示“请选择关联IP”；
...
| 关联IP          | 是       | 从IP管理中搜索IP名称或IP编码；只允许选择启用IP，保存稳定IP编码；未选择时禁止保存                         |
...
| 关联IP、IP编码      | 此活码的IP归因配置                      | 新增和编辑均必填；不凭活码名称猜IP；变更仅影响后续归因              | 现有                |
...
| 字段       | 规则                                                                                              |
| -------- | ----------------------------------------------------------------------------------------------- |
| IP编码     | 管理员人工录入的稳定编码；必填、全局唯一，保存前执行前后端重复性校验；创建后不可修改或复用，在IP名称上方展示                                          |
| IP名称     | 老师姓名或老师对外名称，例如“阿留皮皮”                                                                        |
| IP大类     | 从字典 `ip_category` 解析并展示中文名称；初始仅提供“教育规划”，页面不展示大类编码                                              |
| IP渠道     | 展示渠道名称及稳定渠道编号；初始为“店播/CH000001”“阿留专属/CH000002”                                                   |
| 创建人/创建时间 | 保存主档的实际创建人和时间                                                                                   |
| 状态       | 启用、停用；停用后不可用于新关联，历史关系保留                                                                         |

筛选支持IP名称、IP编码或创建人综合搜索，以及IP大类、IP渠道和状态。列表操作包括编辑、启用和停用。

#### 5.10.2 IP关系维护

IP管理页面直接维护“IP名称—IP大类—IP渠道”的生效关系，IP列表主档是唯一数据源，不另建重复配置入口。

- 配置列表按行展示IP编码、IP名称、IP大类、IP渠道及渠道编号、状态和操作；不展示IP大类编码；
- 新增和编辑使用相同字段顺序，依次为IP编码、IP名称、IP大类、IP渠道和IP说明；保存时先校验IP编码全局唯一，再校验IP名称、IP大类和IP渠道关系；
- 同一IP编码同一时点只能存在一组生效的大类与渠道关系；修改关系必须记录修改前后值和生效时间；
- 停用IP后不能再被新活码选择，但历史活码、扫码、线索及订单仍按原IP编码和渠道快照追溯；
- 活码管理只引用IP编码并解析当前IP配置，禁止在活码侧维护第二套IP大类或IP渠道关系。

#### 5.10.3 新增、编辑与字典联动

- 新增和编辑必填IP编码、IP名称、IP大类、IP渠道；IP说明选填；IP编码字段位于IP名称上方；
- IP编码由管理员人工输入，保存时前端先提示格式与重复问题，服务端再次执行全局唯一校验；重复时禁止保存并提示“IP编码已存在”；创建后IP编码不可修改或复用；
- IP大类只读取系统管理—字典管理中编码为 `ip_category` 且处于启用状态的字典项；
- IP大类仍读取字典稳定值用于接口、筛选和历史归因，但列表、新增、编辑及详情页面均不展示大类编码，也不提供大类编码输入框；
- 初始字典项为“教育规划”；业务表保存字典稳定值，不因页面不展示而改写历史数据；
...
- [ ] IP列表展示IP编码、IP名称和IP大类名称，不展示IP大类编码；
- [ ] IP编码在新增表单中位于IP名称上方，必填且由管理员输入；重复编码在前端提示并由服务端最终阻止保存，创建后不可修改或复用；
- [ ] 点击“IP配置”可集中查看和编辑IP名称、IP大类与IP渠道的唯一生效关系；
- [ ] IP大类筛选按中文名称搜索并准确过滤，页面不暴露大类编码；
- [ ] IP列表展示IP渠道名称与渠道编号，并支持按IP渠道筛选；
- [ ] 新增、编辑表单可选择启用的IP大类，初始可选项只有“教育规划”；不展示或编辑大类编码；
- [ ] 示例及新增IP名称使用老师名称口径，可录入“阿留皮皮”；
- [ ] 字典中新增启用大类后，IP页面无需发版即可选择；停用大类不影响历史展示；
- [ ] IP编码人工输入、全局唯一且不可修改，商品和平台关系可下钻查看。
- [ ] 新建或编辑活码未选择关联IP时禁止保存；选择IP后自动带出只读IP渠道，保存后列表和详情展示同一IP编码、渠道编号，变更留痕可追溯。
...
| 关联IP      | 展示IP名称和不可变IP编码；支持从IP管理带入IP编码下钻筛选 |
...
- 触发业务事件后，系统读取当前生效策略并调用业务配置中的短信模板与签名；短信发送结果统一进入“审计管理—发送结果”；
...
| 版本    | 日期         | 内容                                                                                                                                                       |
| ----- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| V1.78 | 2026-09-22 | 活码关联IP改为新增、编辑必填；IP管理新增人工录入且全局唯一的IP编码并移除页面大类编码展示；保留查询异常订单和同步异常订单两个独立入口，清除旧验收冲突                                                                 |
