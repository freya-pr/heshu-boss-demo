...
| 业务域 | V1.0能力 |
|---|---|
| 首页 | 线索概览、渠道分析、IP渠道分析、经营报表及指标下钻 |
| 线索中心 | 引流线索、线索配置、活码管理、渠道管理、店铺管理、IP管理、商品列表、短信配置 |
| 客户中心 | 客户总览、客户列表、客户360°档案、撞单管理、企微标签、客户等级编辑 |
| 问卷管理 | 问卷、答卷、线索/客户关联、S/A/B/C自动评级输入及异常处理 |
| 个人中心 | 用户信息、密码设置、外部绑定和我的下载 |
| 业务配置 | 短信管理、企微管理；应用由企微配置操作列进入 |
...
> 系统不建设客户公海、自动回收或公海重新分配。超过跟进时限后，系统生成超时待办并通知主管，由有权限的管理人员选择提醒原负责人或人工改派；原负责人关系和改派历史必须完整保留。
...
| 5 | IP | IP编码 | 商品—IP配置优先，名称解析兜底 |
| 6 | IP渠道 | 渠道编号 | IP配置自动带出 |
| 7 | IP大类 | 字典稳定值（后台保存，页面不展示） | 字典和IP配置 |
...
| 规则项 | 人工指定 | 自动轮询 |
|---|---|---|
| 选择范围 | 当前用户有权操作的组织及员工 | 当前命中活码排班规则的生效人员名单 |
| 排班名单限制 | 不限制 | 必须在当前全天在线或分时段规则中 |
| 接量权重 | 不使用 | 轮询时按规则内员工权重计算分配比例 |
| 基础校验 | 员工在职、账号启用、操作者有改派/分配权限 | 员工在职、账号启用、活码有效、排班规则生效 |
| 记录要求 | 操作人、原因、目标员工和时间 | 规则版本、名单版本、权重及当前接量快照、轮询游标和时间 |

### 7.2 活码匹配规则

- 活码新增和编辑时必须关联一个启用IP；期次选填，未关联期次时按通用活码处理；不再配置班级及接量日期；
- 选择IP后自动带出只读IP渠道名称和渠道编号，不允许在活码内另行覆盖；未选择IP时禁止保存；
- 活码统一按多人接待配置，不展示单人活码类型；全天在线单独配置接待员工，分时段规则及不可删除的全天兜底均在规则内直接选择员工并设置接量权重；
...
- [合数BOSS V1.0客户中心详细需求（GitHub唯一正式来源）](https://github.com/freya-pr/heshu-boss-demo/blob/main/docs/product/%E5%90%88%E6%95%B0BOSS%20V1.0%E5%AE%A2%E6%88%B7%E4%B8%AD%E5%BF%83%E9%9C%80%E6%B1%82%E6%96%87%E6%A1%A3.md)
