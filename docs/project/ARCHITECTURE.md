...
组织员工初始化只接收整改后的企业微信基线，飞书不进入初始化、身份匹配、员工编号或同步对账链路。合数BOSS登录账号与企微 `CorpID + UserID` 采用账号一对多、外部身份全局唯一的生效绑定；当前绑定表允许同一 `account_id` 存在多条记录，并对 `(corp_id, wecom_user_id)` 建立生效唯一约束。解绑与迁移写入历史表后再变更当前关系。自有系统主控开启时，员工管理可创建企微通讯录成员并发送激活邀请；先按手机号查重，创建、绑定、邀请分步持久化，企微已创建但BOSS落库失败时通过任务补偿，不重复创建成员。
