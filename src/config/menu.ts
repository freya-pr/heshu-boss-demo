...
export const v1Menus: MenuGroup[] = [
  {
    name: '首页', code: 'HOME', children: [
      { name: '线索概览', path: '/leads/analytics', description: '作为合数BOSS默认首页，按引流期次概览线索从加微、问卷到成交退款的全链路经营表现。', features: ['日期/期次筛选', '店铺筛选', 'IP大类筛选', 'IP渠道筛选', 'IP名称筛选', '经营数据', '转化链路', '渠道分析', 'IP渠道分析', 'GMV与退款', '团队效能', '指标口径', '关键指标下钻', '经营报表导出'] }
    ]
  },
  {
    name: '线索中心', code: 'LEAD', children: [
      ...
      { name: 'IP管理', path: '/leads/ip', description: '管理IP大类、IP渠道、IP名称及稳定编号。', features: ['IP大类（字典）', 'IP渠道', 'IP名称', '自增IP编号', '新增', '编辑', '启停', '筛选'] },
      { name: '商品列表', path: '/leads/products', description: '查看第三方商品、店铺归属、活码分配状态，支持同步、筛选、导出与修改备注。', features: ['创建时间', '更新时间', '商品名称', '商品ID', '商品状态', '是否分配活码', '店铺名称', '店铺类型', '同步所有商品', '分平台同步', '同步任务记录', '修改备注'] },
  ...
  },
  {
    name: '客户中心', code: 'CUSTOMER', children: [
      ...
      { name: '企微标签', path: '/customers/tags', description: '按企业微信标签组维护企微企业标签和企微个人标签，并与当前企业主体同步。', features: ['企微企业标签', '企微个人标签', '标签分组', '批量新增标签', '编辑', '删除', '同步企微标签'] }
    ]
  },
  {
