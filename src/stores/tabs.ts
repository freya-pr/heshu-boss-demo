...
const NON_TAB_PAGE_PATHS = new Set<string>(['/system/applications'])
...
function restore(): WorkTab[] {
  try {
    ...
    return [
      HOME_TAB,
      ...saved.filter((item: WorkTab) => item.path !== HOME_TAB.path && menuByPath.has(item.path) && !NON_TAB_PAGE_PATHS.has(item.path))
    ]
  } catch {
    return [HOME_TAB]
  }
...
export const useTabsStore = defineStore('work-tabs', {
  ...
  actions: {
    open(tab: Omit<WorkTab, 'closable'>) {
      if (NON_TAB_PAGE_PATHS.has(tab.path)) {
        this.items = this.items.filter(item => item.path !== tab.path)
        this.persist()
        return
      }
