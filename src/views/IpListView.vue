...
const rows = ref<IpRow[]>([
  {
    id: 1, ipNo: 'IP000001', name: '合数家庭教育', description: '面向家庭教育与亲子关系的核心品牌IP',
  ...
  {
    id: 2, ipNo: 'IP000002', name: '学习力研究院', description: '聚焦青少年学习习惯与学习能力提升',
  ...
  {
    id: 3, ipNo: 'IP000003', name: '规划师说升学', description: '升学规划、选科与家庭决策内容IP',
...
const query = reactive({ keyword: '', platform: '', status: '' })
const platformDialogVisible = ref(false)
const productDrawerVisible = ref(false)
const editorVisible = ref(false)
const activeRow = ref<IpRow | null>(null)
const form = reactive({ name: '', description: '', platforms: [] as string[] })
...
function resetQuery() { Object.assign(query, { keyword: '', platform: '', status: '' }) }
function openPlatforms(row: IpRow) { activeRow.value = row; platformDialogVisible.value = true }
function openProducts(row: IpRow) { activeRow.value = row; productDrawerVisible.value = true }
function startCreate() { Object.assign(form, { name: '', description: '', platforms: [] }); editorVisible.value = true }
function createIp() {
  ...
  rows.value.unshift({ id: Date.now(), ipNo: `IP${String(sequence).padStart(6, '0')}`, name: form.name.trim(), description: form.description.trim(), platforms: [...form.platforms], products: [], creator: '林校长', createdAt: '2026-08-19 10:30', status: '启用' })
  editorVisible.value = false
  ElMessage.success('IP主档已创建')
}
function toggleStatus(row: IpRow) { row.status = row.status === '启用' ? '停用' : '启用'; ElMessage.success(`IP已${row.status}`) }
function meta(name: string) { return platformMeta[name] || { code: name.slice(0, 2), color: '#70839e' } }
</script>

<template>
  <section class="page ip-page">
    <PageHeader title="IP列表" description="统一维护内容IP主档，以及IP与商品、投放平台之间的关联关系。">
      <el-button type="primary" :icon="Plus" @click="startCreate">新增IP</el-button>
    </PageHeader>
    ...
    <article class="ip-ledger surface">
      ...
      <el-table :data="filteredRows" row-key="id">
        <el-table-column prop="ipNo" label="IP编号" width="135"><template #default="{ row }"><code>{{ row.ipNo }}</code></template></el-table-column>
        <el-table-column label="IP名称" min-width="230"><template #default="{ row }"><div class="ip-name"><b>{{ row.name }}</b><small>{{ row.description }}</small></div></template></el-table-column>
        ...
        <el-table-column label="操作" width="185" fixed="right"><template #default="{ row }"><el-button link type="primary" @click="openProducts(row)">详情</el-button><el-button link type="primary">编辑</el-button><el-button link type="primary" @click="toggleStatus(row)">{{ row.status === '启用' ? '停用' : '启用' }}</el-button></template></el-table-column>
      </el-table>
    </article>

    <el-drawer v-model="productDrawerVisible" :title="`${activeRow?.name || ''} · 关联商品`" size="760px">
      <div v-if="activeRow" class="relation-head"><code>{{ activeRow.ipNo }}</code><div><b>{{ activeRow.name }}</b><span>共关联 {{ activeRow.products.length }} 个商品</span></div></div>
      <el-table v-if="activeRow" :data="activeRow.products" row-key="id">
        <el-table-column prop="name" label="商品名称" min-width="170" />
        <el-table-column prop="thirdPartyId" label="第三方商品ID" min-width="150"><template #default="{ row }"><code>{{ row.thirdPartyId }}</code></template></el-table-column>
        <el-table-column prop="platform" label="平台" width="90" />
        <el-table-column prop="store" label="所属店铺" min-width="150" />
        <el-table-column label="状态" width="80"><template #default="{ row }"><el-tag :type="row.status === '在售' ? 'success' : 'info'">{{ row.status }}</el-tag></template></el-table-column>
      </el-table>
      <el-empty v-if="activeRow && !activeRow.products.length" description="暂未关联商品" />
    </el-drawer>
    ...
    <el-drawer v-model="editorVisible" title="新增IP" size="600px">
      <div class="editor-note"><b>IP编号将自动生成</b><span>保存后系统按当前最大编号顺序生成，编号不可修改。</span></div>
      <el-form label-position="top">
        <el-form-item label="IP名称" required><el-input v-model="form.name" maxlength="40" show-word-limit placeholder="请输入对内统一使用的IP名称" /></el-form-item>
