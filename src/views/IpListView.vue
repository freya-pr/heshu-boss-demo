...
<template>
  <section class="page ip-page">
    <PageHeader title="IP管理" description="统一维护IP名称、IP大类与IP渠道的绑定关系。">
      <el-button type="primary" :icon="Plus" @click="openEditor()">新增IP</el-button>
    </PageHeader>
    ...
    <article class="ip-ledger surface">
      ...
      <el-table :data="filteredRows" row-key="id">
        ...
        <el-table-column label="操作" width="130" fixed="right"><template #default="{ row }"><el-button link type="primary" @click="openEditor(row)">编辑</el-button><el-button link type="primary" @click="toggleStatus(row)">{{ row.status === '启用' ? '停用' : '启用' }}</el-button></template></el-table-column>
      </el-table>
    </article>
...
.ip-summary{grid-template-columns:1fr}.ip-filter{grid-template-columns:1.5fr .75fr .65fr .65fr auto auto}
</style>
