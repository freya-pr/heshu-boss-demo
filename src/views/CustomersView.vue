  ...
  <section class="page customer-list-page">
    ...
    <el-drawer v-model="detailDrawer" size="880px">
      ...
      <el-tabs v-model="detailTab" class="profile-tabs">
        <el-tab-pane label="基本档案" name="profile"><div class="profile-grid"><div><span>昵称</span><b>{{ customerProfile.nickname }}</b></div><div><span>手机号</span><b class="detail-mobile">{{ detailMobileVisible?(activeCustomer?.mobile||'—'):maskMobile(activeCustomer?.mobile) }}<el-button link type="primary" :icon="View" aria-label="预览手机号" @click="detailMobileVisible=!detailMobileVisible"/></b></div><div class="wide"><span>地址</span><b>{{ customerProfile.address }}</b></div><div><span>性别</span><b>{{ customerProfile.gender }}</b></div><div><span>客户编号</span><b>{{ activeCustomer?.customer_no }}</b></div><div><span>企微客户ID</span><b>{{ activeCustomer?.external_user_id||'wmfo4ODQAAHOWLykyqvVwbtGtgP8Fluw' }}</b></div><div><span>UnionID</span><b>{{ activeCustomer?.union_id||'oHhQ26ieKZgGhi9Go07upN8bIPQk' }}</b></div></div></el-tab-pane>
