...
type TagGroup = { id:number; name:string; departments:string[]; tags:TagRow[] }
...
const groupForm = reactive({ name:'', departments:['全部部门'] as string[], tagName:'', tags:[] as string[] })
const groupEditForm = reactive({ name:'', departments:['全部部门'] as string[] })
...
const groups = ref<TagGroup[]>([
  { id:1, name:'客户阶段', departments:['全部部门'], tags:[
  ...
  { id:2, name:'购买意向', departments:['深圳二转组','课程顾问部'], tags:[
  ...
  { id:3, name:'运营跟进', departments:['全部部门'], tags:[{ id:301, name:'重点客户', externalId:'etKfAQAAE6519', customers:2320, creator:'陈老师', updatedAt:'2026-09-07 11:08' }]},
  { id:4, name:'课程偏好', departments:['全部部门'], tags:[] },
  { id:5, name:'来源渠道', departments:['全部部门'], tags:[] }
])
const departmentTree = [
  { value:'全部部门', label:'全部部门' },
  { value:'光合成长教育', label:'光合成长教育', children:[
    { value:'光合新家人', label:'光合新家人', children:[
      { value:'深圳二转组', label:'深圳二转组' },
      { value:'课程顾问部', label:'课程顾问部' },
      { value:'培训团队', label:'培训团队' },
      { value:'北京HRBP', label:'北京HRBP' },
      { value:'其他（待设置账号）', label:'其他（待设置账号）' }
    ]},
    { value:'总经办', label:'总经办' }
  ]}
]
function departmentText(departments:string[]){ return departments.join('、') }
function normalizeDepartments(values:string[], target:'create'|'edit'){
  let next=[...values]
  if(next.length>1 && next.includes('全部部门')) next=next.at(-1)==='全部部门'?['全部部门']:next.filter(item=>item!=='全部部门')
  if(target==='create') groupForm.departments=next
  else groupEditForm.departments=next
}
...
const globalTags = computed<GlobalTagRow[]>(() => {
  ...
  return currentGroups.value.flatMap(group => group.tags.map(tag => ({ ...tag, groupId:group.id, groupName:group.name, department:departmentText(group.departments) }))).filter(tag => !query || `${tag.name}${tag.externalId}${tag.groupName}`.toLowerCase().includes(query))
})
...
function openGroupCreator(){ Object.assign(groupForm,{name:'',departments:['全部部门'],tagName:'',tags:[]}); addGroupVisible.value=true }
...
function openGroupEditor(group:TagGroup){ editingGroupId.value=group.id; Object.assign(groupEditForm,{name:group.name,departments:[...group.departments]}); editGroupVisible.value=true }
function saveGroupEdit(){
  ...
  if(!groupEditForm.departments.length) return ElMessage.warning('请至少选择一个适用部门')
  if(group) Object.assign(group,{name,departments:[...groupEditForm.departments]})
...
function createGroup(){
  ...
  if(!groupForm.departments.length) return ElMessage.warning('请至少选择一个适用部门')
  ...
  currentGroups.value.unshift({ id:now, name, departments:[...groupForm.departments], tags }); selectedGroupId.value=now
  addGroupVisible.value=false; Object.assign(groupForm,{name:'',departments:['全部部门'],tagName:'',tags:[]}); ElMessage.success(`分组已创建，并新增 ${tags.length} 个标签`)
}
...
<template>
  <div class="page wecom-tag-page">
    ...
    <el-dialog v-model="addGroupVisible" title="新增分组" width="720px" class="group-create-dialog"><el-form label-position="top"><section class="group-form-block"><el-form-item label="分组名称" required><el-input v-model="groupForm.name" maxlength="15" show-word-limit placeholder="请输入标签组名称"/></el-form-item><el-form-item label="适用部门" required><p class="field-tip">所选部门均可使用及编辑该分组下的企微标签</p><el-tree-select v-model="groupForm.departments" :data="departmentTree" multiple show-checkbox check-strictly default-expand-all collapse-tags collapse-tags-tooltip clearable node-key="value" placeholder="请选择一个或多个部门" style="width:100%" @change="normalizeDepartments($event,'create')"/></el-form-item></section><section class="group-form-block tag-create-block"><el-form-item label="标签" required><div class="single-tag-row"><el-input v-model="groupForm.tagName" maxlength="15" show-word-limit placeholder="请输入标签名称" @keyup.enter="addGroupTag"/><el-button circle :icon="Plus" aria-label="添加标签" @click="addGroupTag"/></div></el-form-item><el-button link type="primary" class="batch-create-link" @click="openBatchGroupTags">批量新建</el-button><div v-if="groupForm.tags.length" class="pending-tags"><el-tag v-for="(tag,index) in groupForm.tags" :key="tag" closable effect="plain" @close="removeGroupTag(index)">{{ tag }}</el-tag></div></section></el-form><template #footer><el-button @click="addGroupVisible=false">取消</el-button><el-button type="primary" @click="createGroup">保存</el-button></template></el-dialog>
    <el-dialog v-model="editGroupVisible" title="编辑标签分组" width="560px"><el-form label-position="top" class="dialog-form"><el-form-item label="分组名称" required><el-input v-model="groupEditForm.name" maxlength="15" show-word-limit placeholder="请输入标签组名称"/></el-form-item><el-form-item label="适用部门" required><p class="field-tip">所选部门均可使用及编辑该分组下的企微标签</p><el-tree-select v-model="groupEditForm.departments" :data="departmentTree" multiple show-checkbox check-strictly default-expand-all collapse-tags collapse-tags-tooltip clearable node-key="value" placeholder="请选择一个或多个部门" style="width:100%" @change="normalizeDepartments($event,'edit')"/></el-form-item></el-form><template #footer><el-button @click="editGroupVisible=false">取消</el-button><el-button type="primary" @click="saveGroupEdit">确定</el-button></template></el-dialog>
