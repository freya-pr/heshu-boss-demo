...
async function load() { loading.value = true; error.value = ''; try { const effectiveViewScope = auth.user?.role === 'ADMIN' ? scopeFilters.value.viewScope : 'SELF'; const result: any = await http.get('/leads/analytics', { params: { dateRange: dateRange.value, period: period.value, channel: channel.value, organizationId: scopeFilters.value.organizationId, ownerId: scopeFilters.value.ownerId, viewScope: effectiveViewScope } }); data.value = result.data } catch (e: any) { error.value = e.message || '数据分析加载失败' } finally { loading.value = false } }
function exportReport() { ElMessage.success('数据分析报表已生成，开始下载') }
onMounted(async () => { const [organizationResult, employeeResult]: any = await Promise.all([http.get('/system/organizations'), http.get('/system/employees')]); organizations.value = organizationResult.data; employees.value = employeeResult.data; await load() })
</script>
    ...
    <StatePanel :loading="loading" :error="error" @retry="load">
      ...
      <div class="analytics-layout lower">
        ...
        <article class="surface channel-panel">
          <div class="panel-heading"><div><span class="section-kicker">CHANNEL BREAKDOWN</span><h3>渠道结果</h3></div><router-link to="/leads/channels?tab=analysis">进入渠道管理分析 →</router-link></div>
...
.analytics-filter{display:block}
.analytics-filter .analytics-business-filter{display:flex;align-items:center;gap:10px;margin-top:12px}
.analytics-filter .analytics-business-filter :deep(.el-date-editor){width:260px}
.analytics-filter .analytics-business-filter :deep(.el-select){width:174px}
.analytics-filter .analytics-business-filter>span{margin-left:auto;color:var(--muted);font-size:12px}
</style>
