...
const periodOperatingGroups = computed<PeriodMetricGroup[]>(() => {
  ...
  return [
    { label: '过程转化', tone: 'process', metrics: [
      ...
      { key: 'reservation', label: '预约率', value: pct(div(f.reservation, lead)), sub: format(f.reservation), count: f.reservation },
      { key: 'arrival', label: '到课率', value: pct(div(f.arrival, lead)), sub: format(f.arrival), count: f.arrival },
      { key: 'completion', label: '完课率', value: pct(div(f.completion, lead)), sub: format(f.completion), count: f.completion }
    ] },
