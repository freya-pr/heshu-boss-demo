...
function reportRows() {
  ...
  const rows: any[][] = [
    ...
    ['团队转化离散率', `${e.conversionDispersion}%`, '转化率标准差 / 平均转化率'], [], ['IP渠道分析'],
    ['IP渠道', '渠道编号', 'IP数', '线索数', '加微数', '问卷数', '预约数', '到课数', '完课数', '成交数', '退款数', '最终转化率', '毛GMV', '净GMV']
  ]
...
<template>
  <section class="page lead-analytics-page">
    <PageHeader eyebrow="LEAD PERFORMANCE · 营期经营驾驶舱" title="线索概览" description="从营期结果回看线索旅程，并继续下钻到IP渠道、IP名称和商品。">
    ...
    <StatePanel :loading="loading" :error="error" @retry="load">
      <article class="period-strip surface">
        ...
        <el-button type="primary" plain @click="exportReport">导出总览 + IP渠道 + 商品</el-button>
      </article>
