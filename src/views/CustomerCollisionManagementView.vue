...
const summary = computed(() => ({
  ...
  highRisk: rows.value.filter(item => item.risk_level === 'HIGH' && item.status !== 'RESOLVED').length,
...
async function load() {
  ...
  try {
    const response = await http.get('/customers/collisions', { params: { keyword: keyword.value, status: status.value, sourceType: sourceType.value, riskLevel: riskLevel.value } })
    rows.value = response.data
  } catch (event: any) { error.value = event.message || '撞单案件加载失败' }
  finally { loading.value = false }
}
function reset() {
  keyword.value = ''; status.value = ''; sourceType.value = ''; riskLevel.value = ''; dateRange.value = []
  load()
}
...
<template>
  <div>
    ...
    <section class="collision-metrics">
      ...
      <article class="risk"><span>未结高风险</span><b>{{ summary.highRisk }}</b><small>两项强身份发生冲突</small></article>
    ...
    <section class="surface filter-panel">
      ...
      <el-select v-model="riskLevel" clearable placeholder="全部风险"><el-option label="高风险" value="HIGH"/><el-option label="中风险" value="MEDIUM"/></el-select>
    ...
    <section class="surface table-shell collision-table">
      ...
      <el-table v-else :data="filteredRows" row-key="id">
        ...
        <el-table-column label="风险 / 状态" width="130"><template #default="{row}"><el-tag :type="row.risk_level === 'HIGH' ? 'danger' : 'warning'" effect="light">{{ row.risk_level === 'HIGH' ? '高风险' : '中风险' }}</el-tag><small><el-tag :type="statusTag[row.status]" effect="plain">{{ statusLabels[row.status] }}</el-tag></small></template></el-table-column>
