...
const query = reactive({ keyword: '', platform: '' as '' | Platform, status: '' as '' | StoreStatus, dateType: '创建时间' as ArchiveDateType, dateRange: [] as string[] })
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter(row => (!term || `${row.name}${row.thirdPartyStoreId}`.toLowerCase().includes(term))
...
function resetQuery() { Object.assign(query, { keyword: '', platform: '', status: '', dateType: '创建时间', dateRange: [] }) }
...
function saveStore() {
  ...
  const owner = employees.find(item => item.id === form.ownerId) || employees[0]
  if (editingId.value) {
...
<template>
  <section class="page store-page">
    <PageHeader title="店铺管理" description="统一维护外部平台店铺身份及经营归因信息。">
    ...
    <div class="store-filter surface">
      ...
      <div class="filter-controls">
        <el-input v-model="query.keyword" clearable :prefix-icon="Search" placeholder="店铺名称或第三方店铺ID" />
    ...
    <div class="stats-scope surface">
      <div class="scope-heading"><span>经营统计范围</span><small>列表指标统一采用线索归因口径</small></div>
      <el-tag type="primary" effect="dark" size="large">线索归因口径</el-tag>
      ...
      <el-select v-model="analyticsCamp" filterable placeholder="所属期次"><el-option label="全部期次" value="全部期次"/><el-option label="2026年8月第1期" value="2026年8月第1期"/><el-option label="2026年8月第2期" value="2026年8月第2期"/><el-option label="2026年8月第3期" value="2026年8月第3期"/></el-select>
    ...
    <article class="store-ledger surface">
      ...
      <el-table :data="filteredRows" row-key="id">
        ...
        <el-table-column label="操作" width="145" fixed="right"><template #default="{ row }"><el-button link type="primary" :icon="EditPen" @click="openEdit(row)">编辑</el-button><el-dropdown trigger="click"><el-button link type="primary">更多⌄</el-button><template #dropdown><el-dropdown-menu><el-dropdown-item @click="toggleStatus(row)">{{ row.status === '启用' ? '停用店铺' : '启用店铺' }}</el-dropdown-item><el-dropdown-item @click="ElMessage.info(`最近更新：${row.updatedAt}`)">查看变更时间</el-dropdown-item></el-dropdown-menu></template></el-dropdown></template></el-table-column>
      </el-table>
    ...
    <el-drawer v-model="analyticsVisible" title="店铺经营详情" size="920px" class="store-analytics-drawer">
      <template v-if="activeStore">
        ...
        <div class="analytics-filter">
          ...
          <el-select v-model="analyticsCamp" placeholder="所属期次"><el-option label="全部期次" value="全部期次"/><el-option label="2026年8月第1期" value="2026年8月第1期"/><el-option label="2026年8月第2期" value="2026年8月第2期"/><el-option label="2026年8月第3期" value="2026年8月第3期"/></el-select>
...
.filter-controls{grid-template-columns:1.45fr .65fr .6fr .68fr 1.35fr auto auto}
</style>
