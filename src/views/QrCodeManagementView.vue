...
type LiveCode = {
  ...
  autoAccept: boolean; scheduleType: '全天在线' | '分时段排班'; allocationMode: '轮询' | '顺序' | '随机'; scheduledEmployeeIds: number[]; workDays: number[]; onlineTime: string[]; scheduleRules?: ScheduleRule[]; fallbackEmployeeIds?: number[];
...
function openEdit(row: LiveCode) {
  ...
  Object.assign(form, { name: row.name, type: row.type, groupId: row.groupId, ipNo: row.ipNo || '', campId: row.campId, autoAccept: row.autoAccept, scheduleType: row.scheduleType, allocationMode: row.allocationMode, scheduledEmployeeIds: [...row.scheduledEmployeeIds], workDays: [...row.workDays], onlineTime: [...row.onlineTime], scheduleRules, fallbackEmployeeIds: row.fallbackEmployeeIds?.length ? [...row.fallbackEmployeeIds] : row.employees.map(item => item.id), employeeIds: row.employees.map(item => item.id), wecomTags: [...row.wecomTags], internalTags: [...row.internalTags], status: row.status })
...
function save() {
  ...
  if (editingId.value) {
    ...
    rows.value.unshift({ id: Date.now(), codeNo: `QR${Date.now().toString().slice(-11)}`, name: form.name, type: form.type, corpId: selectedCorp.value.corpId, corpName: selectedCorp.value.name, groupId: group.id, groupName: group.name, ...ipBinding, campId: camp?.id, campName: camp?.name, autoAccept: form.autoAccept, scheduleType: form.scheduleType, allocationMode: form.allocationMode, ...scheduleConfig, status: '启用', employees, wecomTags: [...form.wecomTags], internalTags: [...form.internalTags], scans: 0, added: 0, creatorName: '林校长', createdAt: new Date().toLocaleString('zh-CN', { hour12: false }) })
  ...
  <section class="page qr-page">
    ...
    <article class="code-list surface">
      ...
      <el-table :data="filteredRows" row-key="id">
        ...
        <el-table-column label="操作" width="190" fixed="right"><template #default="{ row }"><el-button link type="primary" :icon="View" @click="showDetail(row)">详情</el-button><el-button link type="primary" :icon="EditPen" @click="openEdit(row)">编辑</el-button><el-dropdown trigger="click"><el-button link type="primary">更多⌄</el-button><template #dropdown><el-dropdown-menu><el-dropdown-item @click="toggleStatus(row)">{{ row.status === '启用' ? '停用活码' : '启用活码' }}</el-dropdown-item></el-dropdown-menu></template></el-dropdown></template></el-table-column>
      </el-table>
