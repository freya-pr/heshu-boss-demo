  ...
  <section class="page qr-page">
    ...
    <article class="code-list surface">
      ...
      <el-table :data="filteredRows" row-key="id">
        ...
        <el-table-column label="所属期次 / 班级" min-width="230"><template #default="{ row }"><div class="camp-cell"><b>{{ row.campName || '通用活码' }}</b><span>{{ row.className || '未关联班级' }}</span></div></template></el-table-column>
    ...
    <el-drawer v-model="drawerVisible" :title="editingId ? '编辑活码' : '新建活码'" size="760px" class="qr-config-drawer">
      ...
      <el-form label-position="top" class="qr-form">
        <div class="form-section"><h4>基础信息</h4><div class="form-grid">
          ...
          <el-form-item><template #label>所属期次 <em class="optional">选填</em></template><el-select v-model="form.campId" clearable filterable placeholder="从引流期次库选择" @change="onCampChange"><el-option v-for="item in camps" :key="item.id" :label="item.name" :value="item.id" /></el-select><small>不选择时按通用活码保存；期次来源于线索中心—引流期次。</small></el-form-item>
          <el-form-item><template #label>所属班级 <em class="optional">选填</em></template><el-select v-model="form.classId" clearable filterable :disabled="!form.campId" placeholder="选择所属期次后可选班级"><el-option v-for="item in classOptions" :key="item.id" :label="item.name" :value="item.id" /></el-select><small>不选择班级时，活码仅归属期次。</small></el-form-item>
