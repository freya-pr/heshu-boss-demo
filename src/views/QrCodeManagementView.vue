...
const form = reactive({ name: '', type: '多人活码' as LiveCode['type'], groupId: 1, campId: undefined as number | undefined, classId: undefined as number | undefined, receptionRange: [] as string[], employeeIds: [] as number[], wecomTags: [] as string[], internalTags: [] as string[], status: '启用' as LiveCode['status'] })
...
function resetForm() { Object.assign(form, { name: '', type: '多人活码', groupId: selectedGroupId.value === 'all' ? 1 : selectedGroupId.value, campId: undefined, classId: undefined, receptionRange: [], employeeIds: [], wecomTags: [], internalTags: [], status: '启用' }) }
function openCreate() { editingId.value = null; resetForm(); drawerVisible.value = true }
function openEdit(row: LiveCode) {
  editingId.value = row.id
  Object.assign(form, { name: row.name, type: row.type, groupId: row.groupId, campId: row.campId, classId: row.classId, receptionRange: [row.receptionStart, row.receptionEnd], employeeIds: row.employees.map(item => item.id), wecomTags: [...row.wecomTags], internalTags: [...row.internalTags], status: row.status })
...
function save() {
  ...
  if (editingId.value) {
    ...
    Object.assign(target, { ...form, status: target.status === '停用' ? '停用' : dateStatus, groupName: group.name, campName: camp?.name, className: classItem?.name, receptionStart: form.receptionRange[0] || '', receptionEnd: form.receptionRange[1] || '', employees })
    ElMessage.success('活码配置已更新')
  } else {
    rows.value.unshift({ id: Date.now(), codeNo: `QR${Date.now().toString().slice(-11)}`, name: form.name, type: form.type, groupId: group.id, groupName: group.name, campId: camp?.id, campName: camp?.name, classId: classItem?.id, className: classItem?.name, receptionStart: form.receptionRange[0] || '', receptionEnd: form.receptionRange[1] || '', status: dateStatus, employees, wecomTags: [...form.wecomTags], internalTags: [...form.internalTags], scans: 0, added: 0, creatorName: '林校长', createdAt: new Date().toLocaleString('zh-CN', { hour12: false }) })
    ...
    <el-drawer v-model="drawerVisible" :title="editingId ? '编辑活码' : '新建活码'" size="760px" class="qr-config-drawer">
      ...
      <el-form label-position="top" class="qr-form">
        <div class="form-section"><h4>基础信息</h4><div class="form-grid">
          ...
          <el-form-item><template #label>所属营期 <em class="optional">选填</em></template><el-select v-model="form.campId" clearable filterable placeholder="从营期库选择" @change="onCampChange"><el-option v-for="item in camps" :key="item.id" :label="item.name" :value="item.id" /></el-select><small>不选择时按通用活码保存；数据来源：交付中心－营期管理。</small></el-form-item>
